...
} from "#civ7-control-service/model/policy/direct-control-failure";
import type {
...
const DEFAULT_LIFECYCLE_POLL_MS = 1000;
const DEFAULT_LIFECYCLE_SETUP_WAIT_MS = 30000;
const DEFAULT_LIFECYCLE_START_WAIT_MS = 120000;
...
type PollState<A> =
  ...
  | PollResult<A>;
type FailedRuntimeProbeObservation = Readonly<{
  ok: false;
  error: string;
}>;
type FiniteNumberProbeObservation = Readonly<{
  ok: true;
  value: number;
}>;
type StringProbeObservation = Readonly<{
  ok: true;
  value: string;
}>;
type BooleanProbeObservation = Readonly<{
  ok: true;
  value: boolean;
}>;
type SavedConfigLoadBaseline =
  | Readonly<{
      kind: "confirmed-response";
      revision: number;
    }>
  | Readonly<{
...
export const singlePlayerStart = module.singlePlayer.start.effect(function* ({
  input,
  context,
  errors,
}) {
  ...
  const transition = yield* Effect.fromNullable(
    ...
    Effect.mapError(() => verificationFailure("admit-setup-phase", "inconsistent-admission-result"))
  );
...
function isPlotCount(value: number): boolean {
  return Number.isInteger(value) && value >= 1 && value <= 100000000;
}
...
function isBeginRefusal(value: unknown): value is Readonly<{
  accepted: false;
  reason: string;
}> {
...
function setupRevisionOption(result: {
  snapshot: {
    setup: {
      revision: unknown;
    };
...
function hasDirectControlCode<Code extends Civ7DirectControlErrorCode>(
  cause: unknown,
  code: Code
): cause is Civ7DirectControlErrorShape &
  Readonly<{
    code: Code;
  }> {
