...
import type { Civ7ControlOrpcProgressionTargetResult } from "#civ7-control-service/model/ports/direct-control";
import type {
...
type ProgressionTargetResult =
  | Civ7ProgressionTechnologyTargetResult
  | Civ7ProgressionCultureTargetResult;
type ProgressionTargetRuntimeInput = Civ7ProgressionTargetInput &
  Readonly<{
    playerId: number;
  }>;
export const targetRequest = {
  progressionTechnologyTargetRequestProcedure: civ7ControlOrpcMutationProcedure(
    module.technology.target.request
  ...
  progressionCultureTargetRequestProcedure: civ7ControlOrpcMutationProcedure(
    module.culture.target.request
  ).effect(function* ({ context, errors, input }) {
    const kind = "culture";
