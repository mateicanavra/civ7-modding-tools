...
const civilization = new CivilizationBuilder({
  ...
  civilizationTraits: [TRAIT.ANTIQUITY_CIV, TRAIT.ATTRIBUTE_MILITARISTIC, TRAIT.ATTRIBUTE_CULTURAL],
  ...
  localizations: [
    {
      ...
      abilityDescription:
        "[LIST][LI]Units receive +5 [icon:YIELD_COMBAT] Combat Strength when fighting in Hills or Forest terrain.[/LI][LI]Gold Mines provide +1 [icon:YIELD_PRODUCTION] Production and +1 [icon:YIELD_CULTURE] Culture.[/LI][/LIST]",
    },
  ],
});
  ...
  localizations: [
    {
      name: "Falx Warrior",
      description:
        "Dacian unique melee unit that replaces the Swordsman. +5 [icon:YIELD_COMBAT] Combat Strength when fighting on Hills.",
    },
  ],
});
...
const mountainSanctuary = new ConstructibleBuilder({
  ...
  constructibleMaintenances: [{ yieldType: YIELD.GOLD, amount: 1 }],
  localizations: [
    {
