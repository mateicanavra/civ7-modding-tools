...
import { measureStandardRiverNetwork } from "../../../../../../src/recipes/standard/metrics/families/hydrology/river-network.js";

const { classifyRiverNetwork } = hydrologyOpsPublic.hydrography.ops;
const defaultClassificationSelection = {
...
describe("Standard river-network measurements", () => {
  it("derives upstream area, stream hierarchy, and accepted-lake mouths", () => {
    ...
    const result = runAdmittedOperationForTest(
      ...
      defaultClassificationSelection
    );
    ...
    expect(measurements.lowOrderRiverShareOfRiverTiles).toBe(1);
  ...
  it("records endorheic closed-basin terminals as typed hydrology outcomes", () => {
    ...
    expect(measurements.closedOrLakeTerminalLandShare).toBe(1);
    expect(measurements.lakeConnectedTerminalDischargeShare).toBe(0);
    expect(measurements.nonPerennialRiverShareOfRiverTiles).toBeGreaterThan(0);
  });
  ...
  it("keeps wet headwater channels as the majority before a major trunk", () => {
    ...
    expect(measurements.minorRiverShareOfRiverTiles).toBe(0.9);
    expect(measurements.majorRiverShareOfRiverTiles).toBe(0.1);
    expect(measurements.lowOrderRiverShareOfRiverTiles).toBe(1);
    expect(measurements.nonPerennialRiverShareOfRiverTiles).toBeGreaterThan(0.8);
  ...
  it("marks spill-routed paths separately from direct ocean mouths", () => {
    ...
    expect(measurements.lakeLandShare).toBe(0);
    expect(measurements.riverLandShare).toBeCloseTo(4 / 5);
    expect(measurements.lowOrderRiverShareOfRiverTiles).toBe(1);
    expect(measurements.nonDryFlowLandShare).toBeCloseTo(4 / 5);
  ...
  it("surfaces routing and basin health counters without hiding typed output", () => {
    ...
    expect(measurements.invalidReceiverTileCount).toBe(1);
    expect(measurements.downstreamDischargeDropEdgeCount).toBe(1);
    expect(measurements.assignedBasinLandTileCount).toBe(2);
    expect(measurements.unassignedBasinLandTileCount).toBe(1);
    expect(measurements.unresolvedMouthTileCount).toBe(2);
    expect(measurements.closedBasinMouthTileCount).toBe(1);
    expect(measurements.riverTileCount).toBe(2);
    expect(measurements.riverDryTileCount).toBe(0);
    expect(measurements.riverIntermittentTileCount).toBe(1);
    expect(measurements.riverPerennialTileCount).toBe(1);
