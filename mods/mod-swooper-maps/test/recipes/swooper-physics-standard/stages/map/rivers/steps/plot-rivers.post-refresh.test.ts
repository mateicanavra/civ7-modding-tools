...
describe("map-rivers/plot-rivers", () => {
  it("stamps MapGen-projected navigable rivers and refreshes downstream caches", () => {
    ...
    expect(mapRiversArtifactModules.engineProjectionRivers.artifact.id).toBe(
    ...
    const readback = readValidatedArtifact(
      context,
  ...
  it("preserves diagnostic evidence when runtime validation dries a planned sink", () => {
    ...
    withMapContextExecutionForTest(context, (stepContext) => {
      ...
      LakesStep.run(stepContext, {}, {}, buildStepTestDependencies(LakesStep));
      PlotRiversStep.run(
        stepContext,
        {
          selectNavigableRiverTerrain: {
    ...
    expect(projection.sinkMismatchCount).toBeGreaterThanOrEqual(1);
