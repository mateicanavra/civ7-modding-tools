...
import { selectNavigableRiverTerrain } from "../../../../../../../../../src/recipes/standard/stages/map/rivers/steps/plot-rivers/rules/select-navigable-river-terrain.js";
...
describe("select navigable river terrain", () => {
  it("prefers the strongest major-discharge trunk when density only allows one chain", () => {
    ...
    const result = selectNavigableRiverTerrain(
      {
        width,
        height,
        riverClass,
      ...
      { endpointDischargePercentileMin: 1, targetMajorTileFraction: 0.3 }
    );
  ...
  it("fills the target budget from admitted endpoints", () => {
    ...
    const result = selectNavigableRiverTerrain(
      ...
      { endpointDischargePercentileMin: 0, targetMajorTileFraction: 0.7 }
    );
  ...
  it("does not backfill endpoints below the discharge floor", () => {
    ...
    const result = selectNavigableRiverTerrain(
      ...
      { endpointDischargePercentileMin: 1, targetMajorTileFraction: 0.7 }
    );
  ...
  it("does not promote minor rivers into navigable river terrain", () => {
    ...
    const result = selectNavigableRiverTerrain(
      ...
      { endpointDischargePercentileMin: 0, targetMajorTileFraction: 1 }
    );
  ...
  it("continues Hydrology major trunks through nonprojectable lake gaps without stamping them", () => {
    ...
    const result = selectNavigableRiverTerrain(
      ...
      { endpointDischargePercentileMin: 1, targetMajorTileFraction: 1 }
    );
