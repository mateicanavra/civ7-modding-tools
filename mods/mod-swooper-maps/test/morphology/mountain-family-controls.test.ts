...
import { forEachHexNeighborOddQ, resolveTileAreaSpacingTarget } from "@swooper/mapgen-core/lib/grid";
...
function getMountainComponentSizes(mask: Uint8Array, width: number, height: number): number[] {
...
function countMountainComponents(mask: Uint8Array, width: number, height: number): number {
  return getMountainComponentSizes(mask, width, height).length;
}
...
describe("morphology mountain-family controls", () => {
  it("derives official Civ7 map-size range-system counts from one spacing input", () => {
  ...
  it("uses Earth-scaled range targets to create multiple multi-tile ridge components", () => {
    ...
    expect(countMountainComponents(result.mountainMask, width, height)).toBeGreaterThanOrEqual(3);
    expect(countMask(result.mountainMask)).toBeGreaterThanOrEqual(6);
  });

  it("carries a major range along a Large-map tectonic corridor", () => {
    const width = 96;
    const height = 60;
    const input = createRidgeInput(width, height);
    ...
    const result = planRidges.run(input, {
      ...
      config: {
        ...
        mountainMinFraction: 0,
        mountainSpineFraction: 0.01,
        mountainRangeSpacingTiles: Math.sqrt((width * height) / 1),
    ...
    const componentSizes = getMountainComponentSizes(result.mountainMask, width, height);
    const regionSizes = getMountainComponentSizes(result.mountainRegionMask, width, height);
    expect(componentSizes[0] ?? 0).toBeGreaterThanOrEqual(25);
    expect(regionSizes[0] ?? 0).toBeGreaterThan(componentSizes[0] ?? 0);
  });
  ...
  it("distributes Earth-scaled range systems instead of collapsing them into the highest-score belts", () => {
    ...
    for (const center of centers) {
      ...
      forEachHexNeighborOddQ(center.x, center.y, width, height, (nx, ny) => {
        input.fractalMountain[ny * width + nx] = Math.max(input.fractalMountain[ny * width + nx] ?? -500, 80);
      });
    }
    ...
    const topTwoShare = ((componentSizes[0] ?? 0) + (componentSizes[1] ?? 0)) / Math.max(1, mountainCount);
    expect(componentSizes.filter((size) => size >= 2).length).toBeGreaterThanOrEqual(8);
    expect(topTwoShare).toBeLessThanOrEqual(0.35);
  });

  it("uses distributed physics-gated candidates when broad orogenic fields have too few strict maxima", () => {
    const width = 36;
    const height = 12;
    const input = createRidgeInput(width, height);
    input.fractalMountain.fill(200);
    ...
    const result = planRidges.run(input, {
      ...
      config: {
        ...
        mountainMinFraction: 0,
        mountainSpineFraction: 0.01,
        mountainRangeSpacingTiles: Math.sqrt((width * height) / 8),
        mountainSpineDilationSteps: 0,
        mountainSpineMinDistance: 1,
        mountainThreshold: 0.1,
        mountainShoulderThresholdScale: 0.1,
    ...
    expect(countMask(result.mountainMask)).toBeGreaterThanOrEqual(6);
    expect(countMountainComponents(result.mountainMask, width, height)).toBeGreaterThanOrEqual(6);
  });

  it("publishes broader orogenic province footprints around mountain spines", () => {
    const width = 24;
    const height = 8;
    const input = createRidgeInput(width, height);
    input.fractalMountain.fill(-500);
