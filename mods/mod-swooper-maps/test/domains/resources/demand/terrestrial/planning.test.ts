...
describe("terrestrial resource operation contract", () => {
  it("plans all terrestrial resource rows symbolically without runtime ids", () => {
    ...
    const selection = normalizeOperationSelectionForTest(
      resources.demand.ops.planTerrestrialResources,
      structuredClone(resources.demand.ops.planTerrestrialResources.defaultConfig)
    );

    const result = resources.demand.ops.planTerrestrialResources.run(
      {
        width,
        height,
