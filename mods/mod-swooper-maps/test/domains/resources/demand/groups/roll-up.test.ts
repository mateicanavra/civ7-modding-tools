...
type AquaticExpectation = Parameters<
  typeof resources.demand.ops.planAquaticResources.run
>[0]["expectations"][number];
type CultivatedExpectation = Parameters<
  typeof resources.demand.ops.planCultivatedResources.run
>[0]["expectations"][number];
type TerrestrialExpectation = Parameters<
  typeof resources.demand.ops.planTerrestrialResources.run
>[0]["expectations"][number];
type GeologicalExpectation = Parameters<
  typeof resources.demand.ops.planGeologicalResources.run
>[0]["expectations"][number];
...
describe("resource group rollup operation contract", () => {
  it("publishes one warning-only group plan artifact across all symbolic resource groups", () => {
    const selection = normalizeOperationSelectionForTest(
      resources.demand.ops.planResourceGroups,
      structuredClone(resources.demand.ops.planResourceGroups.defaultConfig)
    );

    const result = resources.demand.ops.planResourceGroups.run(
  ...
  it("preserves group-level blocked, missing-signal, and missing-expectation counts", () => {
    ...
    const result = resources.demand.ops.planResourceGroups.run(
      {
        ...plans,
        geologicalPlan: resources.demand.ops.planGeologicalResources.run(
          {
            width,
            height,
          ...
          resources.demand.ops.planGeologicalResources.defaultConfig
        ),
      },
      resources.demand.ops.planResourceGroups.defaultConfig
    );
...
function allGroupPlans(dimensions: Readonly<{ width: number; height: number }>) {
  const { width, height } = dimensions;
  const size = width * height;
  return {
    aquaticPlan: resources.demand.ops.planAquaticResources.run(
      {
        width,
        height,
      ...
      resources.demand.ops.planAquaticResources.defaultConfig
    ),
    cultivatedPlan: resources.demand.ops.planCultivatedResources.run(
      {
        width,
        height,
      ...
      resources.demand.ops.planCultivatedResources.defaultConfig
    ),
    terrestrialPlan: resources.demand.ops.planTerrestrialResources.run(
      {
        width,
        height,
      ...
      resources.demand.ops.planTerrestrialResources.defaultConfig
    ),
    geologicalPlan: resources.demand.ops.planGeologicalResources.run(
      {
        width,
