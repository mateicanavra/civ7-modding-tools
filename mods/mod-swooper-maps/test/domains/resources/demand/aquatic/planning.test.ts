...
describe("aquatic resource operation contract", () => {
  it("plans all aquatic resource rows symbolically without runtime ids", () => {
    ...
    const selection = normalizeOperationSelectionForTest(
      resources.demand.ops.planAquaticResources,
      structuredClone(resources.demand.ops.planAquaticResources.defaultConfig)
    );

    const result = resources.demand.ops.planAquaticResources.run(
      {
        width,
        height,
