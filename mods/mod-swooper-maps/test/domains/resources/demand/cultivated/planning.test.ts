...
describe("cultivated resource operation contract", () => {
  it("plans all cultivated resource rows symbolically without runtime ids", () => {
    ...
    const selection = normalizeOperationSelectionForTest(
      resources.demand.ops.planCultivatedResources,
      structuredClone(resources.demand.ops.planCultivatedResources.defaultConfig)
    );

    const result = resources.demand.ops.planCultivatedResources.run(
      {
        width,
        height,
