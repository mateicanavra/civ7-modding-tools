...
describe("geological resource operation contract", () => {
  it("plans every geological resource row while preserving blocked product exceptions", () => {
    ...
    const selection = normalizeOperationSelectionForTest(
      resources.demand.ops.planGeologicalResources,
      structuredClone(resources.demand.ops.planGeologicalResources.defaultConfig)
    );

    const result = resources.demand.ops.planGeologicalResources.run(
      {
        width,
        height,
