...
describe("placement adjusted resource-plan artifact", () => {
  it("reconciles coherent resource moves, additions, provenance, and support evidence", () => {
    expect(
      resourceSupportArtifacts.resourcePlanAdjusted.validate(coherentAdjustedResourcePlan(), {
  ...
  it("requires exact terminal floor-shortfall evidence", () => {
    expect(
      resourceSupportArtifacts.resourcePlanAdjusted.validate(inactiveResourcePlan(), {
        dimensions: SYNTHETIC_DIMENSIONS,
      })
    ).toEqual([]);
    ...
    for (const mutate of [
      ...
      mutate(value);
      expect(
        resourceSupportArtifacts.resourcePlanAdjusted.validate(value, {
          dimensions: SYNTHETIC_DIMENSIONS,
        }).length
      ).toBeGreaterThan(0);
    }
  ...
  it("uses producer radius membership at the wrapped even-width seam", () => {
    ...
    expect(
      resourceSupportArtifacts.resourcePlanAdjusted.validate(seam, {
