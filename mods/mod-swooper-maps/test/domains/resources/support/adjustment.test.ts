...
type AdjustResult = ReturnType<typeof resources.support.ops.adjustResourceSupport.run>;
...
function run(
  ...
  const selection = structuredClone(resources.support.ops.adjustResourceSupport.defaultConfig);
  configure?.(selection.config);
  const result = runAdmittedOperationForTest(
    resources.support.ops.adjustResourceSupport,
    input,
    selection
  );
  expect(
