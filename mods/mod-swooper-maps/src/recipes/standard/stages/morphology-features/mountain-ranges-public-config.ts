...
export const MountainRangesPublicSchema = Type.Object(
  {
...
export type MountainRangesPublicConfig = Static<typeof MountainRangesPublicSchema>;

export function resolveMountainRangesPublicConfig(value: unknown): MountainsConfig {
  ...
  const foothillMaxFraction = clamp(
    0.035 + foothillExtentTiles * 0.011 + tectonicActivity * 0.006,
    0,
    hillMaxFraction
  );
  const roughLandMaxFraction = clamp(
    0.045 + interiorHighlandExpression * 0.12 + terrainTextureFractalMix * 0.055,
    0,
