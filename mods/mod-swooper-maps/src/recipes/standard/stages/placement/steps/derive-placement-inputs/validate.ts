...
/**
 * Validate hook for the discovery plan: count coherence and in-bounds unique
 * placements.
 */
export function validateDiscoveryPlanArtifact(value: unknown): ValidationIssue[] {
  if (!isRecord(value)) return [issue("discoveryPlan artifact must be an object.")];
  const issues: ValidationIssue[] = [];
  const width = Number(value.width);
  const height = Number(value.height);
  const size = width * height;
  ...
  for (const placement of placements) {
    ...
    const plotIndex = Number(placement.plotIndex);
    if (!Number.isInteger(plotIndex) || plotIndex < 0 || plotIndex >= size) {
      issues.push(issue(`discoveryPlan placement ${String(placement.plotIndex)} out of bounds.`));
      continue;
    }
    if (seenPlots.has(plotIndex)) {
      issues.push(issue(`discoveryPlan plans two discoveries on plot ${plotIndex}.`));
    }
    seenPlots.add(plotIndex);
  }
  return issues;
