...
export const mountainProximityStrategy = createStrategy(
  PlanFoothillsContract,
  "mountain-proximity",
  {
    run: (input, config) => {
      const {
      ...
      for (let i = 0; i < size; i++) {
        ...
        const strongConvergence =
          collisionByte >= config.driverSignalByteMin ||
          subductionByte >= config.driverSignalByteMin;
