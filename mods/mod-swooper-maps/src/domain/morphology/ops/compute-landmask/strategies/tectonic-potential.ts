...
export const tectonicPotentialStrategy = createStrategy(
  ComputeLandmaskContract,
  "tectonic-potential",
  {
    run: (input, config) => {
      ...
      for (let i = 0; i < size; i++) {
        ...
        else if (bt === BOUNDARY_TYPE.divergent)
          boundary -= b * (BOUNDARY_DIVERGENT_RIFT_WEIGHT * r);
        ...
        for (let era = 0; era < eraCount; era++) {
          ...
          for (let step = 0; step < stepsPerEra; step++) {
            ...
            for (let i = 0; i < size; i++) {
              ...
              const baseElevationUnit = clamp01(crustBaseElevation[i] ?? 0);
              const emergenceBias =
                CRATON_EMERGENCE_BASE + CRATON_EMERGENCE_GAIN * baseElevationUnit;
              ...
              forEachHexNeighborOddQ(x, y, width, height, (nx, ny) => {
                const ni = ny * width + nx;
                cratonMass[ni] =
                  (cratonMass[ni] ?? 0) + CRATON_SHOULDER_DEPOSIT_FRACTION * nucleate;
              });
            }
      ...
      const blurredBoundary = blurHex(
        width,
        height,
        potentialBoundaryRaw,
        BLUR_MID_FREQUENCY_STEPS
      );
