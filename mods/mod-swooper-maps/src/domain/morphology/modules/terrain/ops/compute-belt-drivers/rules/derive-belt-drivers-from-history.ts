...
/**
 * Condenses causal tectonic history into the localized driver fields consumed by terrain and
 * landform generation.
 *
...
export function deriveBeltDriversFromHistory(input: {
  ...
  for (let i = 0; i < size; i++) {
    ...
    dominantEra[i] = quantizeU8(bestEra);
    ...
    let resolvedBoundary = quantizeU8(boundary);
    if (resolvedBoundary === BOUNDARY_TYPE.none) {
      const provenanceBoundary = lastBoundaryType[i] ?? 255;
      if (provenanceBoundary !== 255) resolvedBoundary = quantizeU8(provenanceBoundary);
    }
    ...
    const ageEras = quantizeU8(age);
    beltAgeEras[i] = ageEras;
    beltAge[i] = maxAge > 0 ? quantizeU8((ageEras / maxAge) * 255) : 0;
  }
  ...
  for (let i = 0; i < size; i++) {
    ...
    boundaryCloseness[i] = quantizeU8(255 * normalized);
    upliftPotential[i] = quantizeU8(seedUplift * normalized);
    collisionPotential[i] = quantizeU8(seedCollision * normalized);
    subductionPotential[i] = quantizeU8(seedSubduction * normalized);
    riftPotential[i] = quantizeU8(seedRift * normalized);
    const shear = quantizeU8(seedShear * normalized);
    tectonicStress[i] = quantizeU8(Math.max(upliftPotential[i]!, riftPotential[i]!, shear));
