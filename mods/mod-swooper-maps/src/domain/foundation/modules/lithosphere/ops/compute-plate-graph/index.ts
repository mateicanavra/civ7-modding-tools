...
/**
 * Builds a resistance-weighted Voronoi plate graph with explicit polar-cap policy.
 * The result is the immutable plate identity consumed by motion and tectonic reconstruction.
 */
const computePlateGraph = createOp(ComputePlateGraphContract, {
  strategies: {
    "resistance-weighted-voronoi": createStrategy(
      ComputePlateGraphContract,
      "resistance-weighted-voronoi",
      {
        normalize: (config) => {
        ...
        run: (input, config) => {
          ...
          const roleById: Plate["role"][] = Array.from({ length: platesCount }, () => "tectonic");
          roleById[0] = "polarCap";
          roleById[1] = "polarCap";
          ...
          const kindById: Plate["kind"][] = Array.from({ length: platesCount }, (_, id) =>
          ...
          const characteristic = Math.max(
            GEOMETRY_SCALE_CLAMP_MIN,
            Math.min(mesh.wrapWidth, spanY)
          );
          const minSep =
          ...
          const plates: Plate[] = Array.from({ length: platesCount }, (_, id) => {
            const seedCell = seedCells[id] ?? 0;
