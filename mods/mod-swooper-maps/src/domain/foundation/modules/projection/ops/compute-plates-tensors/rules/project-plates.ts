...
type ProjectionResult = Readonly<{
  ...
  tectonicProvenanceTiles: Readonly<{
    version: number;
    originEra: Uint8Array;
    originPlateId: Int16Array;
...
/**
 * Samples the Foundation model onto map tiles and derives boundary-distance and motion tensors.
 * All projected fields share one nearest-cell mapping so downstream morphology sees coherent evidence.
 */
export function projectPlatesFromModel(input: {
  ...
  tectonicHistory: ProjectionHistory;
  tectonicProvenance?: ProjectionProvenance | null;
