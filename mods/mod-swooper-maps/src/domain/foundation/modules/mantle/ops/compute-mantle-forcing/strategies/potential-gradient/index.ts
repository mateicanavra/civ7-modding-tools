...
import PotentialGradientContract from "./contract.js";
...
/**
 * Converts mantle-potential gradients into the forcing fields consumed by downstream Foundation ops.
 * The potential-gradient strategy keeps that physical interpretation explicit and replaceable.
 */
const potentialGradient = createStrategy(ComputeMantleForcingContract, PotentialGradientContract, {
  run: (input, config) => {
    const mesh = input.mesh;
    const mantlePotential = input.mantlePotential;
    const cellCount = mesh.cellCount | 0;
    if ((mantlePotential.cellCount | 0) !== cellCount) {
      throw new Error("[Foundation] Invalid mantlePotential.cellCount for compute-mantle-forcing.");
    }
...
export default potentialGradient;
