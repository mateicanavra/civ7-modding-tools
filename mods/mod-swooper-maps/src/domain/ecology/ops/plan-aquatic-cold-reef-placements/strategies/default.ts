...
type Config = Static<(typeof PlanAquaticColdReefPlacementsContract)["strategies"]["default"]>;
type Placement = Static<(typeof PlanAquaticColdReefPlacementsContract)["output"]>["placements"][number];
...
export const defaultStrategy = createStrategy(PlanAquaticColdReefPlacementsContract, "default", {
  ...
  run: (input, config) => {
    ...
    for (let y = 0; y < height; y++) {
      ...
      for (let x = 0; x < width; x++) {
        ...
        if (!rollPercent(rng, "features:plan:reef:FEATURE_COLD_REEF", chance)) continue;
        featureField[idx] = FEATURE_KEY_INDEX.FEATURE_COLD_REEF;
        placements.push({ x, y, feature: "FEATURE_COLD_REEF" });
      }
    }
