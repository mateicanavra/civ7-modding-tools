...
type Config = Static<(typeof PlanWetPlacementWateringHoleContract)["strategies"]["default"]>;
type Input = Static<(typeof PlanWetPlacementWateringHoleContract)["input"]>;
type Placement =
  Static<(typeof PlanWetPlacementWateringHoleContract)["output"]>["placements"][number];

const NO_FEATURE = -1;

export function planWetPlacementWateringHole(
  input: Input,
  config: Config
  ...
  const chance = clampChance(config.chances.FEATURE_WATERING_HOLE * multiplier);
  ...
  const featureIdx = FEATURE_KEY_INDEX.FEATURE_WATERING_HOLE;
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      ...
      if (!rollPercent(rng, "features:plan:wet:FEATURE_WATERING_HOLE", chance)) continue;

      featureField[idx] = featureIdx;
      placements.push({ x, y, feature: "FEATURE_WATERING_HOLE" });
    }
  }
