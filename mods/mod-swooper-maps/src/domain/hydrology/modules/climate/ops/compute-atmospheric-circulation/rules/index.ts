...
/**
 * Synthesizes the default tile-varying circulation proxy from planetary bands and pressure flow.
 *
 * The field combines three-cell circulation, deterministic waves/noise, optional seasonal land
 * heating and orographic pressure, then smooths in hex space. `maxSpeed` is the physical-to-i8
 * quantization scale; values beyond it saturate rather than changing the output representation.
 *
 * @param width - Number of tile columns to populate.
 * @param height - Number of tile rows to populate.
 * @param latitudeByRow - Signed latitude in degrees for each row.
 * @param options - Deterministic pressure, circulation, smoothing, and quantization controls.
 * @returns Quantized U/V wind components consumed by moisture and ocean-current operations.
 */
