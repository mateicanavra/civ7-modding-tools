...
const HYDROLOGY_TERMINAL_NONE = 0;
const HYDROLOGY_TERMINAL_OCEAN = 1;
const HYDROLOGY_TERMINAL_CLOSED_BASIN = 2;
...
/**
 * Conditions terrain with a deterministic priority flood and derives one acyclic drainage graph.
 *
 * Water and admitted external edges seed the flood. Every visited land tile receives a downstream
 * tile, basin identity, conditioned elevation, depression depth, and terminal evidence; the same
 * receiver graph is then accumulated into upstream-area counts for downstream river operations.
 *
 * @param params - Grid geometry, elevation/land evidence, and external-edge outlet policy.
 * @returns Aligned routing, basin, accumulation, sink, outlet, and terminal fields.
