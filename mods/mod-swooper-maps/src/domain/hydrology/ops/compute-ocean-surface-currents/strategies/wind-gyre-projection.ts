...
export const defaultStrategy = createStrategy(ComputeOceanSurfaceCurrentsContract, "default", {
  run: (input, config) => {
    ...
    if (
      input.basinId !== undefined &&
      (!(input.basinId instanceof Int32Array) || input.basinId.length !== size)
    ) {
    ...
    if (
      ...
      throw new Error(
        "[Hydrology] Invalid coastDistance for hydrology/compute-ocean-surface-currents."
      );
    }
    if (
      ...
      throw new Error(
        "[Hydrology] Invalid coastTangentU for hydrology/compute-ocean-surface-currents."
      );
    }
    if (
      ...
      throw new Error(
        "[Hydrology] Invalid coastTangentV for hydrology/compute-ocean-surface-currents."
      );
    }
