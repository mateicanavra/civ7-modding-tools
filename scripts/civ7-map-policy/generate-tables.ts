/**
 * Civ7 map-policy table generator (S2, placement-realignment).
 *
 * Regenerates `packages/civ7-map-policy/src/civ7-tables.gen.ts` from the
 * official Civ7 resources submodule at `.civ7/outputs/resources` (READ-ONLY).
 *
 ...
 *
 * Usage:
 *   bun scripts/civ7-map-policy/generate-tables.ts          # regenerate
 *   bun scripts/civ7-map-policy/generate-tables.ts --check  # verify committed output is current
 */

import { execSync } from "node:child_process";
import { readdirSync, readFileSync, statSync, writeFileSync } from "node:fs";
import { join, relative, resolve } from "node:path";
import { CIV7_RIVER_TYPE_METADATA_SOURCE } from "../../packages/civ7-map-policy/src/river-type-metadata.source.js";
...
function fatal(message: string): never {
  console.error(`[civ7-map-policy:gen-tables] ${message}`);
  process.exit(1);
}
  ...
  if (existing !== file) {
    fatal(
      `committed ${relative(REPO_ROOT, OUT_PATH)} is stale — rerun ` +
        "`bun run civ7-map-policy:gen-tables` against the current submodule"
    );
  }
  console.log("[civ7-map-policy:gen-tables] committed tables are current");
} else {
  writeFileSync(OUT_PATH, file);
  console.log(`[civ7-map-policy:gen-tables] wrote ${relative(REPO_ROOT, OUT_PATH)}`);
}
