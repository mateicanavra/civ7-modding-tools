...
export function localFeedbackCheckProjection(report: CheckReport): LocalFeedbackCheckProjection {
  ...
  return Value.Parse(LocalFeedbackCheckProjectionSchema, {
    kind: localFeedbackKind(report, outcome),
...
export function verifyCheckSummaryProjection(
  report: CheckReport,
  ...
  return Value.Parse(VerifyCheckSummaryProjectionSchema, {
...
export function isDiagnosticUnavailableProjection(
  projection: LocalFeedbackCheckProjection
): boolean {
  return projection.kind === "diagnostic-unavailable";
}

function localFeedbackKind(
  report: CheckReport,
  outcome: CheckOutcome
): LocalFeedbackCheckProjection["kind"] {
...
function builtInRules(report: CheckReport): RuleReport[] {
  return report.rules.filter(
    (rule) => rule.ownerTool === "habitat-native" && rule.detect.includes("(built-in)")
  );
}
