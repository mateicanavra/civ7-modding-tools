...
export async function readWorkspaceManifestProjects(
  ...
  for (const manifestRoot of [...manifestRoots].sort()) {
    ...
    projects.push({
      ...
      tags: kindTags([...(manifest.nx?.tags ?? []), ...projectJsonTags]),
    });
  }
  ...
  for (const edge of input.graphEdges) {
    ...
    if (failedConstraint) {
      issues.push({
        ...
        message: `${edge.source} -> ${edge.target} violates ${failedConstraint.sourceTag}.`,
        source: edge.source,
        target: edge.target,
      });
    }
...
function kindTags(values: readonly string[]): string[] {
  return sortedUnique(values.filter((tag) => tag.startsWith("kind:")));
}
