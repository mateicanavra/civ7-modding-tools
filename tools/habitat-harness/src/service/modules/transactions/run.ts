 ...
 * execution is delegated to GritProvider so transaction policy stays separate
 * from Grit command construction.
 */
export function runTransactionApplyService(
  input: unknown,
  options: TransactionsServiceOptions = {}
) {
  return runTransactionApply(input, options);
}

function runTransactionApply(input: unknown, options: TransactionsServiceOptions) {
