...
Platform-filtered actions are shown only where the local Civ7 workflow requires
macOS. Studio, Docs, Playground, resource inspection, builds, and headless
MapGen reports remain unfiltered.
...
Nx owns Studio development as one graph. The Vite target depends on the
continuous daemon target, while the daemon target owns every one-shot build and
recipe prerequisite.
...
The command remains attached to its terminal. Stop that terminal or send
`SIGINT`; do not recreate the graph with separate daemon and Vite processes.
