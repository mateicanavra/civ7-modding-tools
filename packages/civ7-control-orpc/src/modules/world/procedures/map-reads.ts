...
export const worldPlotReadProcedure = civ7ControlOrpcImplementer.world.plot.effect(function* ({
  context,
  errors,
  input,
}) {
...
export const worldGridReadProcedure = civ7ControlOrpcImplementer.world.grid.effect(function* ({
  context,
  errors,
  input,
}) {
...
function worldPlotReadResult(result: Civ7ControlOrpcPlotSnapshotResult): Civ7WorldPlotReadResult {
...
function worldGridReadResult(result: Civ7ControlOrpcMapGridResult): Civ7WorldGridReadResult {
  const plots = result.plots.map(worldPlotSnapshot);
  const probeErrorCount =
    plots.reduce((total, plot) => total + plot.summary.probeErrorCount, 0) +
      ...
      grid:
        result.omitted > 0
          ...
          : probeErrorCount > 0
            ...
            : "read",
      map:
        probeValue(result.map?.width) != null || probeValue(result.map?.height) != null
          ? "read"
          : "skipped-unavailable",
    },
...
function worldPlotSnapshot(
  ...
  return {
    ...
    summary: {
      ...
      probeErrorCount:
        probeErrorCountForRecord(facts) +
...
function probeRecord(
  ...
  return Object.fromEntries(Object.entries(facts).map(([key, value]) => [key, publicProbe(value)]));
}
