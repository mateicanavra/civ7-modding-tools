import * as fs from "node:fs";
import {
  configureRemoteAndFetch,
  getLocalConfig,
  getRemotePushConfig,
  getRemoteUrl,
  listSubtreeConfigs,
  type RemotePushConfig,
  removeSubtreeConfig,
  setLocalConfig,
  subtreeAddFromRemote,
  subtreePullWithFetch,
  subtreePushWithFetch,
} from "./git.js";

/**
 * Returns true if the directory exists and contains entries.
 * Non-directory paths are treated as non-empty.
 */
export function isNonEmptyDir(dir: string): boolean {
  try {
    const s = fs.statSync(dir);
    if (!s.isDirectory()) return true;
    const entries = fs.readdirSync(dir);
    return entries.length > 0;
  } catch {
    return false;
  }
}

// Generic git subtree helpers -----------------------------------------------

interface Logger {
  log: (msg: string) => void;
}

function getLogger(logger?: Logger): Logger {
  const base = logger ?? console;
  return { log: base.log.bind(base) };
}

/** Resolves the persisted remote identity for one domain-scoped subtree slug. */
export async function findRemoteNameForSlug(
  domain: string,
  slug: string
): Promise<string | undefined> {
  try {
    return (await getLocalConfig(`civ7.${domain}.${slug}.remoteName`)) ?? undefined;
  } catch {
    return undefined;
  }
}

/**
 * Resolves and verifies a subtree's persisted remote against the live Git
 * remote table so stale local configuration cannot direct a later operation.
 */
export async function requireRemoteNameForSlug(
  domain: string,
  slug: string,
  opts: { verbose?: boolean } = {}
): Promise<string> {
  const { verbose = false } = opts;
  const name = await findRemoteNameForSlug(domain, slug);
  if (!name) {
    throw new Error(`No remote configured for ${domain} "${slug}". Run setup first.`);
  }
  const repoUrl = await getRepoUrlForSlug(domain, slug);
  const url = await getRemoteUrl(name, { verbose });
  if (!url) {
    throw new Error(`No git remote named ${name} found for ${domain} "${slug}". Run setup again.`);
  }
  if (repoUrl && url !== repoUrl) {
    throw new Error(
      `Stored repoUrl ${repoUrl} does not match remote "${name}" URL ${url}. Run setup again.`
    );
  }
  return name;
}

async function getRepoUrlForSlug(domain: string, slug: string): Promise<string | undefined> {
  try {
    return (await getLocalConfig(`civ7.${domain}.${slug}.repoUrl`)) ?? undefined;
  } catch {
    return undefined;
  }
}

/** Inputs for resolving a subtree branch from explicit or persisted configuration. */
export interface ResolveBranchOptions {
  domain: string;
  slug: string;
  branch?: string;
  verbose?: boolean;
  logger?: Logger;
}

/** Resolves an explicit branch or the branch persisted for the subtree slug. */
export async function resolveBranch(opts: ResolveBranchOptions): Promise<string | undefined> {
  const { domain, slug, branch, verbose = false, logger } = opts;
  if (branch) return branch;
  const log = getLogger(logger).log;
  try {
    const saved = (await getLocalConfig(`civ7.${domain}.${slug}.branch`, { verbose })) ?? undefined;
    if (saved) {
      log(`Using branch from config: ${saved}`);
      return saved;
    }
  } catch {}
  return undefined;
}

/** Resolves a subtree branch and refuses execution when no branch was authored. */
export async function requireBranch(opts: ResolveBranchOptions): Promise<string> {
  const branch = await resolveBranch(opts);
  if (!branch) {
    throw new Error(
      `No branch specified and none configured for slug "${opts.slug}". Re-run with --branch <name> or run setup first.`
    );
  }
  return branch;
}

/** Inputs for admitting and persisting one domain-scoped subtree remote. */
export interface ConfigureRemoteOptions {
  domain: string;
  slug: string;
  repoUrl: string;
  branch?: string;
  defaultBranch?: string;
  verbose?: boolean;
  logger?: Logger;
  /** Custom message when repoUrl cannot be resolved. */
  remoteRequiredMessage?: string;
}

/** Logging controls for projecting one remote's push policy. */
export interface LogRemotePushConfigOptions {
  verbose?: boolean;
  logger?: Logger;
}

/** Projects a remote's persisted push policy through a caller-provided logger. */
export async function logRemotePushConfig(
  remoteName: string,
  opts: LogRemotePushConfigOptions = {}
): Promise<void> {
  const { verbose = false, logger } = opts;
  const log = getLogger(logger).log;
  try {
    const cfg = await getRemotePushConfig(remoteName, { verbose });
    log("Push config:");
    log(`  trunk: ${cfg.trunk ?? "(auto)"}`);
    log(`  autoFastForwardTrunk: ${cfg.autoFastForwardTrunk ?? false}`);
    log(`  createPrOnFfBlock: ${cfg.createPrOnFfBlock ?? false}`);
    if (cfg.prTitle) log(`  prTitle: ${cfg.prTitle}`);
    if (cfg.prBody) log(`  prBody: ${cfg.prBody}`);
    log(`  prDraft: ${cfg.prDraft ?? false}`);
    log(`  prAutoMerge: ${cfg.prAutoMerge ?? true}`);
    log(`  prMergeStrategy: ${cfg.prMergeStrategy ?? "rebase"}`);
  } catch {}
}

/**
 * Creates or updates a subtree remote, fetches it, and persists the admitted
 * remote URL and branch under the caller's domain namespace.
 */
export async function configureRemote(
  opts: ConfigureRemoteOptions
): Promise<"added" | "updated" | "unchanged" | "skipped"> {
  const {
    domain,
    slug,
    repoUrl,
    branch,
    defaultBranch = "main",
    verbose = false,
    logger,
    remoteRequiredMessage,
  } = opts;
  if (!repoUrl) {
    throw new Error(remoteRequiredMessage ?? "repoUrl is required to configure a remote.");
  }
  const rName = slug;
  const res = await configureRemoteAndFetch(rName, repoUrl, { tags: true }, { verbose });
  await setLocalConfig(`civ7.${domain}.${slug}.remoteName`, rName, { verbose });
  await setLocalConfig(`civ7.${domain}.${slug}.repoUrl`, repoUrl, { verbose });
  const branchName = branch ?? defaultBranch;
  await setLocalConfig(`civ7.${domain}.${slug}.branch`, branchName, { verbose });
  const log = getLogger(logger).log;
  const badge = res === "added" ? "added" : res === "updated" ? "updated" : "unchanged";
  log(`Remote "${rName}" ${badge}: ${repoUrl}`);
  log(`Fetched tags from "${rName}". Tracking branch: ${branchName}`);
  await logRemotePushConfig(rName, { logger, verbose });
  return res;
}

/** Safety and history controls for importing one configured subtree. */
export interface ImportSubtreeOptions {
  domain: string;
  slug: string;
  prefix: string;
  branch?: string;
  squash?: boolean;
  allowDirty?: boolean;
  autoUnshallow?: boolean;
  verbose?: boolean;
  logger?: Logger;
  remoteRequiredMessage?: string;
  branchRequiredMessage?: string;
}

/**
 * Imports a configured remote into a subtree after proving the destination,
 * remote, and branch satisfy the requested safety policy.
 */
export async function importSubtree(opts: ImportSubtreeOptions): Promise<void> {
  const {
    domain,
    slug,
    prefix,
    branch,
    squash = false,
    allowDirty = false,
    autoUnshallow = false,
    verbose = false,
    logger,
    remoteRequiredMessage,
    branchRequiredMessage,
  } = opts;
  const log = getLogger(logger).log;

  if (fs.existsSync(prefix)) {
    const nonEmpty = isNonEmptyDir(prefix);
    if (nonEmpty && !allowDirty) {
      throw new Error(
        `Target directory "${prefix}" already exists and is not empty. ` +
          "If this was not previously imported using subtree, add may fail. Re-run with allowDirty=true to continue anyway."
      );
    }
  }

  const rName = await requireRemoteNameForSlug(domain, slug, { verbose }).catch((err) => {
    if (remoteRequiredMessage) throw new Error(remoteRequiredMessage);
    throw err;
  });
  const branchName = await requireBranch({
    domain,
    slug,
    branch,
    verbose,
    logger,
  }).catch((err) => {
    if (branchRequiredMessage) throw new Error(branchRequiredMessage);
    throw err;
  });

  log(
    `Importing subtree: prefix=${prefix} remote=${rName} branch=${branchName} squash=${
      squash ? "yes" : "no"
    } autoUnshallow=${autoUnshallow ? "yes" : "no"}`
  );
  await configureRemoteAndFetch(rName, undefined, { tags: true }, { verbose });
  await subtreeAddFromRemote(
    prefix,
    rName,
    branchName,
    { squash, autoUnshallow, allowDirty },
    { verbose }
  );
  log(`✅ Imported ${rName}/${branchName} into ${prefix}`);
}

/** Safety, branch, and trunk controls for pushing one configured subtree. */
export interface PushSubtreeOptions {
  domain: string;
  slug: string;
  prefix: string;
  branch?: string;
  allowDirty?: boolean;
  autoUnshallow?: boolean;
  autoFastForwardTrunk?: boolean;
  trunk?: string;
  verbose?: boolean;
  logger?: Logger;
  pushConfig?: RemotePushConfig;
  remoteRequiredMessage?: string;
  branchRequiredMessage?: string;
}

/** Pushes a configured subtree while preserving the caller's dirty-tree and trunk policy. */
export async function pushSubtree(opts: PushSubtreeOptions): Promise<void> {
  const {
    domain,
    slug,
    prefix,
    branch,
    allowDirty = false,
    autoUnshallow = false,
    autoFastForwardTrunk = false,
    trunk,
    verbose = false,
    logger,
    pushConfig,
    remoteRequiredMessage,
    branchRequiredMessage,
  } = opts;
  const log = getLogger(logger).log;

  const rName = await requireRemoteNameForSlug(domain, slug, { verbose }).catch((err) => {
    if (remoteRequiredMessage) throw new Error(remoteRequiredMessage);
    throw err;
  });
  const branchName = await requireBranch({
    domain,
    slug,
    branch,
    verbose,
    logger,
  }).catch((err) => {
    if (branchRequiredMessage) throw new Error(branchRequiredMessage);
    throw err;
  });

  log(
    `Pushing subtree: prefix=${prefix} → ${rName}/${branchName} autoUnshallow=${
      autoUnshallow ? "yes" : "no"
    } autoFFTrunk=${autoFastForwardTrunk ? "yes" : "no"}`
  );
  await subtreePushWithFetch(
    prefix,
    rName,
    branchName,
    { allowDirty, autoUnshallow, autoFastForwardTrunk, trunkOverride: trunk, ...pushConfig },
    { verbose }
  );
  log(`✅ Pushed "${prefix}" to ${rName}/${branchName}`);
}

/** Safety and history controls for pulling one configured subtree. */
export interface PullSubtreeOptions {
  domain: string;
  slug: string;
  prefix: string;
  branch?: string;
  squash?: boolean;
  allowDirty?: boolean;
  autoUnshallow?: boolean;
  verbose?: boolean;
  logger?: Logger;
  remoteRequiredMessage?: string;
  branchRequiredMessage?: string;
}

/** Pulls a configured remote branch into its subtree under explicit history safety options. */
export async function pullSubtree(opts: PullSubtreeOptions): Promise<void> {
  const {
    domain,
    slug,
    prefix,
    branch,
    squash = false,
    allowDirty = false,
    autoUnshallow = false,
    verbose = false,
    logger,
    remoteRequiredMessage,
    branchRequiredMessage,
  } = opts;
  const log = getLogger(logger).log;

  const rName = await requireRemoteNameForSlug(domain, slug, { verbose }).catch((err) => {
    if (remoteRequiredMessage) throw new Error(remoteRequiredMessage);
    throw err;
  });
  const branchName = await requireBranch({
    domain,
    slug,
    branch,
    verbose,
    logger,
  }).catch((err) => {
    if (branchRequiredMessage) throw new Error(branchRequiredMessage);
    throw err;
  });

  log(
    `Pulling into subtree: prefix=${prefix} from ${rName}/${branchName} squash=${
      squash ? "yes" : "no"
    } autoUnshallow=${autoUnshallow ? "yes" : "no"}`
  );
  await subtreePullWithFetch(
    prefix,
    rName,
    branchName,
    { squash, allowDirty, autoUnshallow },
    { verbose }
  );
  log(`✅ Pulled updates into "${prefix}" from ${rName}/${branchName}`);
}
