...
describe("git utilities", () => {
  ...
  describe("configuration helpers", () => {
    ...
    it("configureRemote logs push config", async () => {
      ...
      expect(setLocalConfig).toHaveBeenCalledWith("civ7.mod.foo.remoteName", "foo", {
        verbose: false,
      });
      expect(setLocalConfig).toHaveBeenCalledWith(
        "civ7.mod.foo.repoUrl",
        "git@github.com:me/repo.git",
        { verbose: false }
      );
      expect(setLocalConfig).toHaveBeenCalledWith("civ7.mod.foo.branch", "main", {
        verbose: false,
      });
      expect(configureRemoteAndFetch).toHaveBeenCalledWith(
        "foo",
