...
export function graphToJson(g: Graph) {
  return {
    nodes: Array.from(g.nodes.values()).map((n: GraphNode) => ({
      table: n.key.table,
      id: n.key.id,
      file: n.file,
      row: n.row,
    })),
    edges: g.edges.map((e: GraphEdge) => ({
      from: `${e.from.table}:${e.from.id}`,
      to: `${e.to.table}:${e.to.id}`,
      label: e.label,
    })),
  };
}
...
export function graphToDot(g: Graph): string {
  ...
  lines.push(
    "  graph [rankdir=LR, compound=true, overlap=false, splines=true, concentrate=true, nodesep=0.5, ranksep=0.6];"
  );
  lines.push(
    '  node  [fontname="Helvetica", fontsize=10, shape=box, style=filled, fillcolor="#ffffff", color="#555555", penwidth=1.2];'
  );
  ...
  lines.push(
    `    "Legend_ReqSet_A" -> "Legend_ReqSet_B" [${edgeStyleAttrs("SubjectRequirementSetId")}];`
  );
  lines.push("  }");
  ...
  for (const n of g.nodes.values()) {
    ...
    lines.push(
      `  "${id}" [label="${label}"${styleAttrs ? ", " + styleAttrs : ""}, tooltip="${nodeTooltip}"${urlAttr}${targetAttr}];`
    );
  ...
  for (const n of g.nodes.values()) {
    ...
    listA.push(anchorId);
    membersByLayer.set(anchorLayer, listA);
  ...
  for (const c of clusters) {
    ...
    const rsType = (rsNode as any)?.row?.RequirementSetType
      ? `ReqSet: ${String((rsNode as any).row.RequirementSetType)}`
      : "ReqSet";
    const isAbs =
      rsNode && typeof (rsNode as any).file === "string" && path.isAbsolute((rsNode as any).file);
    const relOrAbs =
      rsNode && isAbs
        ? path.relative(process.cwd(), (rsNode as any).file)
        : ((rsNode as any)?.file ?? "");
    ...
    lines.push(
      `    label=\"${escapeDotString(rsType)}\"; labelloc=\"t\"; margin=\"10\"; fontname=\"Helvetica\"; fontsize=10;`
    );
    lines.push(`    tooltip=\"${tooltip}\";`);
    if (rsNode && isAbs) {
      lines.push(
        `    URL=\"${escapeDotString(buildFileUrl((rsNode as any).file))}\"; target=\"_blank\";`
      );
    }
    ...
    for (const layer of innerLayers) {
      ...
      for (const nid of ids) {
        if (nid.startsWith("RSANCHOR_")) {
          lines.push(
            `      "${nid}" [label="", shape=point, width=0.01, height=0.01, color=\"#1565c0\"];`
          );
        } else {
          lines.push(`      "${nid}";`);
        }
  ...
  const reqsetEdgeLabels = new Set([
  ...
  for (const e of g.edges) {
    ...
    const edgeTooltip = e.label
      ? `${e.label} from ${e.from.table}:${e.from.id} → ${e.to.table}:${e.to.id}`
      : "";
...
function buildSemanticTooltip(n: GraphNode, g: Graph): string | undefined {
  try {
    ...
    if (t === "Modifiers") {
      ...
      const perm =
        (row as any).Permanent !== undefined ? `Permanent=${(row as any).Permanent}` : undefined;
    ...
    if (t === "RequirementSets") {
      const type = (row as any).RequirementSetType
        ? `Type=${(row as any).RequirementSetType}`
        : undefined;
    ...
    if (t === "Requirements") {
      const rtype = (row as any).RequirementType
        ? `Type=${(row as any).RequirementType}`
        : undefined;
...
function summarizeArgsFor(
  argTable: "ModifierArguments" | "RequirementArguments",
  foreignKey: string,
  fkValue: string,
  g: Graph,
  maxItems: number
...
function nodeStyleAttrs(table: string): string {
  const styles: Record<
    string,
    { shape: string; fill: string; border: string; style?: string; pen?: number }
  > = {
    Leaders: {
      shape: "box",
      fill: "#eef7ff",
