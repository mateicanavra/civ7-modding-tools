...
import { requestCiv7CultureChoiceCloseout } from "../src/index";
...
describe("culture choice closeout requests", () => {
  test("requests App UI culture chooser closeout with semantic notification and operation sends", async () => {
    const server = await startCultureCloseoutTunerServer();
    try {
...
async function startCultureCloseoutTunerServer(): Promise<FakeTunerServer> {
...
function executeAppUiCommand(
  ...
  const context = createContext({
    ...
    Players: {
      get: (playerId: number) => ({
        Culture: {
          getActiveTree: () => (playerId === 0 ? 7 : null),
          getTargetNode: () => (playerId === 0 ? 23 : null),
          getAllAvailableNodeTypes: () => [-1404789184, 1515870810],
        },
      }),
    },
    Game: {
