...
export type Civ7NotificationDismissalPostconditionClassification = Static<
  typeof Civ7NotificationDismissalPostconditionClassificationSchema
>;
...
export function classifyNotificationDismissalPostcondition(
  ...
  if (
    probeValue(before.engineQueueContains) === true &&
    probeValue(after.engineQueueContains) === false
  ) {
    return "engine-queue-cleared";
  }
  if (
    probeValue(before.notificationTrainContains) === true &&
    probeValue(after.notificationTrainContains) === false
  ) {
    return "notification-train-cleared";
  }
  if (
    probeValue(before.isEngineQueueFront) === true &&
    probeValue(after.isEngineQueueFront) === false
  ) {
    return "engine-front-moved";
  }
  if (
    probeValue(before.isNotificationTrainFront) === true &&
    probeValue(after.isNotificationTrainFront) === false
  ) {
