...
export type Civ7ProcedureCoreCallDiagnostics = Static<
  typeof Civ7ProcedureCoreCallDiagnosticsSchema
>;
...
export type Civ7ProcedureCoreDescriptorErrorReason = Static<
  typeof Civ7ProcedureCoreErrorReasonSchema
>;

export function isCiv7ProcedureCoreDescriptor(
  value: unknown
): value is Civ7ProcedureCoreDescriptor {
...
export function summarizeCiv7ProcedureCoreError(
  ...
  if (err.code !== "procedure-descriptor-invalid" && err.code !== "procedure-call-failed")
    return null;
...
function validateContextOwnership(descriptor: Civ7ProcedureCoreDescriptor): void {
  ...
  const contextOwnedInputFields = [
    ...
    context.has("state-selection")
      ? ["session", "state", "stateName", "stateSelection", "tunerState"]
      : [],
  ].flat();
  const invalid = descriptor.inputFields.filter((field) => {
    const normalized = normalizeFieldKey(field);
    return (
      !hasRawTunnelKey(field) &&
...
function procedureCallDiagnostics(
  ...
  return {
    ...
    telemetryCorrelation:
      descriptor.correlation.telemetry === "attached-when-procedure-telemetry-enabled",
  };
}
...
function resolveProcedureCorrelationId(
  ...
  const createCorrelationId =
    ...
    ((procedureKey: string) =>
      createCiv7ControlRequestId(`civ7-procedure-${procedureKey.replace(/[^a-z0-9]+/g, "-")}`));
...
function procedurePayloadErrorDetails(error: unknown): Record<string, unknown> {
  ...
  return {
    ...
    message:
      typeof candidate.message === "string" ? candidate.message : "value failed schema validation",
  };
}
...
function hasRawTunnelKey(field: string): boolean {
  ...
  return (
    FORBIDDEN_RAW_TUNNEL_OWNER_PARTS.some((ownerPart) => normalizedPath.includes(ownerPart)) ||
