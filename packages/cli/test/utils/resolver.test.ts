...
describe("CLI Resolver Utilities", () => {
  ...
  beforeEach(() => {
    ...
    mockedFs.readFileSync.mockImplementation((p) => {
      if (String(p) === rootPackageJson)
        return JSON.stringify({ workspaces: ["apps/*", "packages/*", "mods/*"] });
  ...
  describe("Output Path Resolvers", () => {
    ...
    it("resolveZipPath: should use defaults when no config is provided", () => {
      ...
      expect(result).toBe(
        path.resolve(projectRoot, DEFAULT_OUTPUT_BASE_DIR, DEFAULT_ZIP_DIR, DEFAULT_ARCHIVE_NAME)
      );
    });
    ...
    it("resolveGraphOutDir: should use defaults when no config is provided", () => {
      ...
      expect(result).toBe(
        path.resolve(projectRoot, DEFAULT_OUTPUT_BASE_DIR, DEFAULT_GRAPH_DIR, "SEED")
      );
    });
  });
});
