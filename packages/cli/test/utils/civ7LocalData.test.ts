...
describe('inspectCiv7LocalData', () => {
  test('labels local disk surfaces as enrichment rather than live authority', () => {
    ...
    try {
      ...
      expect(inventory.databases.map((database) => database.relativePath)).toContain('Debug/gameplay-copy.sqlite');
      expect(inventory.saves.map((save) => save.relativePath)).toContain('Saves/SinglePlayer/AutoSave.Civ7Save');
...
function createSqliteOrPlaceholder(file: string): void {
  ...
  if (sqlite3) {
    spawnSync(sqlite3, [file, 'create table Things(id integer primary key);'], { encoding: 'utf8' });
    return;
  }
