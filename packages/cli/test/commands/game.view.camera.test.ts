...
describe("game view camera", () => {
  ...
  test("rejects a malformed --plot before any wire traffic", async () => {
    ...
    try {
      await expect(runCommand(server, ["--plot", "32;17", "--json"])).rejects.toThrow(
        "--plot must be x,y"
      );
...
async function runCommand(server: FakeTunerServer, args: string[]): Promise<{ writes: string[] }> {
  const writes: string[] = [];
  const log = vi.spyOn(GameViewCamera.prototype, "log").mockImplementation(function (
    this: unknown,
    message?: string
  ) {
