...
describe("game play end-turn command", () => {
  ...
  test("sends end-turn only with send enabled", async () => {
    ...
    const writes: string[] = [];
    const log = vi
      .spyOn(GamePlayEndTurn.prototype, "log")
...
async function startEndTurnTunerServer(
  options: { canEndTurnBefore?: boolean; playNotificationMode?: EndTurnNotificationMode } = {}
  ...
  return startFakeTunerServer({
    handle({ message }) {
      if (message.includes("readPlayNotifications")) {
        return [
          JSON.stringify(
            endTurnNotificationView(options.playNotificationMode ?? "blocking-choice")
          ),
        ];
      }
      ...
      if (message.includes("hasSentTurnComplete")) {
        return [
          JSON.stringify(turnCompletionStatus(turnCompleteSent, options.canEndTurnBefore ?? true)),
        ];
      }
...
function endTurnNotificationView(mode: EndTurnNotificationMode) {
  ...
  if (mode === "stale-unit-command" || mode === "stale-unit-command-disabled") {
    ...
    const notification = {
      ...
      details: {
        ...
        classification: hasEnabledCloseout
  ...
  return notificationView({
    notification,
    decision,
    selectedCityId: { owner: 0, id: 131073, type: 1 },
