...
describe("game play diplomacy response commands", () => {
  test("wraps diplomacy response as RESPOND_DIPLOMATIC_ACTION", async () => {
    ...
    try {
      ...
      expect(server.received.some((message) => message.includes("RESPOND_DIPLOMATIC_ACTION"))).toBe(
        true
      );
  ...
  test("validates diplomacy responses as a dry-run player operation", async () => {
    ...
    try {
      ...
      expect(
        server.received.some((message) => message.includes("sendDiplomacyResponseCloseout"))
      ).toBe(false);
    } finally {
      await server.close();
    }
  ...
  test("sends diplomacy responses through the App UI closeout path", async () => {
    ...
    try {
      ...
      expect(server.received.some((message) => message.includes("GameContext.localPlayerID"))).toBe(
        true
      );
      expect(
...
async function startDiplomacyResponseTunerServer(
...
function operationValidation(message: string) {
  return {
    ...
    target: { playerId: 0 },
    args:
      message.includes('"ID":8') || message.includes("actionId:8")
        ? { ID: 8, Type: 926305338 }
...
function diplomacyResponseCloseout() {
  ...
  return {
    ...
    notes: [
      "This follows the official response-panel path more closely than a raw player-operation send.",
    ],
  };
}
...
function playNotificationView(mode: "stale-diplomacy", diplomacyCloseoutObserved = false) {
  const diplomacyDecision = {
    ...
    notes: [
      "Visible response panel option; send mode must verify the notification no longer blocks turn completion.",
    ],
  };
  const notificationId = { owner: 0, id: 19, type: 20 };
  const notification = {
