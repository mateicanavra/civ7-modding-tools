...
describe('game map noun topic', () => {
  test('topic index keeps the original flag-multiplexed id and behavior', async () => {
    ...
    try {
      const indexWrites = await runCommand(GameMap, server, ['--plot', '3,4', '--player-id', '0', '--json']);
      const subWrites = await runCommand(GameMapPlot, server, ['3,4', '--player-id', '0', '--json']);
  ...
  test('game map grid reads a bounded grid with focused args', async () => {
    const server = await startWorldTunerServer();
    try {
      const writes = await runCommand(GameMapGrid, server, ['0,0,2,1', '--fields', 'terrain', '--max-plots', '1', '--json']);
      expect(server.received.some((message) => message.includes('locationsFromBounds'))).toBe(true);
      const payload = JSON.parse(writes.join('')) as {
        ok: boolean;
        result: { bounds: { x: number; y: number; width: number; height: number }; plots: unknown[] };
      };
      expect(payload.ok).toBe(true);
...
async function runCommand(
  ...
  const writes: string[] = [];
  const log = vi.spyOn(commandClass.prototype, 'log').mockImplementation(function (this: unknown, message?: string) {
