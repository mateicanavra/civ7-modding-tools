...
describe('game play dismiss-notification-queue command', () => {
  test('bulk dismisses only eligible informational queue items with approval', async () => {
    ...
    try {
      ...
      expect(dryRun.server.received.some((message) => message.includes('readNotificationDismissal'))).toBe(false);
    } finally {
...
async function runDismissNotificationQueue(mode: DismissQueueMode, extraArgs: readonly string[] = []) {
...
async function startDismissNotificationQueueTunerServer(mode: DismissQueueMode): Promise<FakeTunerServer> {
