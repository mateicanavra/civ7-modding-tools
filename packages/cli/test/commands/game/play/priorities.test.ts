...
describe("game play priorities command", () => {
  test("reads play priorities without sending operations", async () => {
    const { payload, server } = await runPriorities("ready-unit", {
      compact: false,
      battlefield: true,
    });
    try {
  ...
  test("does not treat partial HUD probe failures as clean end-turn proof", async () => {
    ...
    try {
      const view = payload.view as {
        priorities: Array<{
          kind: string;
          nextAction?: CompactPriorityAction;
          evidenceLabels?: string[];
        }>;
      };
      const runtimeError = view.priorities.find((item) => item.kind === "runtime-state-error");
      expect(runtimeError?.nextAction).toMatchObject({
        kind: "observe",
        readOnly: true,
        sendsMutation: false,
      });
      expect(runtimeError?.evidenceLabels?.some((item) => item === "probe-error:blocker")).toBe(
        true
      );
  ...
  test("emits compact play priorities without raw evidence by request", async () => {
    ...
    try {
      ...
      expect(payload.omitted.some((item) => item.path === "priorities[].evidenceLabels")).toBe(
        true
      );
  ...
  test("keeps battlefield recommendations out of semantic blockers", async () => {
    ...
    try {
      expect(payload.semanticEnvelope.blockers.some((item) => item.kind === "ready-unit")).toBe(
        true
      );
      expect(
        payload.semanticEnvelope.blockers.some((item) => item.kind.startsWith("battlefield:"))
      ).toBe(false);
      expect(
        payload.semanticEnvelope.decisions.some((item) => item.kind.startsWith("battlefield:"))
      ).toBe(true);
      expect(server.received.some((message) => message.includes("readBattlefieldScan"))).toBe(true);
      expect(server.received.some((message) => message.includes("sendOperation("))).toBe(false);
    } finally {
  ...
  test("surfaces dismissal diagnostics for empty narrative choices in compact priorities", async () => {
    ...
    try {
      ...
      expect(top.nextAction?.parameters).toMatchObject({
        componentId: { owner: 0, id: 5, type: 20 },
      });
      expect(payload.nextAction).toEqual(top.nextAction);
      expect(server.received.some((message) => message.includes("sendOperation("))).toBe(false);
    } finally {
  ...
  test("surfaces informational dismissal action in compact priorities", async () => {
    ...
    try {
      ...
      expect(top.nextAction).toMatchObject({ kind: "inspect-notification", readOnly: true });
      expect(top.nextAction?.parameters).toMatchObject({
        componentId: { owner: 0, id: 89, type: 20 },
      });
      expect(payload.nextAction).toEqual(top.nextAction);
  ...
  test("routes unit-lost reports to reviewed dismissal in compact priorities", async () => {
    ...
    try {
      ...
      expect(top.nextAction).toMatchObject({ kind: "inspect-notification", readOnly: true });
      expect(top.nextAction?.parameters).toMatchObject({
        componentId: { owner: 0, id: 34, type: 20 },
      });
      expect(payload.nextAction).toEqual(top.nextAction);
  ...
  test("routes diplomatic action reports without response options to reviewed dismissal in compact priorities", async () => {
    ...
    try {
      ...
      expect(top.nextAction).toMatchObject({ kind: "inspect-notification", readOnly: true });
      expect(top.nextAction?.parameters).toMatchObject({
        componentId: { owner: 0, id: 118, type: 20 },
      });
      expect(payload.nextAction).toEqual(top.nextAction);
      expect(top.reason).toContain("live ComponentID");
...
async function runPriorities(
  ...
  const writes: string[] = [];
  const log = vi
    .spyOn(GamePlayPriorities.prototype, "log")
...
function priorityHudView(mode: PriorityHudMode) {
  ...
  if (mode === "clean-read")
    return basePriorityHudView({ firstReadyUnitId: null, nextDecision: null });
  if (mode === "ready-unit") {
    return basePriorityHudView({
      firstReadyUnitId: { owner: 0, id: 458752, type: 26 },
      nextDecision: null,
    });
  }
  if (
    mode === "stale-unit-command" ||
...
function decisionForMode(
  ...
  const choiceDetails = (kind: string, enabledOptions: unknown[] = [{}]) => ({
    kind,
    enabledOptions,
  });
  switch (mode) {
...
function commandUnitsDecision(
  mode: "stale-unit-command" | "stale-unit-command-disabled" | "stale-unit-command-pending"
) {
...
function readyCityView() {
  ...
  return {
    ...
    productionCandidates: {
      ok: true,
      value: [{ kind: "constructible", typeName: "BUILDING_WALLS" }],
    },
    townFocusOptions: { ok: true, value: [{ name: "LOC_PROJECT_FISHING_TOWN_NAME" }] },
    populationPlacement: {
      ok: true,
      value: { isReadyToPlacePopulation: { ok: true, value: true } },
    },
