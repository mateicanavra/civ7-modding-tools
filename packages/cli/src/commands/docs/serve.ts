...
export default class DocsServe extends Command {
  ...
  static flags = {
    siteDir: Flags.string({
      description: "Path to docs directory (Mintlify: apps/docs, Docsify: apps/docs/site)",
      required: false,
    }),
    engine: Flags.string({
      description: "Docs engine to serve",
      options: ["mint", "docsify"],
      default: "mint",
    }),
    port: Flags.integer({ description: "Port to serve on", default: 4000 }),
    host: Flags.string({
      description: "Host/interface to bind (docsify only)",
      default: "127.0.0.1",
    }),
    open: Flags.boolean({
    ...
    skipSync: Flags.boolean({
      description: "Skip syncing documentation resources before serving",
      default: false,
    }),
  } as const;
  ...
  public async run(): Promise<void> {
    ...
    const defaultDir =
      engine === "docsify" ? path.join("apps", "docs", "site") : path.join("apps", "docs");
    ...
    if (engine === "docsify") {
      const { proc, url } = await this.startDocsifyServer(
        projectRoot,
        siteDir,
        flags.port!,
        flags.host!,
    ...
    const { proc, url } = await this.startMintlifyServer(
      projectRoot,
      siteDir,
      flags.port!,
      flags.open!
  ...
  private async syncResourcesForEngine(
    engine: "mint" | "docsify",
    projectRoot: string,
    siteDir: string
  ): Promise<void> {
    const SOURCE_DIR = path.resolve(projectRoot, ".civ7/outputs/resources");
    const SOURCE_ZIP = path.resolve(
      projectRoot,
      ".civ7/outputs/archives/civ7-official-resources.zip"
    );
    const mintPublicDir = path.resolve(
      siteDir,
      engine === "docsify" ? path.join("..") : ".",
      "public",
      "civ7-official",
      "resources"
    );
    const mintZipDest = path.resolve(
      siteDir,
      engine === "docsify" ? path.join("..") : ".",
      "public",
      "civ7-official",
    ...
    const docsifyResourcesDir =
      engine === "docsify" ? path.resolve(siteDir, "civ7-official", "resources") : null;
    const docsifyZipDest =
      engine === "docsify"
        ? path.resolve(siteDir, "civ7-official", "civ7-official-resources.zip")
        : null;
    ...
    if (zipExists) {
      ...
      this.log(
        "  > Neither archive nor pre-extracted resources found. Run `bun run refresh:data` at repo root."
      );
    }
  ...
  private async startMintlifyServer(
    projectRoot: string,
    siteDir: string,
    port: number,
    open: boolean
  ): Promise<{ proc: any; url: string }> {
  ...
  private async startDocsifyServer(
    projectRoot: string,
    siteDir: string,
    port: number,
    host: string,
