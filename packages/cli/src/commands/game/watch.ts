...
export default class GameWatch extends Command {
  ...
  public async run(): Promise<void> {
    ...
    if (flags.json) {
      this.log(JSON.stringify({ ok: observations.every((observation) => observation.ok), observations }));
    }
  }
  ...
  private async readObservation(index: number, flags: WatchFlags): Promise<WatchObservation> {
    ...
    try {
      ...
      const readyUnit = flags['include-ready-unit'] && firstReadyUnitId
...
function staleGuard(durationMs: number, slowMs: number, staleMs: number): WatchObservation['staleGuard'] {
  if (durationMs >= staleMs) return 'stale-risk';
  if (durationMs >= slowMs) return 'slow-read';
  return 'fresh';
}

function summarizeNextDecision(decision: Record<string, unknown> | null): Record<string, unknown> | null {
...
function formatObservation(observation: WatchObservation): string {
  const decision = observation.nextDecision && typeof observation.nextDecision === 'object'
