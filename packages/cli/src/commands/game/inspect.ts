...
export default class GameInspect extends Command {
  ...
  public async run(): Promise<void> {
    ...
    if (flags['app-ui-snapshot']) {
      ...
      this.log(`Map: ${formatProbe(snapshot.snapshot.map.width)} x ${formatProbe(snapshot.snapshot.map.height)}`);
      return;
    }
    ...
    const inspection = await inspectCiv7RuntimeApi({
      ...
      port: flags.port,
      state: flags.state,
      roots: flags.roots?.split(',').map((root) => root.trim()).filter(Boolean),
      timeoutMs: flags['timeout-ms'],
    });
    ...
    for (const root of inspection.roots) {
      ...
      if (methods.length > 0) {
        this.log(`  methods: ${methods.map((method) => `${method.name}/${method.length}`).join(', ')}`);
      }
