...
export default class GamePlayCivilianRouteTriage extends Command {
  ...
  static flags = {
    ...
    x: Flags.integer({
      description:
        "Civilian or Settler origin x coordinate. Defaults to the first ready unit when omitted.",
      dependsOn: ["y"],
    }),
    y: Flags.integer({
      description:
        "Civilian or Settler origin y coordinate. Defaults to the first ready unit when omitted.",
      dependsOn: ["x"],
    }),
    "to-x": Flags.integer({
      description:
        "Optional candidate destination x coordinate. Defaults to the first settlement recommendation when available.",
      dependsOn: ["to-y"],
    }),
    "to-y": Flags.integer({
      description:
        "Optional candidate destination y coordinate. Defaults to the first settlement recommendation when available.",
      dependsOn: ["to-x"],
    }),
    count: Flags.integer({
...
function buildCliView(result: CivilianRouteTriageServiceResult): CivilianRouteTriageCliView {
  return {
    ...result,
    triage: {
      ...result.triage,
