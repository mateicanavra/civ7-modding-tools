...
export default class GamePlayTargetCandidates extends Command {
  ...
  public async run(): Promise<void> {
    ...
    for (const candidate of view.candidates) {
      this.log(
        `- owner ${candidate.owner}: distance=${candidate.nearestDistance ?? "<unknown>"}; cities=${candidate.cityCount}; units=${candidate.unitCount}; nearby=${candidate.nearbyUnitCount}; strength=${candidate.apparentStrength}`
      );
      this.log(
        `  civ=${candidate.civilizationName ?? "<unknown>"} leader=${candidate.leaderName ?? "<unknown>"} route=${candidate.approach.routeHint} kind=${candidate.approach.routeKind ?? "<unknown>"}`
      );
