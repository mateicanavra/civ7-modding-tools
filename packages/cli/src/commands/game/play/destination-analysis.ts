...
export default class GamePlayDestinationAnalysis extends Command {
  ...
  static flags = {
    ...
    "from-x": Flags.integer({
      description:
        "Optional origin x coordinate. Defaults to the first ready or selected unit when omitted.",
      dependsOn: ["from-y"],
    }),
    "from-y": Flags.integer({
      description:
        "Optional origin y coordinate. Defaults to the first ready or selected unit when omitted.",
      dependsOn: ["from-x"],
    }),
    origin: Flags.string({
      description:
        "Optional origin as x,y. Defaults to the first ready or selected unit when omitted.",
    }),
    "to-x": Flags.integer({
  ...
  public async run(): Promise<void> {
    ...
    this.log(
      `Player: ${view.playerId}; origin: ${formatValue(view.origin)}; destination: ${formatValue(view.destination)}`
    );
    this.log(
      `Corridor radius=${view.corridorRadius}; destination radius=${view.destinationRadius}`
    );
