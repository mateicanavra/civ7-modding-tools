...
export default class GamePlaySettlementRecommendations extends Command {
  ...
  public async run(): Promise<void> {
    ...
    const locations = flags.x === undefined || flags.y === undefined ? undefined : [{ x: flags.x, y: flags.y }];
    ...
    for (const recommendation of view.recommendations) {
      this.log(`Origin: ${recommendation.origin.kind} ${formatValue(recommendation.origin.location)}`);
