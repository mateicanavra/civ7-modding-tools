...
export default class GameRestart extends Command {
  ...
  static flags = {
    ...
    begin: Flags.boolean({
      description: 'Follow restart by calling the native Begin Game action when App UI reaches WaitingForUIReady',
      default: false,
    }),
  ...
  public async run(): Promise<void> {
    ...
    const health = flags.wait && !shouldBegin
    ...
    if (flowResponse) {
      this.log(`Begin Game action: ${flowResponse.begin ? flowResponse.begin.output.join('\n') || '<empty>' : '<not needed>'}`);
      this.log(`Final App UI loading state: ${flowResponse.finalAppUi.snapshot.ui.loadingStateName ?? '<unknown>'}`);
