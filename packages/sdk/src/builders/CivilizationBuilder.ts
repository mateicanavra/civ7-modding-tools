...
export class CivilizationBuilder extends BaseBuilder<TCivilizationBuilder> {
  ...
  migrate() {
    ...
    this._localizations.fill({
      englishText: this.localizations
        .map((item) => {
    ...
    const cityNamesCount =
      lodash.maxBy(this.localizations, (loc) => loc.cityNames?.length || 0)?.cityNames?.length || 0;
    for (let i = 1; i <= cityNamesCount; i++) {
      this._current.cityNames.push(
        new CityNameNode({
    ...
    items.forEach((item) => {
      ...
      if (item instanceof ModifierBuilder) {
        item._gameEffects.modifiers.forEach((modifier) => {
          ...
          if (!item.isDetached) {
            this._current.traitModifiers.push(
              new TraitModifierNode({
      ...
      if (item instanceof ProgressionTreeBuilder) {
        item._current.progressionTrees.forEach((progressionTree) => {
          this._gameEffects.modifiers.push(
            new ModifierNode({
          ...
          item._current.progressionTreeNodeUnlocks.push(
            new ProgressionTreeNodeUnlockNode({
      ...
      if (item instanceof CivilizationUnlockBuilder) {
        this._shell.civilizationUnlocks.push(
          new CivilizationUnlockNode({
      ...
      if (item instanceof LeaderUnlockBuilder) {
        this._shell.leaderUnlocks.push(
          new LeaderUnlockNode({
        ...
        this._shell.leaderCivilizationBias.push(
          new LeaderCivilizationBiasNode({
            civilizationDomain: CIVILIZATION_DOMAIN.from(item.leaderUnlock.ageType),
            civilizationType: item.leaderUnlock.type,
            reasonType: locale(
              `PLAY_AS_${trim(item.leaderUnlock.leaderType)}_${trim(item.leaderUnlock.type)}`,
              "TOOLTIP"
            ),
            ...item.leaderUnlock,
