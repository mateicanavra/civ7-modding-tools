...
export class UniqueQuarterBuilder extends BaseBuilder<TUniqueQuarterBuilder> {
  ...
  uniqueQuarter: TPartialRequired<
    UniqueQuarterNode,
    "uniqueQuarterType" | "buildingType1" | "buildingType2"
  > = {
  ...
  bind(items: (ModifierBuilder | ConstructibleBuilder)[]) {
    items.forEach((item) => {
      if (item instanceof ModifierBuilder) {
        ...
        if (!item.isDetached) {
          item._gameEffects.modifiers.forEach((modifier) => {
            this._always.gameModifiers.push(
              new GameModifierNode({
                modifierId: modifier.id,
              })
            );

            this._always.uniqueQuarterModifiers.push(
              new UniqueQuarterModifierNode({
