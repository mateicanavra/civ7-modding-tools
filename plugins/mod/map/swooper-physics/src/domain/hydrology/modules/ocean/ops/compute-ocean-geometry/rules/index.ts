...
/**
 * Derives connected ocean basins and bounded coastal geometry from water topology.
 *
 * Basin ids are one-based on water and zero on land. Coast distance is measured by hex traversal,
 * capped for far-ocean water, and left at `0xffff` on land; advisory normal/tangent vectors are
 * emitted only within the configured coast-vector radius for current projection.
 *
 * @param width - Number of tile columns in both masks.
 * @param height - Number of tile rows in both masks.
 * @param isWaterMask - Binary water topology used for connectivity and traversal.
 * @param coastalWaterMask - Water cells that seed zero-distance coastal traversal.
 * @param options - Bounds for distance traversal and near-coast vector generation.
 * @returns Basin ids, coast distance, and quantized normal/tangent fields aligned to tile indices.
 */
