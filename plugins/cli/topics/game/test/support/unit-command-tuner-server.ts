...
/** Starts the admitted Civ7 control scenario shared by exact unit-command tests. */
export async function startUnitCommandTunerServer() {
...
function unitCommandSend() {
  return {
    ...
    before: unitCommandSnapshot({ owner: 0, id: 65536, type: 26 }),
    after: unitCommandSnapshot({ owner: 0, id: 131072, type: 26 }),
  };
}

function unitCommandSnapshot(firstReadyUnitId: { owner: number; id: number; type: number }) {
  return {
    unit: {
      ok: true,
      value: {
