...
export default class GamePlayChooseTech extends Command {
  ...
  public async run(): Promise<void> {
    ...
    if (flags.options) {
      ...
      const enabledOptionCount = surfaces.reduce(
      ...
      emitPlayResult(this.log.bind(this), flags.json, {
        ...
        notes: ["Rows come from live HUD choices with official technology validation evidence."],
      });
      return;
    }
    ...
    if (typeof flags["player-id"] !== "number") {
      throw new Error(
        "game play choose-tech requires --player-id unless --options or --send is used"
      );
    }
    const input = {
...
function technologyChoiceDetails(
