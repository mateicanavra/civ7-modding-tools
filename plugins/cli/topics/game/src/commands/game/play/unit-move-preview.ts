...
export default class GamePlayUnitMovePreview extends Command {
  ...
  static flags = {
    ...
    compact: Flags.boolean({
      description:
        "In JSON mode, emit a summary-first movement preview envelope instead of the full official preview payload",
      default: false,
    }),
    json: Flags.boolean({
...
function buildCompactView(view: UnitMovePreviewView): {
  ...
  const movementCandidates = compactPlotCandidates(
    probeValue(view.reachableMovement),
    view.unitId,
    unit,
    candidateLimit
  );
  const targetCandidates = compactPlotCandidates(
    probeValue(view.reachableTargets),
    view.unitId,
    unit,
    candidateLimit
  );
  const zoneOfControlCandidates = compactPlotCandidates(
    probeValue(view.reachableZonesOfControl),
    view.unitId,
    unit,
    candidateLimit
  );
  ...
  return {
    ...
    omitted: [
      {
        path: "view.reachableMovement",
        reason: `compact includes up to ${candidateLimit} reachable movement candidate rows`,
      },
      {
        path: "view.reachableZonesOfControl",
        reason: `compact includes up to ${candidateLimit} zone-of-control candidate rows`,
      },
      {
        path: "view.reachableTargets",
        reason: `compact includes up to ${candidateLimit} reachable target candidate rows`,
      },
      {
        path: "view.requestedPath.plots",
        reason: "compact output summarizes requested path evidence without plot samples",
      },
      {
        path: "view.queuedPath.plots",
        reason: "compact output summarizes queued path evidence without plot samples",
      },
    ],
    hiddenInfoPolicy:
      "runtime-preview; visibility and route safety are not proven by compact movement output",
...
function compactPlotCandidates(
  value: unknown,
  unitId: UnitMovePreviewView["unitId"],
  unit: Record<string, unknown> | null,
  limit: number
  ...
  return flattenPlots(value)
    ...
    .map((plot) => ({
      ...
      nextAction: movementValidationDescriptor(
        unitId,
        typeof plot.x === "number" && typeof plot.y === "number" ? { x: plot.x, y: plot.y } : null
      ),
    }));
}
