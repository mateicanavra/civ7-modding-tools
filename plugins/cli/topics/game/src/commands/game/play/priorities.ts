...
import {
  ...
  type SemanticCliEnvelope,
} from "../../../utils/game-play/semantic-envelope";
...
export default class GamePlayPriorities extends Command {
  ...
  static flags = {
    ...
    compact: Flags.boolean({
      description:
        "In JSON mode, emit a summary-first play-agent envelope instead of the full dashboard payload",
      default: false,
    }),
    json: Flags.boolean({
...
function buildCompactView(view: PriorityView): {
  ...
  const compactPriorities = view.priorities
    .slice(0, 6)
    .map(({ priority, kind, summary, reason, nextAction }) => ({
      priority,
      kind,
      summary,
      reason,
  ...
  return {
    ...
    contractVersion: "play-agent-v0",
    surface: "priorities",
    summary: top ? `${top.kind}: ${top.summary}` : "no priorities surfaced",
    decisionHud,
    ...
    omitted: [
      {
        path: "view.notes",
        reason: "compact output keeps priority notes out of the main action surface",
      },
      {
        path: "view.readyUnit",
        reason: "compact output keeps ready-unit detail summarized by priority rows",
      },
      {
        path: "view.readyCity",
        reason: "compact output keeps ready-city detail summarized by priority rows",
      },
      {
        path: "view.battlefield.pointsOfInterest",
        reason: "compact output keeps battlefield detail summarized by priority rows",
      },
      {
        path: "priorities[].evidenceLabels",
        reason: "compact output omits bounded service evidence labels",
      },
    ],
