...
export default class GamePlayFormationSnapshot extends Command {
  ...
  static flags = {
    ...
    "contact-radius": Flags.integer({
      description:
        "Maximum grid distance for other-owner units to count as immediate civilian contacts. Defaults to 4.",
      min: 1,
      max: 8,
    }),
...
function formationCliView(result: FormationSnapshotServiceResult): FormationSnapshotCliView {
  return {
    ...result,
    formation: {
      ...result.formation,
