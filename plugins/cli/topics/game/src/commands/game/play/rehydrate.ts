...
export default class GamePlayRehydrate extends Command {
  ...
  public async run(): Promise<void> {
    ...
    const readyUnit = flags["skip-ready-unit"]
      ? null
      : await readReadyUnit(
          readyUnitId ?? expectedReadyUnit,
          flags.radius,
          flags["max-operations"],
          options
        );
    ...
    this.log(
      `Live turn: ${formatProbe(notifications.turn)} (${formatProbe(notifications.turnDate)})`
    );
    this.log(`Continuity: ${snapshot.continuity.status}`);
    for (const warning of snapshot.continuity.warnings) this.log(`Warning: ${warning}`);
    this.log(
      `Blocker: ${formatProbe(notifications.blocker)}; next decision: ${notifications.hud.nextDecision?.category ?? "<none>"}`
    );
...
function buildSnapshot(
  ...
  const warnings = checks
    ...
    .map(
      (check) =>
        `${check.label} mismatch: expected ${formatValue(check.expected)}, live ${formatValue(check.actual)}`
    );
  return {
    source: "live-direct-control",
    purpose: "restart-rehydration",
