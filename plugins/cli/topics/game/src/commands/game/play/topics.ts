...
const TOPICS: ReadonlyArray<Topic> = [
  {
    ...
    boundary:
      "specialized gameplay blockers outrank generic dismissal; re-read after every mutation or long-latency call",
  },
  {
    family: "progression",
    aliases: [
    ...
    loadWhen:
      "when the HUD names tech, culture, government, celebration, narrative, attribute, or tradition blockers",
  ...
  },
  {
    ...
    purpose:
      "Read city blockers and choose production, town focus, worker placement, expansion, or settlement planning paths.",
    ...
    loadWhen:
      "when production, town focus, new population, city expansion, or Settler route quality is the decision",
    boundary:
  ...
  },
  {
    ...
    references: [
      ...
      "docs/projects/civ7-live-play-support/topics/bulk-unit-analysis-and-operations.md",
  ...
  },
  {
    ...
    purpose:
      "Handle diplomatic action responses and first-meet greetings without confusing them with dismissible reports.",
    ...
    boundary:
      "grievance reports can be informational; first-meet and action responses are real player operations",
  },
  {
    family: "runtime-sources",
    aliases: ["sqlite", "local-data", "catalog", "authority"],
    purpose:
      "Separate live runtime authority from static local catalog, save, log, and forensic evidence.",
  ...
  },
  {
    ...
    purpose:
      "Recover after restart/reconnect and passively observe human or agent turns with freshness markers.",
    ...
    boundary:
      "prior thread state is only an expectation after restart; live rehydrate/watch output is the authority",
  },
  {
    family: "surface-design",
    aliases: ["commands", "api", "grammar", "aliases", "subcommands"],
    purpose:
      "Review and evolve the game play command grammar without breaking proven live-play commands.",
    proof: "design-reference",
    references: [
      "docs/projects/civ7-live-play-support/topics/command-surface-design.md",
      "docs/projects/civ7-live-play-support/topics/play-agent-response-contract.md",
      "docs/projects/civ7-live-play-support/topics/official-runtime-affordance-inventory.md",
      "docs/projects/civ7-live-play-support/agent-thread-ledger.md",
    ],
    commands: [
      "game play topics --family surface-design",
    ...
    loadWhen:
      "when command names, topic shortcuts, aliases, response tiers, or preview/check/send phases are being redesigned",
    boundary:
      "add wrappers and aliases first; do not remove existing commands until play-agent callers and tests have migrated",
  },
  {
    family: "evented-stream",
    aliases: ["pubsub", "subscription", "stream", "materialized-view", "events"],
    purpose:
      "Design semantic watcher topics and latest decision views so agents can subscribe instead of manually reconciling repeated reads.",
    ...
    loadWhen:
      "when deciding whether pub/sub, JSONL replay, latest-view cache, or direct snapshot polling should own watcher state",
    boundary:
      "streams may reduce reconciliation; direct-control snapshots and validators remain live authority until stronger runtime events are proven",
  },
  {
    family: "strategy",
    aliases: ["multi-turn", "autoplay", "objectives", "runner"],
    purpose:
      "Frame short-horizon objectives and decide whether to use external runner, static mod, or native autoplay.",
    ...
    loadWhen:
      "when setting a 5-10 turn posture, comparing against autoplay, or deciding what the next inspection should be",
    boundary:
      "strategy snapshots rank what to inspect; they never bypass live blocker reads, validators, or postconditions",
  },
  {
    family: "rhq-ai",
    ...
    loadWhen:
      "when comparing native autoplay/static AI mods with the external direct-control strategy runner",
    boundary:
      "RHQ public claims seed experiments; actual loaded GameInfo rows and bounded telemetry must prove behavior",
  },
];
...
function findTopics(query: string): ReadonlyArray<Topic> {
  ...
  return TOPICS.filter(
    (topic) =>
      normalize(topic.family) === normalized ||
      topic.aliases.some((alias) => normalize(alias) === normalized)
  );
}
