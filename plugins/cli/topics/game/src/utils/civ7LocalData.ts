...
/** Metadata for a discovered Civ7 save or log, with paths anchored to the inspected data root. */
...
/**
 * A discovered SQLite database and the optional result of its schema probe.
 * Probe failures remain inventory data so a missing or broken `sqlite3` executable does not hide the file.
 */
...
/**
 * Snapshot returned by the local-data inspection command.
 * The authority labels keep filesystem evidence distinct from live turn legality reported by direct control.
 */
...
/** Controls the filesystem root, optional SQLite probing, and result caps for local-data inspection. */
...
/**
 * Builds a synchronous inventory of Civ7 databases, saves, and logs for forensic CLI output.
 * A missing application-support directory produces an empty snapshot; save and log results are newest-first.
 *
 * @param options - Overrides the macOS data root, SQLite probing, or maximum save and log counts.
 * @returns The discovered files plus labels describing which conclusions require live game authority.
 */
