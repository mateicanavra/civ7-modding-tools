...
- [ ] 2.2 Confirm `rules.json` metadata for `grit-contract-export-all`.
- [ ] 2.3 Confirm current Grit predicate file classes and exact Habitat Grit
  adapter scan roots.
- [ ] 2.4 Confirm the old ESLint/script value-star invariant and retirement
  parity claim.
- [ ] 2.5 Reconcile registry metadata, taxonomy, discrepancy records, corpus
  row, and Grit predicate on domain-root facade coverage.
- [ ] 2.6 Decide whether `export * as name from ...` is allowed, forbidden, or
  sibling-owned.
...
- [ ] 3.3 Add or record proof for negative named value exports and named type
  exports.
- [ ] 3.4 Add or record proof for `export type *` allowance.
- [ ] 3.5 Add or record proof for namespace re-export classification.
...
- [ ] 4.5 Run current-tree inventory for value-star and type-star exports inside
  and outside effective scope.
- [ ] 4.6 Record live domain-root facade value-star examples and link predicate
...
- [ ] 8.1 `bun run openspec -- validate habitat-grit-proof-contract-export-all --strict`
- [ ] 8.2 native fixture proof
- [ ] 8.3 type-star proof
- [ ] 8.4 namespace re-export disposition
...
- [ ] 8.8 current-tree export inventory
- [ ] 8.9 domain-root facade disposition
...
- [ ] 8.14 aggregate proof matrix aligned
- [ ] 8.15 recovery claim ledger aligned
- [ ] 8.16 active-packet language guardrail scan
- [ ] 8.17 `git diff --check`
- [ ] 8.18 `bun run openspec:validate`
- [ ] 8.19 commit via Graphite with a clean worktree
