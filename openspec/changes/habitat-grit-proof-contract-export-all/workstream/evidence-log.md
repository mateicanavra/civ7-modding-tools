...
| CEA-E5 | `GRIT_TELEMETRY_DISABLED=true PATH="$PWD/node_modules/.bin:$PATH" grit patterns test --filter contract_export_all --json` | Exit 0; one testable pattern succeeded with one positive and one negative sample. | Native fixture proof. | No type-star, current-tree, baseline, injected, or rewrite proof. |
...
| CEA-E7 | `GRIT_TELEMETRY_DISABLED=true PATH="$PWD/node_modules/.bin:$PATH" grit check mods/mod-swooper-maps/src/domain mods/mod-swooper-maps/src/recipes --json --level error --no-cache` | Exit 0 with JSON `results: []`. | Bounded raw zero-result seed for domain/recipe roots. | Does not prove exact Habitat projection or injected behavior. |
