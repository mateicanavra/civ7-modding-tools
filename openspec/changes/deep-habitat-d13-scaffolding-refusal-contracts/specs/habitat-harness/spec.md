...
- **WHEN** a request names project kind `plugin` and
...
- **AND** the decision names the supported project kind, canonical root,
  canonical package name, and write set
...
set `plugin` until another accepted owner extends the contract. A supported
scaffold SHALL write only its canonical root layout.
...
### Requirement: Unsupported Project Kinds Refuse Before Writes

Habitat SHALL refuse unsupported scaffold kinds before writes. Current
schema-admitted but runtime-unsupported names SHALL be treated as intentional
compatibility/refusal surfaces only after D0 rows exist; they SHALL NOT become
generic Habitat taxonomy by accident.
...
- **AND** the refusal names the unsupported kind, recovery action, retry
  condition, and no-write result
...
- **AND** registration remains outside the candidate generator
...
### Requirement: Active Pattern Registration Is Pattern Governance-Owned

D13 SHALL NOT admit or register patterns by itself. Registered advisory/enforced
requests SHALL refuse before active writes with a Pattern Governance reason
projection.
...
- **AND** the refusal names the missing Pattern Authority manifest and recovery
  action
