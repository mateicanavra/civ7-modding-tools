...
- [x] 3.2 Record zero current Node builtin/runtime token matches in scanned
  present bundles.
- [x] 3.3 Record the Swooper manifest/output mismatch as a generated-output
  freshness blocker, not row closure.
...
- [x] 4.3 `bun run habitat:check -- --json --tool wrapped-test` recorded the
  separate Swooper generated-output freshness blocker; aggregate wrapped-test
  remains current-red.
...
- [x] 5.2 No active Grit check, Grit baseline, or injected Grit probe is
  claimed.
- [x] 5.3 No Swooper map bundle freshness repair is claimed.
