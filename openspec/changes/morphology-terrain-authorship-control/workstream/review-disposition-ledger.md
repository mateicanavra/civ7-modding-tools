...
| Elevation/cliff proof is incomplete. | P1 | accepted | Repaired for live surface proof by bounded `GameplayMap.getElevation` and `GameplayMap.isCliffCrossing` readback; target-map product proof remains unresolved. |
...
| Expectation ledger was stale after rough-land and live-readback slices. | P2 | accepted | Updated expectation rows to mark rough-land ownership implemented and live elevation/cliff surface observed while preserving target-map proof boundary. |
...
| World-balance component/local-relief stats use local no-wrap topology rather than canonical odd-q wrapping. | P2 | accepted | Follow-up repair slice required; stats must use canonical map topology before closure. |
| Resource regression proof is not covered by current terrain/feature stats. | P2 | accepted | Downstream resources realignment remains required; follow-up repair slice should add measured resource proof or an explicit Resources handoff. |
| `plan-rough-lands` lacks owner-local core invariant tests. | P2 | accepted | Follow-up repair slice required for water/mountain/foothill exclusion, causal gating, budget, and scoring invariants. |
| `fractalRoughLand` contract understates its role. | P2 | accepted | Follow-up repair slice required to align contract language with scoring and clustering behavior. |
