...
| rivers | Final river terrain is pipeline-controlled or exactly mirrored by Studio mock | after `map-rivers` | medium | hydrology truth plus `map-rivers` projection | river intent mask, engine river mask, mismatch mask | river terrain mismatch count trends to zero | live terrain and river metadata readback | proposed |
...
- Proxy gaps: `MockAdapter.buildElevation()` and `MockAdapter.modelRivers()` are
  known proxies; `MockAdapter.canHaveFeature()` and `MockAdapter.canHaveResource()`
