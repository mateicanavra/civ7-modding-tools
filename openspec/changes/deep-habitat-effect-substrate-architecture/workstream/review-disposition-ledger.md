...
| P1 | Movement map missed `src/lib/fix.ts`, leaving the `runFix` package export and D9/D10-adjacent command path without a target owner. | Repaired in `workstream/source-movement-map.md`; `src/lib/fix.ts` now splits across transformation transaction, pattern governance, Grit provider, and the public facade. |
