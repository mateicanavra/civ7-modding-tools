...
| `GritProvider` | `src/providers/grit/**` | Grit command construction, pattern discovery, scan-root admission facts, check/apply/test execution, cache acquisition, diagnostic/output parsing | pattern governance decisions or baseline policy | `CommandRunner`, `HabitatConfig`, `ResourceScope`, `HabitatFileSystem` | fake Grit outputs, parser fixtures, scan-root matrix |
