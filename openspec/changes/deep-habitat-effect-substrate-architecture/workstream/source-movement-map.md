...
| `src/lib/fix.ts` | 1 | `src/domains/transformation-transaction/**`, `src/domains/pattern-governance/**`, `src/providers/grit/**`, `src/public/index.ts` | split and facade | `runFix` remains package-export guarded while fix orchestration moves to transaction/pattern domains and Grit provider execution. |
