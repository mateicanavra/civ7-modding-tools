...
| `src/adapters/grit/**` | 10 | `src/providers/grit/**`, `src/domains/diagnostic-pattern-catalog/**` | move | Command construction, scan-root admission, diagnostics parsing, failure rendering become provider/domain split. |
| `src/lib/grit.ts` | 1 | `src/providers/grit/index.ts` | facade during cutover | Public/internal callsites must route through explicit provider exports, then retire broad facade. |
| `src/lib/grit-env.ts` | 1 | `src/providers/grit/commands.ts`, `src/config/**` | move | Grit environment/config is provider-owned and config-fed. |
| `src/lib/grit-failures.ts` | 1 | `src/providers/grit/errors.ts`, `src/errors/provider-errors.ts` | move | Preserve stable failure tags while introducing tagged provider errors. |
...
| `src/lib/fix.ts` | 1 | `src/service/modules/fix/**`, later `src/domains/transformation-transaction/**`, `src/domains/pattern-governance/**`, `src/providers/grit/**` | split and delete | `runFix` package helper export is refused; fix command orchestration moves to the Effect-oRPC service module, and lower-level transaction/pattern/Grit ownership drains in follow-on packets. |
...
| `src/lib/hooks.ts` | 1 | `src/domains/local-feedback/hooks.ts` | move | Public hook behavior remains command-facing; mutable staged/restage logic moves into local feedback domain. |
...
| `src/lib/pattern-apply/**` | 11 | `src/domains/transformation-transaction/**`, `src/providers/grit/**`, `src/domains/pattern-governance/**` | split | Apply admission, worktree, rollback, record, and render become transaction/domain responsibilities. |
