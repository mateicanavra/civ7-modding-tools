...
| Hook behavior | `.husky/pre-commit`; `.husky/pre-push`; `src/lib/hooks.ts`; `src/lib/hook-runtime/**`; `src/commands/hook.ts` | Effect resource scoping can change staged/restage behavior or local-only non-claims | staged/unstaged/partial/restage tests, pre-push base proof, hook non-claim receipt |
...
| Grit checks/apply | `src/adapters/grit/**`; `src/lib/pattern-apply/**`; `validate:grit-patterns` | Providerization can change scan-root admission, dry-run/apply output, or diagnostics parsing | Grit parser/scan-root/apply matrix and native pattern tests |
