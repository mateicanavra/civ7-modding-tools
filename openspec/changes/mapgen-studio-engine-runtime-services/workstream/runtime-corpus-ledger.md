...
| Run in Game registry | `createRunInGameOperationStore` stores public operation DTOs in a Map and prunes TTL. | package runtime stores internal Run ADTs and projects public DTOs. | public DTO shape becomes internal mutable state. | ADT projection and export privacy tests. |
| Save/Deploy registry | `createMapConfigSaveDeployOperationStore` stores public status DTOs in a Map and prunes TTL. | package runtime stores internal Save ADTs and projects public DTOs. | rollback/failure state remains patch-based. | Save projection and TTL tests. |
