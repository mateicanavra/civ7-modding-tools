...
| `docs/projects/habitat-harness/recovery-claim-ledger.md` | `CLAIM-H7-HOOKS` needed current resource-policy proof boundary. | patched in resource-policy checkpoint | Row now records implicit publish removal, read-only resource-state gate, fail-closed remediation, and open full hook proof classes. | patched-pending-review |
| `docs/process/resources-submodule.md` | Current docs said every monorepo commit auto-publishes resources through the hook. | patched in resource-policy checkpoint | Docs now make `bun run resources:publish` the explicit publish path and state pre-commit only checks resources state. | patched-pending-review |
| Root `AGENTS.md` | Current hook guidance preserved resource publish behavior as if settled. | patched in resource-policy checkpoint | Router now says hooks are local friction reduction and resource publishing is explicit, not a hidden default pre-commit side effect. | patched-pending-review |
| `tools/habitat-harness/README.md` | Hook guidance could imply local hook success is broader proof than it is. | patched in resource-policy checkpoint | Hook guidance now describes read-only resource-state checks, explicit remediation, formatter-touched restage, and local-proof limits. | patched-pending-review |
