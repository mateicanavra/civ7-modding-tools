...
| `proofClass` / `AdapterProofClass` | inherited from owning surface row | field wrapper or family-specific kind rename | Target field is family-specific kind such as `recordKind`, `artifactKind`, `captureKind`, or transaction state. |
...
| `d0_plane` | One D0 plane for this row, such as package-export, command-json, human-output, hook, command-behavior, docs-example, or generated/workstream-artifact. |
...
## D0 Surface Dependency Inventory

The seed rows below identify the required inventory coverage. Rows with multiple D0 planes must be expanded into one row per plane in the execution inventory before implementation.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `CheckReport` | `tools/habitat-harness/src/lib/diagnostics.ts`, `src/commands/check.ts` | expand-per-plane: package-export, command-json, docs-example | `blocked-pending-d0-row` per plane | copy from D0 | copy closed D0 value | check report | `CheckReport` | preserve schemaVersion 1 unless D0 versions | D1 check report summary | D6/D7 diagnostic taxonomy | tighten constructor/validator | `test/commands/habitat-entrypoints.test.ts`; `habitat check --json` | `ok: true` with failing rule | `does-not-prove-rule-correctness`, `does-not-prove-current-tree-cleanliness` | D6, D7, D12 | `blocked-pending-d0-row` |
| `validateCheckReport` | `tools/habitat-harness/src/lib/diagnostics.ts` | expand-per-plane: package-export, package-internal | `blocked-pending-d0-row` per plane | copy from D0 | copy closed D0 value | check report | `validateCheckReport` or successor | not applicable | D1 check report summary | D6/D7 diagnostic taxonomy | semantic validator/constructor gate | `test/commands/habitat-entrypoints.test.ts`; malformed payload unit test | structurally valid but semantically contradictory report | `does-not-prove-rule-correctness` | D6, D7, D12 | `blocked-pending-d0-row` |
| `RuleReport`, `HabitatDiagnostic` | `tools/habitat-harness/src/lib/diagnostics.ts` | expand-per-plane: package-export, command-json | `blocked-pending-d0-row` per plane | copy from D0 | copy closed D0 value | diagnostic containment | `RuleReport`, `Diagnostic` | preserve schemaVersion 1 unless D0 versions | D6/D7 diagnostic taxonomy | D1 receipt boundary only constrains containment/non-claims | preserve names and narrow validation | `test/commands/habitat-entrypoints.test.ts`; invalid severity unit test | unknown severity/status or standalone receipt use | `does-not-prove-rule-correctness` | D6, D7 | `blocked-pending-d0-row` |
| `VerifyProof`, `createVerifyProof` | `tools/habitat-harness/src/lib/command-engine.ts`, `src/commands/verify.ts` | expand-per-plane: command-json, human-output, command-behavior, docs-example | `blocked-pending-d0-row` per plane | copy from D0 | copy closed D0 value | verify receipt | `VerifyReceipt` target, `VerifyProof` legacy public name | preserve schemaVersion 1 or version by D0 row | D12 verify composition constrained by D1 | D1 shared receipt boundary | legacy-name wrapper or versioned public rename | `test/lib/verify-proof.test.ts`; `habitat verify --json` | executed Nx affected after failing check | `does-not-prove-ci`, `does-not-prove-graphite-readiness`, `does-not-prove-apply-safety` | D12, D14 | `blocked-pending-d0-row` |
| `HookTrace`, hook proof notice | `tools/habitat-harness/src/lib/hooks.ts` | expand-per-plane: hook, human-output, docs-example | `blocked-pending-d0-row` per plane | copy from D0 | copy closed D0 value | local feedback trace | `HookTrace` / `LocalFeedbackTrace` | not applicable unless public schema emerges | D11 local feedback constrained by D1 | CI/review systems | preserve trace, reclassify or version human wording | `test/lib/hooks.test.ts`; `habitat hook --help` | hook output implying CI authority | `local-feedback-only`, `does-not-prove-ci` | D11 | `blocked-pending-d0-row` |
| `GritApplyTransactionResult`, `GritApplyTransactionProof` | `tools/habitat-harness/src/lib/grit-apply.ts` | expand-per-plane: package-export, command-behavior, docs-example | `blocked-pending-d0-row` per plane | copy from D0 | copy closed D0 value | apply transaction | `ApplyTransactionRecord` target | preserve or version by D0 row | D9 apply transaction constrained by D1 | D1 shared receipt boundary only | legacy-name wrapper; D9 owns behavior | `test/lib/grit-apply.test.ts`; `habitat fix --dry-run` | `ok: true` with `failureTag` | `does-not-prove-runtime`, `does-not-prove-apply-safety` | D9, D11 | `blocked-pending-d0-row` |
| `AdapterProofArtifact`, `ProofArtifactWriter`, `adapterProofArtifactPath` | `tools/habitat-harness/src/lib/proof-artifact.ts` | expand-per-plane: package-export, generated/workstream-artifact, docs-example | `blocked-pending-d0-row` per plane | copy from D0 | copy closed D0 value | adapter command artifact | `AdapterCommandArtifact` target | generated artifact schema stance copied from D0 | current adapter artifact owner constrained by D1 | D15 unless triggered | legacy-name wrapper or generated artifact version | `test/lib/proof-artifact.test.ts` | unsafe artifact id, unknown retention, unbounded raw output body | `command-output-only`, `does-not-prove-current-tree-cleanliness` | D8, D9, D15 evaluation | `blocked-pending-d0-row` |
| Proof/evidence phrases in Habitat docs | `tools/habitat-harness/docs/IMPLEMENTED-SURFACE.md`, `tools/habitat-harness/docs/SCENARIOS.md` | docs-example | `blocked-pending-d0-row` | copy from D0 | copy closed D0 value | terminology compatibility | current-state prose or target replacement | not applicable | D1 terminology compatibility | downstream owner for non-D1 docs | classify before rewrite | docs review plus affected command tests | durable docs teaching proof as target domain | row-specific | D8, D10, D13, D14 | `blocked-pending-d0-row` |
| `ClassifiedTarget.proof` | `tools/habitat-harness/src/lib/command-engine.ts` | expand-per-plane: command-json, package-export | `blocked-pending-d0-row` per plane | copy from D0 | copy closed D0 value | orientation/routing metadata | downstream D3/D4 term | preserve unless D3/D4 versions | D3/D4 orientation/routing | D1 | no D1 source edit | `test/lib/classify.test.ts` or D3/D4 gate | D1 renaming classify metadata prematurely | `command-output-only` | D3, D4 | `protected-for-downstream` |
| Pattern Authority proof fields | `tools/habitat-harness/src/rules/pattern-authority/manifest.ts` | expand-per-plane: package-internal/rule-manifest, docs-example | `blocked-pending-d0-row` per plane | copy from D0 | copy closed D0 value | pattern governance terminology | downstream-owned | D8/D13-owned | D8/D13 pattern governance | D1 | no D1 source edit | `test/rules/pattern-authority-manifest.test.ts` or D8/D13 gate | D1 redesigning pattern governance terminology | row-specific | D8, D13 | `protected-for-downstream` |
