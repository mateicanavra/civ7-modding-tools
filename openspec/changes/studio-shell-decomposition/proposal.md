...
- **Test infrastructure (Step 0):** add `@testing-library/react` + jsdom, scoped via `environmentMatchGlobs: [['test/controllers/**','jsdom']]` so existing node tests are untouched (enables `renderHook` proof for the lifecycle-bound contracts).
- **Pure-move slices** (behavior identical, verified by parity/characterization tests): module pure-helpers, `useViewportLayout`, `useKeyboardShortcuts`, `useBrowserRun`, `useVizSelection`, `useLiveRuntime`, `useSetupControls`.
- **Atomic-move-with-contract slices** (a pure move that must honor a named cross-hook contract — object identity, ordered write, or synchronous derivation): `useStudioOperations` (busy-gate synchronicity + single-owner error channel), `usePresetLifecycle` (`markPresetApplied` `===`-identity guard + `applyAuthoringSnapshot` ordered write), `useSaveDeploy` and `useRunInGame` (which call those contracts).
