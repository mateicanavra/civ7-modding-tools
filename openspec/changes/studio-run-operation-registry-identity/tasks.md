...
- [x] 1.1 Change admission to request-id identity only.
- [x] 1.2 Rename content fingerprints in registry-facing code to correlation or
      attribution digests.
- [x] 1.3 Make expired request records lookup facts only for their own request
      id.

## 2. Durable Operation Record

- [x] 2.1 Add minimal `RunOperationRecord` persistence.
- [x] 2.2 Write phase/status/diagnostics updates through the durable record.
- [x] 2.3 Add `RuntimeOwnershipLease` acquisition at Run in Game admission.
- [x] 2.4 Block second active Run in Game operations and Save/Deploy
      deployed-mod writes while the lease is held.
- [x] 2.5 Terminalize abandoned non-terminal records on daemon startup.
- [x] 2.6 Release stale durable lease slots for abandoned records during startup
      terminalization.

## 3. Verification

- [x] 3.1 Add behavior tests for same-content new request admission after prior
      terminalization.
