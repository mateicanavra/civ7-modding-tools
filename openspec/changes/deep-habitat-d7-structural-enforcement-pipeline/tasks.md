...
- [x] 3.3 Introduce internal D7 closed states for `StructuralCheckRequest`,
...
- [x] 3.5 Replace manual `CheckReport.command` string construction with
  Oclif-derived command context in `StructuralCheckRequest`, preserving
  D0/D1-compatible serialized command output.
...
- [x] 4.2 Migrate execution planning to consume D2 execution facts and D3 graph
  availability/refusal facts.
- [x] 4.3 Migrate diagnostic consumption to consume D6 diagnostic projections and
...
- [x] 4.7 Delete local recomputation paths after each owning upstream projection
  is live and covered by tests.

## 5. Consumer Projections

- [x] 5.1 Publish `LocalFeedbackCheckProjection` for D11 and migrate hook
  compatibility code only through D0/D1-approved handling.
- [x] 5.2 Publish `VerifyCheckSummaryProjection` for D12, consume it for the
  verify affected-target allow/skip gate, and preserve the current D1/D12 receipt
  summary subset until D12 owns schema evolution.
- [x] 5.3 Preserve D7 non-claims in both projections: no CI, runtime/product,
...
- [x] 6.3 Run `bun run --cwd tools/habitat-harness test -- test/lib/enforcement-surface.test.ts`
  and disposition the current generated-output freshness failure.
- [x] 6.4 Run focused D7 report-construction, lane/status, rendering, baseline
  consumption, diagnostic-refusal, staged not-applicable, D11 projection, and D12
  projection tests added by implementation. D10 protected-zone tests remain out of
  scope until D10 source authority exists.
- [x] 6.5 Run focused command proofs for implemented current-tree/staged
...
Validation note: the focused D7 command/report suite passes.
`test/lib/enforcement-surface.test.ts` now distinguishes generated-output-gated
wrapped-test rules from ordinary direct wrapped-test proof; stale generated
output is projected as a diagnostic instead of being treated as a false-green
direct execution pass.
Command proof note: `bun run habitat check --json --rule definitely-not-a-rule`
exits 1 with valid schemaVersion 1 JSON and one `rule-selection-integrity`
failure. `bun run habitat check --staged --tool file-layer --json` exits 1 with
valid schemaVersion 1 JSON, four passing file-layer rules, and a dirty-worktree
`baseline-integrity` failure; this is expected staged-refusal behavior, not a D7
current-tree cleanliness claim. `bun run habitat check --staged --tool
grit-check --json` exits 1 with valid schemaVersion 1 JSON and 33 explicit D7
not-applicable Grit reports when the staged scope has no approved Grit roots; it
does not collapse to a baseline-only report.

## 7. Downstream Realignment And Closure

- [x] 7.1 Update D11 and D12 packet assumptions only after D7 acceptance records
  the final projection contracts.
- [x] 7.2 Update D8/D9/D13/D14/D15 downstream assumptions where D7 check outcome
  non-claims or trigger decisions affect them.
- [x] 7.3 Submit the D7 source-closure layer through Graphite with a clean
