...
| D8/D9 `useStudioEvents.ts` | Existing event hook owns event-stream hello/adoption behavior. | D6 protects `apps/mapgen-studio/src/app/hooks/useStudioEvents.ts` except explicit deletion-target notes; event/push semantics remain D8/D9-owned. |
...
| `operationAdoption.ts` | Current boot adoption helper is D6-owned. | Implementation updates this helper to read `studio.operations.current` once and preserve D8/D9 event hook ownership. |
