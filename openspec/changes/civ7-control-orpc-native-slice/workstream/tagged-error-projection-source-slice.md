...
historical `unit.ready.view` wrapper named below has since been burned down
after `attention.current` became the ready-unit service composer.

The write set is:

- existing `runtime.playable.status`, `notifications.view`, and the historical
...
- handler-local calls to the procedure `errors` map constructors for
  `DIRECT_CONTROL_UNAVAILABLE`, `NOTIFICATION_VIEW_UNAVAILABLE`, and the
  historical `READY_UNIT_VIEW_UNAVAILABLE`;
