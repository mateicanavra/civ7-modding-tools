...
| Mountain contrast row | retained mountain configurations | fixed shipped-identity scenarios | `local-stats`, exact retained-catalog membership, targeted review | Reclassified. The behaviorally duplicate `mountain-rivers-patch` comparison entry was retired; the catalog-wide mountain-drama metric study now owns retained contrast and relief evidence. |
