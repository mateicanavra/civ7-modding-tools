...
- **WHEN** `habitat:check -- --json --rule grit-placement-outcome-boundary`
  exits 0 with only `grit-placement-outcome-boundary` and
...
- **AND** Habitat records explicit empty baseline ownership through
  `baseline-integrity`
- **AND** Habitat SHALL NOT claim raw direct Grit acquisition, broader placement
  product closure, apply safety, generator/migration behavior, neighboring row
  proof, retired parity, or product proof from that command

#### Scenario: Injected violation path control passes

- **WHEN** the registered POB injected probe reports one finding on terminal
  placement apply source and keeps the out-of-scope placement index control
  clean
...
- **AND** aggregate injected-corpus closure SHALL remain separate from this row
  while unrelated injected rows remain blocked
...
- **WHEN** raw acquisition, Effect adapter behavior, apply safety,
  generator/migration proof, retired parity, broader placement product closure,
  neighboring row proof, aggregate injected-corpus closure, or product/runtime
  proof is not available in the current row proof set
