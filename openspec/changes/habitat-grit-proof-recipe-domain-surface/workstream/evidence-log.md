...
| RDS-E5 | `GRIT_TELEMETRY_DISABLED=true PATH="$PWD/node_modules/.bin:$PATH" grit patterns test --filter recipe_domain_surface --json` | Exit 0; one testable pattern succeeded with one positive and one negative sample. | Native fixture proof. | No parser-edge, current-tree, baseline, injected, overlap, or rewrite proof. |
...
| RDS-E7 | `GRIT_TELEMETRY_DISABLED=true PATH="$PWD/node_modules/.bin:$PATH" grit check mods/mod-swooper-maps/src/recipes --json --level error --no-cache` | Exit 0 with JSON `results: []`. | Bounded raw zero-result seed for the recipe root. | Does not prove exact Habitat projection or injected behavior. |
