...
Historical design/planning commands below were run from the original D0
remediation branch/worktree. Those paths are provenance only, not current
execution authority. Current implementation validation uses the active checkout
declared in
`docs/projects/habitat-harness/openspec-remediation/context.md`; see the matrix
Validation Evidence table for expected status, actual status, sample locations,
cache stance, and non-claims.
...
- Matrix:
  `docs/projects/habitat-harness/public-surface-compatibility-matrix.md`.
- Row count: 346 concrete D0 rows across all required planes.
...
- OpenSpec validation:
  `bun run openspec -- validate deep-habitat-d0-command-surface-inventory --strict`
  passed.
- OpenSpec all validation: `bun run openspec:validate` passed; latest
  record-authority repair run passed with 269 items.
