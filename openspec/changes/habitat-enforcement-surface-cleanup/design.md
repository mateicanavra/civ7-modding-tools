...
| Stale owner tool | Historical seed evidence found `check --tool wrapped-eslint` exiting 0 with only `baseline-integrity`; the accepted selector-proof checkpoint now rejects it through `rule-selection-integrity` with no `baseline-integrity` false green. | Command selector implementation belongs to command repair; this cleanup consumes that accepted boundary and records that stale owner-tool proof is not accepted. |
