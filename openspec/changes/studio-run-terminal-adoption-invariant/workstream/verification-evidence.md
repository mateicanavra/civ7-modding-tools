...
| --- | --- | --- | --- | --- | --- | --- | --- |
| openspec-strict | required | `bun run openspec -- validate studio-run-terminal-adoption-invariant --strict` | packet files written | not run | n/a | change validates strictly | open |
...
| adoption-tests | required | focused browser operation adoption tests | implementation diff exists | not run | n/a | UI adopts daemon terminal state | open |
| live-reload-reconnect | required | browser reload/reconnect against running Studio daemon | Studio server available | not run | n/a | terminal state visible after recovery | open |
