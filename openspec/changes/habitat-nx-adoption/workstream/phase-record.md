...
- Target movement: Nx fully adopted via the native Turbo migration; turbo retired; `tools/*` workspace glob + root runtime pins in place; all root scripts and CI re-pointed; build/check/test parity proven.
...
- Current code evidence: turbo.json (18 task entries), turbo 2.7.6 devDependency, no nx.json/runtime pins/tools/
...
- Write set: root `package.json`, `turbo.json` (delete), `nx.json` (new), `.nvmrc`, `.bun-version`, `.gitignore`, per-project `package.json` `"nx"` fields (7 project-scoped overrides), `.github/workflows/ci.yml`, `scripts/lint/lint-workspace-entrypoints.mjs`, root `AGENTS.md` + doc sweep, this phase record
