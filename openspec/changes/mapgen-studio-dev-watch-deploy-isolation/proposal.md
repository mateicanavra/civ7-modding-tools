...
- Operation deploy build commands are expressed through the accepted Nx/Habitat baseline. On the migrated baseline, Play and Save&Deploy use the dedicated repo-local mod package studio-deploy target: `bun run nx run mod-swooper-maps:build:studio-deploy --outputStyle=static`. D1 must verify that target with `bun run nx show project mod-swooper-maps --json` before changing the deploy plan. Broad root build orchestration, the broad mod package `build` target, and generated recipe targets are not deploy-operation authority.
