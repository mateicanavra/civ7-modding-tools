...
| D1-R5 | Dev-platform/Nx/Habitat review | Active proposal used vague `bun x nx` command example instead of repo-local pinned Nx command and verified mod target. | P2 | accepted, repaired | Replaced with `bun run nx run mod-swooper-maps:build:studio-deploy --outputStyle=static` and required `bun run nx show project mod-swooper-maps --json`. |
