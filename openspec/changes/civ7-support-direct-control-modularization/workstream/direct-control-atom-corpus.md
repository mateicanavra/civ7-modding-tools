...
| Autoplay and turn completion              | Autoplay types around 849-872, turn completion types around 893-906, wrappers around 2788-2920                                                                                                                            | `src/play/autoplay.ts` and `src/play/turn-completion.ts`                                                | `test/autoplay-and-turn.test.ts` owns autoplay status read plus approval/configure/start/stop coverage and turn-completion status, complete, and unready result-shape coverage. `src/play/autoplay.ts` owns `getCiv7AutoplayStatus`, `configureCiv7Autoplay`, `startCiv7Autoplay`, `stopCiv7Autoplay`, autoplay command builders, player inference, config matching, wait/stop-settling helpers, autoplay default constants, and default App UI snapshot, command execution, approval, validation/bounds, serializer, sleep, and timing/default dependency wiring while public facade exports stay in `index.ts`. `src/play/turn-completion.ts` owns `getCiv7TurnCompletionStatus`, `sendCiv7TurnComplete`, `sendCiv7TurnUnready`, the generated status command, private fallback classifier helpers, and default App UI execution/parser/notification-read/approval dependency wiring while public facade exports stay in `index.ts`; the public facade now re-exports the read-only status wrapper directly while retaining the send-turn-complete/send-unready mutation wrappers. CLI `game play end-turn` remains the focused consumer proof for the public play surface.                                                                                                                                                                                                                                                    | Preserve approval gates, explicit unbounded start semantics, stop-settling/pause behavior, turn-completion action result shapes, stale notification fallback classification, and command strings. Keep hotseat runtime proof, AI ingestion, semantic CLI projection, telemetry, and Effect/oRPC procedure cores in separate slices. | runtime proof for autoplay/turn-completion behavior changes                     | Autoplay wrapper/source/default constants and turn-completion wrapper/source ownership extracted; turn-completion facade dependency injection, autoplay status call-through, and turn-completion status call-through are pruned through existing non-facade owner imports/direct owner re-exports; hotseat runtime proof, AI ingestion, semantic CLI projection, telemetry, and Effect/oRPC procedure cores still pending. |
