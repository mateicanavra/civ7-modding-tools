...
Host Policy Boundary Gate is a downstream implementation-control packet derived from
`G-HOST-host-policy-boundary-gate.md`. The packet is complete only when it leaves the later execution
...
- Owner: Host Policy Boundary.
- Primary scenario: Habitat stays generic while host-specific repos such as Civ/MapGen declare protected paths, generated zones, and supported/refused host behavior through an explicit boundary.
