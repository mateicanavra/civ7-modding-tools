...
- [x] 1.2 Confirm `rules.json`, current Grit predicate, taxonomy, MapGen owner
  docs, invariant corpus, corpus ledger, and proof matrix source.
...
- [x] 2.3 Run parser inventory over the current MapGen core/engine roots with
...
- [ ] 3.4 Explicit baseline proof.
  - Blocked/non-claim until the scaffold/baseline contract surface is
    available.
- [ ] 3.5 Import predicate-gap and live type-only adapter import disposition.
  - Blocked/non-claim: native fixture proof shows runtime-global member
    branches match, but intended import branches do not report import syntax.
    Parser inventory found 4 current-predicate type-only adapter imports and 0
    adapter value imports, 0 `/base-standard/` imports, and 0 runtime-global
    member accesses. Clean import-class enforcement closure remains blocked
    until predicate semantics and source-owner/type-import policy are
    dispositioned.
