...
const usage = `Usage:
  nx run swooper-physics-mod:verify:studio-run-in-game-live -- [flags]
...
export function buildSwooperMapScriptDeploymentStage(args: {
  ...
  return {
    ...
    ...(ok
      ...
      : {
          recoveryHint:
            "Build and deploy the current Swooper Maps bundle before live verification: nx run swooper-physics-mod:deploy",
        }),
  };
}
