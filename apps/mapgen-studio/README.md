...
- **Storybook workbench** — package-hosted, stories co-located in the package
  `src/`: `cd packages/mapgen-studio-ui && bun run storybook`. The app has zero
  story files; `packages/mapgen-studio-ui/.storybook/EXCLUSIONS.md` records
  which app-side hosts (StudioShell, StudioProviders, DeckCanvas, CanvasStage)
  deliberately stay unstoried.
