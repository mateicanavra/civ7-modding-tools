...
export type AuthoringState = AuthoringData & {
  ...
  setGameSeed: (next: Updater<string>) => void;
...
function buildInitialAuthoringData(): AuthoringData {
  ...
  return {
    ...
    gameSeed: persisted?.gameSeed ?? "123",
  ...
  persist(
    (set) => {
      ...
      return {
        ...
        setGameSeed: (next) =>
          set((state) => {
            const gameSeed = resolve(next, state.gameSeed);
            return parseCiv7StudioSeed(gameSeed).ok
              ? { gameSeed, authoringRevision: state.authoringRevision + 1 }
              : state;
          }),
        setSetupConfig: (next) =>
          set((state) => ({
