...
export function parseMapConfigSaveRequest(body: MapConfigSaveRequestInput): ParsedMapConfigSaveRequest {
  if (body.restart === true || body.verifyRestart === true) {
    throw new Error("Map config save/deploy does not restart Civ; use Run in Game for Civ lifecycle control.");
  }
  if (typeof body.id !== "string" || !MAP_CONFIG_ID_PATTERN.test(body.id)) {
    throw new Error("Map config id must be kebab-case");
  }
  if (body.requestId !== undefined && (typeof body.requestId !== "string" || !REQUEST_ID_PATTERN.test(body.requestId))) {
