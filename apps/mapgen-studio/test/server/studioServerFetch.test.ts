...
import { createStudioServerFetch, type StudioServerDeps } from "../../src/server/studioServer";

function makeDeps(overrides: Partial<StudioServerDeps> = {}): {
...
describe("studio server fetch router", () => {
  test("serves /healthz from the health probe", async () => {
    const { deps } = makeDeps();
    const fetchHandler = createStudioServerFetch(deps);
    const res = await fetchHandler(new Request("http://studio.test/healthz"));
  ...
  test("healthz reports 503 when the probe is unhealthy", async () => {
    ...
    const res = await createStudioServerFetch(deps)(new Request("http://studio.test/healthz"));
  ...
  test("routes every /rpc namespace to the ONE handler", async () => {
    ...
    for (const path of [
      ...
      const res = await fetchHandler(new Request(`http://studio.test${path}`, { method: "POST" }));
  ...
  test("unmatched RPC paths under the mounted prefix are 404", async () => {
    ...
    const res = await createStudioServerFetch(deps)(
      new Request("http://studio.test/rpc/nope", { method: "POST" }),
    );
  ...
  test("retired /api paths are 404 — legacy REST and the former satellite mounts", async () => {
    ...
    for (const path of [
      ...
      const res = await fetchHandler(new Request(`http://studio.test${path}`));
  ...
  test("retired /api paths stay 404 even with a static assets root", async () => {
    ...
    const res = await createStudioServerFetch(deps)(
      new Request("http://studio.test/api/civ7/rpc/readiness/current"),
    );
    expect(res.status).toBe(404);
  });
  ...
  test("non-API paths without an assets root are 404", async () => {
    const { deps } = makeDeps();
    const res = await createStudioServerFetch(deps)(new Request("http://studio.test/"));
    expect(res.status).toBe(404);
  });
});
