...
function loadConfig(): Config {
  if (!existsSync(CONFIG_PATH)) {
    return { prefixes: ["Civilization VII Typescript Modding Tools", "Typescript Modding Tools"] };
  }
  try {
    ...
    if (!parsed.prefixes || !Array.isArray(parsed.prefixes) || parsed.prefixes.length === 0) {
      return {
        prefixes: ["Civilization VII Typescript Modding Tools", "Typescript Modding Tools"],
      };
    }
...
function parseFrontmatter(text: string): {
  ...
  if (!text.startsWith("---"))
    return { hasFm: false, fmStart: 0, fmEnd: 0, titleLineIdx: -1, lines: text.split(/\r?\n/) };
...
function updateTitleInFile(filePath: string, prefixes: string[]): boolean {
  ...
  if (mQuoted) {
    currentTitle = mQuoted[1];
    quote = "double";
  } else if (mSQuoted) {
    currentTitle = mSQuoted[1];
    quote = "single";
  } else if (mBare) {
    currentTitle = mBare[1].trim();
    quote = "none";
  }
...
function main(): void {
  ...
  console.log(
    `Title prefix cleanup complete. Files processed: ${processed}. Files changed: ${changed}.`
  );
}
