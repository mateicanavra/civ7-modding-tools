...
`$body` where {
  $filename <: r".*tools/habitat/src/service/runtime/grit/.*\.ts$",
  $text = text($body),
  or {
...
```typescript
// @filename: tools/habitat/src/service/runtime/grit/constants.ts
...
// @filename: .habitat/habitat/toolkit/_self/check/example-rule/example-rule.rule.json
