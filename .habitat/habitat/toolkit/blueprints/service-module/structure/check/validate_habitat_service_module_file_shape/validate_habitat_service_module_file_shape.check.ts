    ...
    if (kind === "policy" && !isPolicyFile(relativeToKind, options)) {
      report(
        ...
        options.strictPolicyNames
          ? "shared service model/policy files must be index.ts, schemas, *.policy.ts, *.policy.mjs, or *.policy.json."
...
function findRepoRoot(startDirectory: string): string {
  ...
  while (true) {
    if (
      fs.existsSync(path.join(current, ".habitat")) &&
      fs.existsSync(path.join(current, "tools/habitat/package.json"))
    ) {
      return current;
    }
