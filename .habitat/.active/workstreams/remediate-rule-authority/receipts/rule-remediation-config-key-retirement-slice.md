...
| --- | --- | --- | --- | --- |
| Higher authority frame still allowed retired-literal pressure to route into package-local tests/validators. | P2 | repaired | `.habitat/.active/frames/FRAME.md` now qualifies package-local validation as live behavior/validation only and routes retired exact literals with no recurrence risk to `remove`. | none |
