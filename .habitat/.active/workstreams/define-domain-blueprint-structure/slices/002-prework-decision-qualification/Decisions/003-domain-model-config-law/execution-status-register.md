...
| `NATIVE_GRIT_UNAVAILABLE_RECORDED` | Harness capability proof | Native `grit patterns test --filter <rule>` is proven unavailable in this checkout because active `.grit/grit.yaml` registers no testable corpus. | Does not prove rule behavior. |
