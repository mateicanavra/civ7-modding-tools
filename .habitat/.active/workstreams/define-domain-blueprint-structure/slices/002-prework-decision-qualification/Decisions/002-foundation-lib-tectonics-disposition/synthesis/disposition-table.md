...
| `lib/require.ts` / all `require*` exports | live | Contract-owned artifact validation surface | shared `foundation/lib`; broad validation bucket; operation-local copied guards as final owner | Route guard semantics to artifact contract modules: `mesh.contract.ts`, `crust.contract.ts`, `mantle-potential.contract.ts`, `mantle-forcing.contract.ts`, `plate-graph.contract.ts`, `plate-motion.contract.ts`, `current-tectonics.contract.ts`, `tectonic-history.contract.ts`, and `tectonic-provenance.contract.ts`. Each module owns schema, artifact definition, and `validate`; add a generic `assert` export only where execution proves an operation still needs contextual compatibility checks such as expected cell count or plate count. | verified | `require-guards.domino.md`; `evidence/require-guards-agent-a.md`; `evidence/require-guards-agent-c.md`; `decision-book/move-classes.md` |
