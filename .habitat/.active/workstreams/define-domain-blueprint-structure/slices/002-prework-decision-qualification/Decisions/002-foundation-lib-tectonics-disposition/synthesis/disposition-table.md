...
| `lib/require.ts` / all `require*` exports | live | Contract-owned artifact validation surface | shared `foundation/lib`; broad validation bucket; operation-local copied guards as final owner | Route guard semantics to artifact contract modules: `mesh.contract.ts`, `crust.contract.ts`, `mantle-potential.contract.ts`, `mantle-forcing.contract.ts`, `plate-graph.contract.ts`, `plate-motion.contract.ts`, `current-tectonics.contract.ts`, `tectonic-history.contract.ts`, and `tectonic-provenance.contract.ts`. Concrete file/export shape is governed by the active artifact contract scope pattern. | verified | `require-guards.domino.md`; `evidence/require-guards-agent-a.md`; `evidence/require-guards-agent-c.md`; `decision-book/move-classes.md`; `../../../../../scopes/domain/scopes/artifacts/scopes/contract/patterns/artifact-contract-shape.md` |
