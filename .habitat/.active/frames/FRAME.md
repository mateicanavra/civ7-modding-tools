...
Execution plans live in workstream records, OpenSpec changes, Graphite
branches, and the relevant `.habitat/.active/dominoes/items/` record when the
work advances a domino. This frame exists to keep those execution artifacts
aimed at the same authority-tree end state.
