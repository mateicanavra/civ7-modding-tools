...
Primary object: the repeatable end-of-one method for authority activation. The
method is a law-to-enforcement loop, with slice inventory carrying execution
state.
...
the assertion, route to named owner-law gaps, or disappear.
...
- all current paths matched by the selected kind glob;
...
4. Every nonconforming file or logic fragment gets a destination, deletion,
   named owner-law gap, or evidence that the asserted law must be narrowed.
...
it belongs under the new shape, which owner-law gap it routes to, or whether
the shape must be narrowed.

Some overlapping rules survive when they protect a distinct concern after the
closed structure is enforced. Duplicate symptom guards collapse into the new
enforcement.

## Falsifier

This frame fails when a selected run lacks a live structural assertion. The
method produces red/green repo feedback, not only explanatory structure text.
...
turning one selected law into a live gate, then burning down every red row
...
The semantic investigation asks for the common denominator across current paths
matched by the selected kind glob, but current commonality can only propose a
candidate law. The law itself must come from direct user decision, accepted
Habitat authority, controlling architecture documents, or source facts that
those authorities already make relevant. Current code remains implementation
evidence. If a file violates the selected law, the question
is "which locked move class owns this logic now that the kind has a closed
structure?" The answer is one of: destination inside the selected structure,
destination outside the selected structure, deletion, named owner-law gap, or
law correction. Before code moves,
each moved fragment has a destination owner, import shape, target file
...
The stages below describe the state change required before the work pauses.
...
Paths read: <current paths matched by the selected kind glob>.
Semantic finding: <common denominator or no-law proof, with source refs>.
Red now: <violating paths or none>.
Destinations: <moves/deletions/owner-law gaps/law corrections decided>.
...
Progress has occurred only when current matched-path investigation is folded
into the selected law, the live gate, or a no-law proof in the same run note.
...
Progress has occurred when the kind's selected structure is stated in enough
...
owner-law gaps to name, or evidence that the assertion was premature.
...
Progress has occurred when red paths have been moved, rewritten, deleted, routed
to a named owner-law gap, or used as law-correction evidence. The run must
decide destinations for old code. A file that violates
the closed shape is decomposed by logic: each piece either belongs in the
new shape, belongs to another owner, is dead, names an owner-law gap, or proves
the law needs correction.
Each destination decision carries source-owner proof, import shape, target
...
the new enforcement. A rule overlaps when its violation condition appears only
without also violating the closed structure, or when its only remediation is
"make this path fit the closed structure."

Overlap means the new enforcement subsumes the old reason for the rule.
Same-file or same-category contact is only a candidate signal.
...
adapter calls, and stage orchestration route to their owning authorities. If an
accepted authority also names exact directory grammar, that grammar can become
the closed shape. Otherwise, the first domain run should assert the semantic
boundary first and make directory grammar a direct decision before enforcing it.
...
Return file:line findings. Generic checklist feedback is out of scope. Ask an
...
Update this frame only when the method itself changes. Selected kind, red paths,
and deleted rules live in the small run output, the relevant commit, and any
existing authority records that the changed structure affects.
