# boundary-taxonomy

Blueprint: `project-boundary-model`

Primary category: `structure`

Secondary categories: `boundary`

Artifact kind: `check`

Lifecycle: `steady`

Admission: `admitted`

Authority path: `.habitat/global/workspace/blueprints/project-boundary-model/structure/check/boundary-taxonomy`

Files:
- `boundary-taxonomy.baseline.json`
- `boundary-taxonomy.check.ts`
- `boundary-taxonomy.rule.json`

Evidence: The check audits package manifests, Nx metadata, boundary config, and taxonomy docs for graph/model consistency.

Notes:
- none
