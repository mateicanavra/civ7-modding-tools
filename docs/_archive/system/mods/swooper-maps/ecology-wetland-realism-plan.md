...
Status: historical implementation plan. The branch, paths, and proposed
artifact placement below preserve the original design record; they are not
current artifact-authority guidance. If this work is resumed, any published
feature-eligibility product belongs in the Ecology features module's artifact
catalog, while masks with no cross-step consumer remain operation-local. A
recipe stage must not define an artifact catalog.
