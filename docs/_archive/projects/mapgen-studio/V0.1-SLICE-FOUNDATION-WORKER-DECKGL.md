...
**Status**  
Complete (implemented: Web Worker runs `browser-test` recipe (Foundation-only) and streams layers directly to deck.gl without requiring a dump).
...
**Goal**  
From MapGen Studio, click “Run (Browser)” → a Web Worker runs the `browser-test` recipe (currently Foundation-only) end-to-end using bundled Civ tables + browser-safe adapter → streams 1–2 visualization layers in-memory → deck.gl renders them immediately. No “generate dump → load dump” is required for visualization.
