...
**Deferred:** 2026-07-08
**Trigger:** A new Claude Code / design-sync skill version (binary-grep for `@kind` or a token-classification/metadata surface); any claude.ai/design changelog touching `check_design_system` token classification. (A third trigger — the post-upload `tokenKinds` falsifier for the value-form migration — FIRED CONFIRMED 2026-07-08; see the outcome note in Context.)
**Context:** `check_design_system` in the MapGen Studio DS project permanently reports ~80 "unclassified tokens" and 33 "selector-scoped custom properties" — all Tailwind v4 engine internals in the compiled bundle. The classifier is app-side (the claude.ai/design self-check that regenerates `_adherence.oxlintrc.json`); no repo-side surface exists to fix or suppress it (verified: no token classifier in the bundled converter, no `@kind` annotation support anywhere in CC v2.1.197). Feedback packet: `openspec/changes/studio-ui-token-noise-disposition/workstream/upstream-feedback.md`. **Value-form update (2026-07-08):** the sibling change `studio-ui-token-value-form` modernizes the authored token dialect to full `hsl(…)` values consumed as `var(--token)`, which **moots upstream-feedback item 2** (recognize the shadcn HSL-triplet color pattern) — there are no more triplets to recognize, and the classifier already handles full color functions (`#000` classifies today). Findings #1 (unclassified `--tw-*`/`@theme` defaults) and #2 (selector-scoped custom properties), plus feedback items 1/3/4, are **unchanged** — still app-side framework noise this deferral covers; only item 2 retires. **Falsifier outcome (2026-07-08, post-upload check run):** CONFIRMED — sampled authored semantic tokens (`--background`, `--primary`, `--accent`, `--destructive`, `--border`) now classify `"color"` in the regenerated `x-omelette.tokenKinds` (all were `"other"` pre-migration); the value-form hypothesis holds. Residual `tokenKinds` mislabels are confined to `--tw-*` utility internals (e.g. `--tw-translate-y`, `--tw-rotate-*`, `--tw-backdrop-*` tagged `"color"`) — in-scope of this deferral's framework-noise disposition; no separate action.
