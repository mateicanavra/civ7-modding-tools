...
**Deferred:** 2026-06-10
**Trigger:** Access to a machine with the Civ7 game install (`bun run refresh:data`); rerun `nx run civ7-map-policy:generate` and `nx run civ7-map-policy:generated:check`, then re-prove byte-stability/diffs immediately after.
...
**Resolved (2026-07-15):** The resources gitlink is now `c9f612ba19242a4add63acf9e5a344178898b725`; the map-policy generator adopted its four new Heian/Sengoku start-bias rows, and the subsequent generated-currentness, typecheck, test, and build graph passed. Generated provenance now records the exact submodule commit rather than a redundant snapshot date.
...
**Context:** `check_design_system` in the MapGen Studio DS project permanently reports ~80 "unclassified tokens" and 33 "selector-scoped custom properties" — all Tailwind v4 engine internals in the compiled bundle. The classifier is app-side (the claude.ai/design self-check that regenerates `_adherence.oxlintrc.json`); no repo-side surface exists to fix or suppress it (verified: no token classifier in the bundled converter, no `@kind` annotation support anywhere in CC v2.1.197). Feedback packet: `openspec/changes/archive/2026-07-08-studio-ui-token-noise-disposition/workstream/upstream-feedback.md`. **Value-form update (2026-07-08):** the sibling change `studio-ui-token-value-form` modernizes the authored token dialect to full color-function values consumed as `var(--token)` (full `hsl(…)` in that Step A change; re-authored to oklch by the follow-on `studio-ui-token-oklch` Step B — an exact pixel-preserving conversion, still a full color function), which **moots upstream-feedback item 2** (recognize the shadcn HSL-triplet color pattern) — there are no more triplets to recognize, and the classifier already handles full color functions (`#000` classifies today). Findings #1 (unclassified `--tw-*`/`@theme` defaults) and #2 (selector-scoped custom properties), plus feedback items 1/3/4, are **unchanged** — still app-side framework noise this deferral covers; only item 2 retires. **Step A falsifier outcome (2026-07-08, post-HSL-upload check run):** CONFIRMED — sampled authored semantic tokens (`--background`, `--primary`, `--accent`, `--destructive`, `--border`) classify `"color"` in the regenerated `x-omelette.tokenKinds` (all were `"other"` before Step A); the full-HSL value-form hypothesis holds. **Step B falsifier outcome (2026-07-10, post-OKLCH-upload check run):** CONFIRMED — the same authored semantic surface continues to classify `"color"`; the `hsl(…)` to `oklch(…)` value-form conversion is classifier-neutral. Residual `tokenKinds` mislabels are confined to `--tw-*` utility internals (e.g. `--tw-translate-y`, `--tw-rotate-*`, `--tw-backdrop-*` tagged `"color"`) — in-scope of this deferral's framework-noise disposition; no separate action. **`@kind` trigger outcome (2026-07-18): FIRED (app-side).** The design-side session reports the app classifier now accepts project-supplied `/* @kind <kind> */` annotations in the scanned stylesheet — the classification input feedback item 4 asked for. (The CC 2.1.209 bundled skill/converter still carries no `@kind` surface — grep-verified — so this shipped app-side only.) Repo action taken same day: `packages/mapgen-studio-ui/scripts/annotate-token-kinds.mjs` (in the package `build` after the tailwind compile) annotates every custom-property declaration in the compiled stylesheet — authored kinds from the token-guard fixture, framework internals explicitly `other`, fail-loud coverage cross-check — and the annotated bundle uploaded to the project. **Annotation falsifier outcome (2026-07-18, same-day): PARTIAL — consumed but insufficient.** The regenerated `_adherence.oxlintrc.json` proves the app consumed the annotations: `x-omelette.tokenKinds` matches the annotation map 1:1 (previously mislabeled `--tw-translate-y`/`--tw-rotate-*`/`--tw-backdrop-*` flipped from `"color"` to `"other"`), and the tokenKinds ordering follows first-declaration order across the whole stylesheet — proof the discovery scan reads every custom-property declaration (utility-scoped included) and never harvests `@property` preludes. But classification ≠ exclusion: the `x-omelette.tokens` registry still lists all ~79 `--tw-*` engine names as design tokens (137 entries total), so annotating plumbing `other` blessed it into the registry instead of removing it — Matei's design-side read: the kind was placed incorrectly / the attribution doesn't hold. **Superseded same day by surface curation:** `scripts/curate-token-surface.mjs` (replacing `annotate-token-kinds.mjs`) strips the trailing `@layer properties` `@supports` fallback block (78 universal `--tw-*` declarations; render-neutral wherever `@property` is supported — the `@property` rules remain, and the utilities carry inline `var(--x, fallback)` defaults besides) and keeps `@kind` annotations on the remaining surface (authored + `@theme` defaults with real kinds; utility-internal `--tw-*` assignments explicitly `other`, since the scan provably reads them). Verified repo-side: 211/211 package tests green, re-sync driver reports zero component render-key changes (no re-grades). ~~Falsifier pending: the next post-upload `check_design_system` run should report zero unclassified tokens and a registry shrunk to authored + `@theme` + a small utility-internal `other` residue (~75 names, down from 137); record the outcome here.~~ **Curation falsifier outcome (2026-07-21): CONTAMINATED — the check ran against a design-side-mutated stylesheet, then the curation was re-asserted.** The remote `_ds_bundle.css` was found carrying a hand-authored `:root { --tw-*: … }` registration prelude (23 declarations mirroring the `@property` initial values, annotated `@kind other`, comment: "registered here so the token scanner recognizes utility-scope overrides") that no repo build ever emitted — an out-of-band design-side edit, i.e. the hoist-to-`:root` advice this deferral's disposition explicitly refuses, applied in "inert" form. Effects measured in the regenerated surfaces: `x-omelette.tokens` at 81 names (58 real + the 23 `--tw-*`), `_ds_manifest.json` at 143 token entries (23 unscoped `--tw-*` + 35 utility-scoped `--tw-*`), and a **themes list of 36 entries where 35 are Tailwind utility classes** (`.shadow-2xl`, `.-translate-x-1/2`, `.ring-warning/40`, …) beside the one real `.light` — scope-harvesting keys off `:root`-registered names, so the prelude is what turned utility classes into "themes". The sync anchor could not see the edit (the `_ds_sync.json` sidecar records what WE uploaded; out-of-band edits don't touch it) — seal discipline now includes a post-upload byte refetch of `_ds_bundle.css`. Repo response (same day): canonical prelude-less stylesheet re-asserted to the project; the ownership rule hardened in the synced conventions + `docs/design-tokens.md` (the "inert registration" loophole is now explicitly foreclosed). **Falsifier re-armed: the next `check_design_system` / manifest recompile after the re-assert should show the registry at ~58 real names (zero `--tw-*`), themes collapsed to `.light` alone, and zero unclassified tokens — record here. If the prelude REAPPEARS without a design-side edit, the rewriter is the app itself and this becomes upstream feedback item 5.** The selector-scoped finding should shrink to the utility-internal vars only (required to be selector-scoped); that residue and feedback items 1/3 remain app-side.
...
**Context:** The setup-correctness audit (initiative `docs/projects/studio-design-pipeline/INITIATIVE-setup-correctness.md`, A1.2) found the previous arrangement was a lie: `import.meta.env.DEV ? app : <StrictMode>{app}</StrictMode>` wrapped only the production tree, but React runs StrictMode checks exclusively in development builds — coverage was structurally ZERO everywhere while the comment claimed prod coverage. The dev-side blocker is real (deck.gl/luma device init crashes under double-mount — the same reason DeckCanvas is excluded from the Storybook workbench). 2026-07-21: main.tsx now renders without StrictMode everywhere, honestly, with this entry as the tracked exception.
**Impact:** Effect-cleanup and double-mount defects in the app's hook-heavy shell (16 orchestration hooks, abort/mounted refs, event-stream reopen timers) are not exercised by StrictMode until the trigger fires; review remains the only guard for that class.
