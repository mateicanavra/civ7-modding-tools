...
import {
  ...
  dumpBiomes,
  dumpContinents,
} from "/base-standard/maps/map-debug-helpers.js";
import {
  DEV_LOG_CFG as __DEV_CFG__,
...
try {
  ...
  if (__cfg && typeof __cfg === "object") {
    ...
    if ("rainfallHistogram" in __cfg) DEV.RAINFALL_HISTOGRAM = !!__cfg.rainfallHistogram;
    if ("LOG_RAINFALL_SUMMARY" in __cfg) DEV.LOG_RAINFALL_SUMMARY = !!__cfg.LOG_RAINFALL_SUMMARY;
    if ("LOG_CORRIDOR_ASCII" in __cfg) DEV.LOG_CORRIDOR_ASCII = !!__cfg.LOG_CORRIDOR_ASCII;
    ...
    if ("LOG_FOUNDATION_ASCII" in __cfg) DEV.LOG_FOUNDATION_ASCII = !!__cfg.LOG_FOUNDATION_ASCII;
    else if ("LOG_WORLDMODEL_ASCII" in __cfg)
      DEV.LOG_FOUNDATION_ASCII = !!__cfg.LOG_WORLDMODEL_ASCII;
    if ("LOG_LANDMASS_ASCII" in __cfg) DEV.LOG_LANDMASS_ASCII = !!__cfg.LOG_LANDMASS_ASCII;
    if ("LOG_LANDMASS_WINDOWS" in __cfg) DEV.LOG_LANDMASS_WINDOWS = !!__cfg.LOG_LANDMASS_WINDOWS;
    if ("LOG_RELIEF_ASCII" in __cfg) DEV.LOG_RELIEF_ASCII = !!__cfg.LOG_RELIEF_ASCII;
    if ("LOG_RAINFALL_ASCII" in __cfg) DEV.LOG_RAINFALL_ASCII = !!__cfg.LOG_RAINFALL_ASCII;
    if ("LOG_BIOME_ASCII" in __cfg) DEV.LOG_BIOME_ASCII = !!__cfg.LOG_BIOME_ASCII;
    if ("LOG_BIOME_SUMMARY" in __cfg) DEV.LOG_BIOME_SUMMARY = !!__cfg.LOG_BIOME_SUMMARY;
    if ("LOG_SWATCHES" in __cfg) DEV.LOG_SWATCHES = !!__cfg.LOG_SWATCHES;
    if ("LOG_BOUNDARY_METRICS" in __cfg) DEV.LOG_BOUNDARY_METRICS = !!__cfg.LOG_BOUNDARY_METRICS;
    if ("LOG_MOUNTAINS" in __cfg) DEV.LOG_MOUNTAINS = !!__cfg.LOG_MOUNTAINS;
    if ("LOG_VOLCANOES" in __cfg) DEV.LOG_VOLCANOES = !!__cfg.LOG_VOLCANOES;
    if ("FOUNDATION_HISTOGRAMS" in __cfg) DEV.FOUNDATION_HISTOGRAMS = !!__cfg.FOUNDATION_HISTOGRAMS;
    else if ("WORLDMODEL_HISTOGRAMS" in __cfg)
      DEV.FOUNDATION_HISTOGRAMS = !!__cfg.WORLDMODEL_HISTOGRAMS;
    if ("LOG_FOUNDATION_SEED" in __cfg) DEV.LOG_FOUNDATION_SEED = !!__cfg.LOG_FOUNDATION_SEED;
    if ("LOG_FOUNDATION_PLATES" in __cfg) DEV.LOG_FOUNDATION_PLATES = !!__cfg.LOG_FOUNDATION_PLATES;
...
function applyFoundationDiagnostics() {
  ...
  if (logSeed || logPlates || logDynamics || logSurface) DEV.ENABLED = true;
  if (logSeed) {
...
export function logRainfallStats(label = "rainfall", width, height) {
  if (!isOn("LOG_RAINFALL_SUMMARY")) return;
  try {
...
export function logBiomeSummary(label = "biomes", width, height) {
  if (!isOn("LOG_BIOME_SUMMARY")) return;
  try {
    const w = Number.isFinite(width) ? width : GameplayMap?.getGridWidth?.();
    const h = Number.isFinite(height) ? height : GameplayMap?.getGridHeight?.();
    if (!w || !h) {
...
export function logFoundationSeed(seedConfig, plateSeed, options = {}) {
  if (!isOn("LOG_FOUNDATION_SEED")) return;
...
export function logFoundationPlates(platesConfig) {
  if (!isOn("LOG_FOUNDATION_PLATES")) return;
  try {
...
export function logFoundationDynamics(dynamicsConfig) {
  if (!isOn("LOG_FOUNDATION_DYNAMICS")) return;
  try {
...
export function logFoundationSurface(surfaceConfig) {
  if (!isOn("LOG_FOUNDATION_SURFACE")) return;
  try {
...
export function logFoundationSummary(WorldModel) {
  if (!isOn("LOG_FOUNDATION_SUMMARY")) return;
  try {
    const enabled =
      !!WorldModel && typeof WorldModel.isEnabled === "function" && !!WorldModel.isEnabled();
    if (!enabled) {
    ...
    const rowsToSample = [
      ...
      height - 1,
    ].filter((y, idx, arr) => y >= 0 && y < height && arr.indexOf(y) === idx);
...
export function logFoundationHistograms(WorldModel, opts = {}) {
  if (!isOn("FOUNDATION_HISTOGRAMS")) return;
  try {
    const enabled =
      !!WorldModel && typeof WorldModel.isEnabled === "function" && !!WorldModel.isEnabled();
    if (!enabled) {
    ...
    const histMasked = (arr, maskSet) => {
      if (!maskSet || !(maskSet instanceof Set) || maskSet.size === 0) return null;
      const h = new Array(bins).fill(0);
      let samples = 0;
    ...
    if (mUbelts && mRbelts) {
      safeLog(
        "[DEV][foundation] uplift (near orogeny belts) hist:",
        pct(mUbelts.h, mUbelts.samples)
      );
      safeLog(
        "[DEV][foundation] rift   (near orogeny belts) hist:",
...
export function logFoundationAscii(WorldModel, opts = {}) {
  if (!isOn("LOG_FOUNDATION_ASCII")) return;
  try {
    const enabled =
      !!WorldModel && typeof WorldModel.isEnabled === "function" && !!WorldModel.isEnabled();
    if (!enabled) {
    ...
    const thresholdRatio =
      typeof opts.boundaryThreshold === "number"
    ...
    safeLog(
      `[DEV][foundation] ascii plates (step=${sampleStep}): base ${legendBasePair(baseWater)} ocean, ${legendBasePair(baseLand)} land; overlays ${overlays.convergent} convergent, ${overlays.divergent} divergent, ${overlays.transform} transform, ${overlays.boundary} boundary/unknown`
    );
...
export function logLandmassAscii(label = "landmass", opts = {}) {
  if (!isOn("LOG_LANDMASS_ASCII")) return;
  try {
    const width = GameplayMap?.getGridWidth?.() ?? 0;
    const height = GameplayMap?.getGridHeight?.() ?? 0;
    if (!width || !height) {
...
export function logTerrainReliefAscii(label = "relief") {
  if (!isOn("LOG_RELIEF_ASCII")) return;
  try {
    const width = GameplayMap?.getGridWidth?.() ?? 0;
    const height = GameplayMap?.getGridHeight?.() ?? 0;
    if (!width || !height) {
...
export function logRainfallAscii(label = "rainfall") {
  if (!isOn("LOG_RAINFALL_ASCII")) return;
  try {
    const width = GameplayMap?.getGridWidth?.() ?? 0;
    const height = GameplayMap?.getGridHeight?.() ?? 0;
    if (!width || !height) {
...
export function logBiomeAscii(label = "biomes") {
  if (!isOn("LOG_BIOME_ASCII")) return;
  try {
    const width = GameplayMap?.getGridWidth?.() ?? 0;
    const height = GameplayMap?.getGridHeight?.() ?? 0;
    if (!width || !height) {
...
export function logBoundaryMetrics(WorldModel, opts = {}) {
  if (!isOn("LOG_BOUNDARY_METRICS")) return;
  try {
    const enabled =
      !!WorldModel && typeof WorldModel.isEnabled === "function" && !!WorldModel.isEnabled();
    if (!enabled) {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        if (bType >= 0 && bType < counts.length) counts[bType]++;
        const bin = Math.min(bins - 1, Math.floor(closeness * bins));
        hist[bin]++;
        thresholds.forEach((t, idx) => {
          if (closeness >= t) thresholdHits[idx]++;
        });
        ...
        if (isMountain) {
          mountains++;
          if (bType >= 0 && bType < mountainByType.length) mountainByType[bType]++;
          thresholds.forEach((t, idx) => {
            if (closeness >= t) mountainByBand[idx]++;
          });
        } else if (terrainType === globals.g_HillTerrain) {
          hills++;
          if (bType >= 0 && bType < hillByType.length) hillByType[bType]++;
          thresholds.forEach((t, idx) => {
            if (closeness >= t) hillByBand[idx]++;
          });
        }
        if (featureType === globals.g_VolcanoFeature) {
          volcanoes++;
          if (bType >= 0 && bType < volcanoByType.length) volcanoByType[bType]++;
          thresholds.forEach((t, idx) => {
            if (closeness >= t) volcanoByBand[idx]++;
          });
        }
      }
    }
    ...
    safeLog(
      `[DEV][foundation] metrics${stage}: counts ${JSON.stringify({
        totalTiles,
        none: counts[0],
        convergent: counts[1],
        divergent: counts[2],
    ...
    safeLog(
      `[DEV][foundation] metrics: share ${JSON.stringify({
    ...
    safeLog(
      `[DEV][foundation] metrics: closeness histogram ${JSON.stringify(hist.map((count, idx) => ({ bin: idx, count })))}`
    );
    thresholds.forEach((t, idx) => {
      safeLog(
        `[DEV][foundation] metrics: closeness >= ${t.toFixed(2)} ${JSON.stringify({
    ...
    safeLog(
      `[DEV][foundation] metrics: mountains ${JSON.stringify({
        total: mountains,
    ...
    safeLog(
      `[DEV][foundation] metrics: hills ${JSON.stringify({
        total: hills,
        none: hillByType[0],
    ...
    safeLog(
      `[DEV][foundation] metrics: volcanoes ${JSON.stringify({
        total: volcanoes,
    ...
    thresholds.forEach((t, idx) => {
      safeLog(
        `[DEV][foundation] metrics: >=${t.toFixed(2)} overlays ${JSON.stringify({
...
export function logCorridorAsciiOverlay(step = 8) {
  ...
  try {
    ...
    safeLog(
      `[DEV][corridor] ASCII overlay (step=${s}): base ${legendBasePair(baseWater)} ocean, ${legendBasePair(baseLand)} land; overlays ${overlayChars.seaLane} sea-lane, ${overlayChars.islandHop} island-hop, ${overlayChars.landOpen} land-open, ${overlayChars.riverChain} river-chain`
    );
...
function computeAsciiSampleStep(width, height, requested) {
  if (Number.isFinite(requested)) return Math.max(1, Math.trunc(requested));
  const targetCols = 72;
  const targetRows = 48;
...
function asciiCell(base, overlay) {
  ...
  const padding = (overlay ? grid.overlayPadding : grid.basePadding) ||
    grid.basePadding || { left: "", right: "" };
...
function summarizeFoundationSeedConfig(cfg) {
  ...
  if (seedCfg.offsets && typeof seedCfg.offsets === "object") {
    ...
    for (const key of Object.keys(seedCfg.offsets)) {
      ...
      if (value != null) offsets[key] = value;
    }
    if (Object.keys(offsets).length > 0) summary.offsets = offsets;
  }
  return summary;
}

function summarizePlateSeed(plateSeed) {
  if (!plateSeed || typeof plateSeed !== "object") return { available: false };
  ...
  const sites = Array.isArray(plateSeed.sites)
    ...
    : Array.isArray(plateSeed.siteCoords)
      ...
      : null;
  if (sites) summary.siteCount = sites.length;
  if (plateSeed.seed != null) summary.seed = plateSeed.seed;
  if (plateSeed.seedOffset != null) summary.seedOffset = plateSeed.seedOffset;
  if (plateSeed.timestamp != null) summary.timestamp = plateSeed.timestamp;
  const rngState = plateSeed.rngState ?? plateSeed.randomState ?? null;
  if (rngState) summary.rngState = summarizeRngState(rngState);
  return summary;
}
...
function summarizeFoundationDynamics(cfg) {
  if (!cfg || typeof cfg !== "object") return {};
  const wind = pickFields(cfg.wind, [
    "jetStreaks",
    "jetStrength",
    "variance",
    "coriolisZonalScale",
  ]);
  const currents = pickFields(cfg.currents, [
    "basinGyreCountMax",
    "westernBoundaryBias",
    "currentStrength",
  ]);
  ...
  const out = {};
  if (wind) out.wind = wind;
  if (currents) out.currents = currents;
  if (mantle) out.mantle = mantle;
  if (directionality) out.directionality = directionality;
  return out;
}
...
function summarizeDirectionality(cfg) {
  ...
  const out = {};
  if (cfg.cohesion != null) out.cohesion = cfg.cohesion;
  const primaryAxes = pickFields(cfg.primaryAxes, [
    "plateAxisDeg",
    "windBiasDeg",
    "currentBiasDeg",
  ]);
  if (primaryAxes) out.primaryAxes = primaryAxes;
  ...
  if (interplay) out.interplay = interplay;
  const hemispheres = pickFields(cfg.hemispheres, [
    "southernFlip",
    "equatorBandDeg",
    "monsoonBias",
  ]);
  if (hemispheres) out.hemispheres = hemispheres;
  const variability = pickFields(cfg.variability, [
    "angleJitterDeg",
    "magnitudeVariance",
    "seedOffset",
  ]);
  if (variability) out.variability = variability;
...
function summarizeOceanSeparation(cfg) {
  ...
  if (Array.isArray(cfg.bandPairs)) out.bandPairs = cfg.bandPairs.length;
  if (cfg.edgeWest && cfg.edgeWest.enabled)
    out.edgeWest = pickFields(cfg.edgeWest, [
      "baseTiles",
      "boundaryClosenessMultiplier",
      "maxPerRowDelta",
    ]);
  if (cfg.edgeEast && cfg.edgeEast.enabled)
    out.edgeEast = pickFields(cfg.edgeEast, [
      "baseTiles",
      "boundaryClosenessMultiplier",
      "maxPerRowDelta",
    ]);
...
function summarizeRngState(state) {
  ...
  if (keys.length > limit) summary.truncatedKeys = keys.length - limit;
  return summary;
}
