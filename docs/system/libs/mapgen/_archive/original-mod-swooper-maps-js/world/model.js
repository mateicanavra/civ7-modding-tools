...
function computePressure(width, height) {
  ...
  for (let i = 0; i < bumps; i++) {
    const cx = TerrainBuilder?.getRandomNumber?.(width, "PressCX") ?? (i * width) / bumps;
    const cy = TerrainBuilder?.getRandomNumber?.(height, "PressCY") ?? (i * height) / bumps;
...
function computeWinds(width, height) {
  ...
  for (let y = 0; y < height; y++) {
    ...
    // Per-row variance
    const varU =
      Math.round(((TerrainBuilder?.getRandomNumber?.(21, "WindUVar") ?? 0) - 10) * variance) | 0;
    const varV =
      Math.round(((TerrainBuilder?.getRandomNumber?.(11, "WindVVar") ?? 0) - 5) * variance) | 0;
    for (let x = 0; x < width; x++) {
      ...
      (function applyWindBias() {
        try {
          ...
          biasDeg += (DIR?.primaryAxes?.plateAxisDeg ?? 0) * followPlates * cohesion;
          // Optional hemisphere flip around equator
          const rawLat = GameplayMap.getPlotLatitude(0, y);
          if ((DIR?.hemispheres?.southernFlip ?? false) && rawLat < 0) {
            biasDeg = -biasDeg;
          }
          const angleJitter = (DIR?.variability?.angleJitterDeg ?? 0) | 0;
          const jitter =
            (TerrainBuilder?.getRandomNumber?.(angleJitter * 2 + 1, "WindDirJit") ?? 0) -
            angleJitter;
      ...
      if (GameplayMap.isWater(x, y)) {
        ...
        (function applyCurrentBias() {
          try {
            ...
            const cohesion = Math.max(0, Math.min(1, DIR?.cohesion ?? 0));
            const windsFactor =
              Math.max(0, Math.min(1, DIR?.interplay?.currentsFollowWinds ?? 0)) * cohesion;
            ...
            const angleJitter = (DIR?.variability?.angleJitterDeg ?? 0) | 0;
            const jitter =
              (TerrainBuilder?.getRandomNumber?.(angleJitter * 2 + 1, "CurrentDirJit") ?? 0) -
              angleJitter;
            // Optional hemisphere flip around equator
            const rawLat = GameplayMap.getPlotLatitude(0, y);
            if ((DIR?.hemispheres?.southernFlip ?? false) && rawLat < 0) {
