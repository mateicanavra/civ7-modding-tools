...
function assignCorridorMetadata(key, kind, style) {
  ...
  if (primitive) StoryTags.corridorAttributes.set(key, primitive);
  else StoryTags.corridorAttributes.delete(key);
}
...
function tagSeaLanes() {
  ...
  const currentsFollowWinds =
    Math.max(0, Math.min(1, DIR?.interplay?.currentsFollowWinds ?? 0)) * COH;
  ...
  function laneVec(orient) {
    ...
    if (orient === "diagNE") return { x: 1 / Math.SQRT2, y: -1 / Math.SQRT2 }; // NE-SW
    if (orient === "diagNW") return { x: 1 / Math.SQRT2, y: 1 / Math.SQRT2 }; // NW-SE
    return { x: 1, y: 0 };
  }
    ...
    if (c.orient === "col") {
      ...
      for (let y = c.start; y <= c.end; y++) {
        if (GameplayMap.isWater(x, y)) {
          ...
          const style = isAdjacentToLand(x, y, 2) ? "coastal" : "ocean";
      ...
      for (let x = c.start; x <= c.end; x++) {
        const y = k - x;
        if (x >= 0 && x < width && y >= 0 && y < height && GameplayMap.isWater(x, y)) {
          const kk = storyKey(x, y);
          StoryTags.corridorSeaLane.add(kk);
          const style = isAdjacentToLand(x, y, 2) ? "coastal" : "ocean";
          assignCorridorMetadata(kk, "sea", style);
...
function tagLandCorridorsFromRifts() {
  ...
  for (let y = 1; y < height - 1 && corridors < maxCorridors; y++) {
    ...
    while (x < width - 1 && corridors < maxCorridors) {
      // Skip non‑shoulder
      while (x < width - 1 && !StoryTags.riftShoulder.has(storyKey(x, y))) x++;
      if (x >= width - 1) break;
      const start = x;
      while (x < width - 1 && StoryTags.riftShoulder.has(storyKey(x, y))) x++;
      const end = x - 1;
          ...
          try {
            ...
            if (cohesion > 0) {
              ...
              if (alignWind >= hiAlign) {
                if (avgRain > 110 || (latDeg < 25 && avgRain > 100)) {
                  style = "grasslandBelt";
...
function tagRiverChainsPostRivers() {
  ...
  while (chains < maxChains && tries < 300) {
    ...
    let endOK = true;
    if (mustEndNearCoast) {
      endOK = GameplayMap.isCoastalLand(x, y) || GameplayMap.isAdjacentToShallowWater(x, y);
    }
...
function backfillCorridorKinds() {
  ...
  for (const key of StoryTags.corridorSeaLane) {
    ...
    if (!style) {
      const [sx, sy] = key.split(",").map(Number);
      style = GameplayMap.isAdjacentToShallowWater(sx, sy) ? "coastal" : "ocean";
    }
