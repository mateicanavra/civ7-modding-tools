  ...
  while (trailsMade < maxTrails && attempts < 200) {
    ...
    for (let s = 0; s < steps; s++) {
      ...
      if (TerrainBuilder.getRandomNumber(5, "HotspotBend") === 0) {
        dIndex =
          (dIndex + (TerrainBuilder.getRandomNumber(3, "HotspotTurn") - 1) + dirs.length) %
          dirs.length;
        [dx, dy] = dirs[dIndex];
      }
    }
  ...
  if (useWM) {
    ...
    for (const seed of chosen) {
        ...
        sdy = 0;
      {
        ...
        for (let dy = -1; dy <= 1; dy++) {
          for (let dx = -1; dx <= 1; dx++) {
            ...
            const nx = x + dx,
              ny = y + dy;
            if (!inb(nx, ny) || GameplayMap.isWater(nx, ny)) continue;
      ...
      for (let s = 0; s < lineSteps; s++) {
        if (!inb(x, y) || GameplayMap.isWater(x, y) || latDegAt(y) > 70) break;
        ...
        for (let ty = -1; ty <= 1; ty++) {
          for (let tx = -1; tx <= 1; tx++) {
            ...
            const cx = x + tx * stepLen,
              cy = y + ty * stepLen;
            if (!inb(cx, cy) || GameplayMap.isWater(cx, cy)) continue;
    ...
    while (riftsMade < maxRiftsPerMap && tries < 300) {
      ...
      for (let s = 0; s < lineSteps; s++) {
        ...
        if (TerrainBuilder.getRandomNumber(6, "RiftBend") === 0) {
          if (useNS) {
            dir = dirsNS[TerrainBuilder.getRandomNumber(dirsNS.length, "RiftDirNS2")];
          } else {
            dir = dirsEW[TerrainBuilder.getRandomNumber(dirsEW.length, "RiftDirEW2")];
          }
          [dx, dy] = dir;
        }
      }
...
export function storyTagContinentalMargins(ctxOrOptions = null, maybeOptions = null) {
  ...
  const baseActiveFrac = Number.isFinite(mcfg.activeFraction) ? mcfg.activeFraction : 0.25;
  const basePassiveFrac = Number.isFinite(mcfg.passiveFraction) ? mcfg.passiveFraction : 0.25;
  const activeFrac = Math.min(0.35, baseActiveFrac + 0.05 * (sqrt - 1));
  const passiveFrac = Math.min(0.35, basePassiveFrac + 0.05 * (sqrt - 1));
  const baseMinSeg = Number.isFinite(mcfg.minSegmentLength) ? mcfg.minSegmentLength : 12;
...
export function storyTagClimateSwatches(ctx = null) {
  ...
  if (!storyMoisture?.swatches) return { applied: false, kind: "missing-config" };
  const width = ctx?.dimensions?.width ?? GameplayMap.getGridWidth();
  const height = ctx?.dimensions?.height ?? GameplayMap.getGridHeight();
...
export function storyTagPaleoHydrology(ctx = null) {
  ...
  const rand = (n, lbl) =>
    ctx ? ctxRandom(ctx, lbl || "Paleo", n) : TerrainBuilder.getRandomNumber(n, lbl || "Paleo");
  ...
  if (cfg.maxDeltas > 0) {
    for (let y = 1; y < height - 1 && deltas < cfg.maxDeltas; y++) {
      for (let x = 1; x < width - 1 && deltas < cfg.maxDeltas; x++) {
        if (
          !(adapter?.isCoastalLand ? adapter.isCoastalLand(x, y) : GameplayMap.isCoastalLand(x, y))
        )
          continue;
        if (
          !(adapter?.isAdjacentToRivers
            ? adapter.isAdjacentToRivers(x, y, 1)
            : GameplayMap.isAdjacentToRivers(x, y, 1))
        )
          continue;
        if (
          (adapter?.getElevation ? adapter.getElevation(x, y) : GameplayMap.getElevation(x, y)) >
          300
        )
          continue;
        ...
        for (let dy = -fanR; dy <= fanR; dy++) {
          for (let dx = -fanR; dx <= fanR; dx++) {
            ...
            if (adapter?.isWater ? adapter.isWater(nx, ny) : GameplayMap.isWater(nx, ny)) continue;
  ...
  if (cfg.maxOxbows > 0) {
    ...
    while (oxbows < cfg.maxOxbows && attempts < 300) {
      ...
      if (adapter?.isWater ? adapter.isWater(x, y) : GameplayMap.isWater(x, y)) continue;
      const elev = adapter?.getElevation
  ...
  if (cfg.maxFossilChannels > 0) {
    ...
    while (fossils < cfg.maxFossilChannels && tries < 120) {
      ...
      if (!inBounds(sx, sy)) continue;
      if (adapter?.isWater ? adapter.isWater(sx, sy) : GameplayMap.isWater(sx, sy)) continue;
      const startElev = adapter?.getElevation
        ? adapter.getElevation(sx, sy)
        : GameplayMap.getElevation(sx, sy);
      if (startElev > 320) continue;
      if (
        adapter?.isAdjacentToRivers
          ? adapter.isAdjacentToRivers(sx, sy, minDistFromRivers)
          : GameplayMap.isAdjacentToRivers(sx, sy, minDistFromRivers)
      )
        continue;
      let x = sx,
        y = sy;
      ...
      while (used < len) {
        if (
          inBounds(x, y) &&
          !(adapter?.isWater ? adapter.isWater(x, y) : GameplayMap.isWater(x, y))
        ) {
          let rf = readRainfall(x, y);
          ...
          if ((canyonCfg.enableCanyonRim ?? true) && rimW > 0) {
            for (let ry = -rimW; ry <= rimW; ry++) {
              for (let rx = -rimW; rx <= rimW; rx++) {
                ...
                if (
                  !inBounds(nx, ny) ||
                  (adapter?.isWater ? adapter.isWater(nx, ny) : GameplayMap.isWater(nx, ny))
                )
                  continue;
                const e0 = adapter?.getElevation
                  ? adapter.getElevation(x, y)
                  : GameplayMap.getElevation(x, y);
                const e1 = adapter?.getElevation
                  ? adapter.getElevation(nx, ny)
                  : GameplayMap.getElevation(nx, ny);
                if (e1 > e0 + 15) {
        ...
        let bestNX = x,
          bestNY = y,
          bestElev = adapter?.getElevation
            ? adapter.getElevation(x, y)
            : GameplayMap.getElevation(x, y);
        for (let dy = -1; dy <= 1; dy++) {
          for (let dx = -1; dx <= 1; dx++) {
            if (dx === 0 && dy === 0) continue;
            if ((dx !== 0 || dy !== 0) && (Math.abs(dx) + Math.abs(dy)) * step > 2) continue;
            const nx = x + dx;
            const ny = y + dy;
            if (!inBounds(nx, ny)) continue;
            const elev = adapter?.getElevation
