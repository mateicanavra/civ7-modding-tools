...
function computePlateBias(closenessNorm, boundaryType, cfg) {
  if (closenessNorm == null || Number.isNaN(closenessNorm)) closenessNorm = 0;
  ...
  if (closenessNorm >= threshold) {
    ...
    let typeMul = 0;
    if (boundaryType === ENUM_BOUNDARY.convergent) typeMul = cfg.convergent;
    else if (boundaryType === ENUM_BOUNDARY.transform) typeMul = cfg.transform;
    else if (boundaryType === ENUM_BOUNDARY.divergent) typeMul = cfg.divergent;
    weight = ramp * typeMul;
...
export function addRuggedCoasts(iWidth, iHeight, ctx) {
  ...
  const minSeaLaneWidth = Number.isFinite(cfg.minSeaLaneWidth) ? cfg.minSeaLaneWidth : 4; // reserved for future shelf/trench guards
  ...
  for (let y = 1; y < iHeight - 1; y++) {
    for (let x = 1; x < iWidth - 1; x++) {
      ...
      if (GameplayMap.isCoastalLand(x, y)) {
        ...
        let bayRollDenUsed =
          _softMult !== 1 ? Math.max(1, Math.round(bayRollDen / _softMult)) : bayRollDen;
        ...
        const laneAttr = (function () {
          for (let ddy = -1; ddy <= 1; ddy++) {
            for (let ddx = -1; ddx <= 1; ddx++) {
              ...
              const k = `${x + ddx},${y + ddy}`;
              if (StoryTags.corridorSeaLane && StoryTags.corridorSeaLane.has(k)) {
        ...
        if (
          h % 97 < noiseGate &&
          (ctx
            ? ctxRandom(ctx, "Carve Bay", bayRollDenUsed)
        ...
        if (isAdjacentToLand(x, y, 1)) {
          {
            ...
            let denomUsed = _softMult !== 1 ? Math.max(1, Math.round(denom / _softMult)) : denom;
            ...
            // Corridor edge effect: if adjacent to a sea-lane tile, increase fjord/coast conversion chance
            {
              ...
              for (let my = -1; my <= 1 && !edgeCfg; my++) {
                for (let mx = -1; mx <= 1; mx++) {
                  ...
                  const kk = `${x + mx},${y + my}`;
                  if (StoryTags.corridorSeaLane && StoryTags.corridorSeaLane.has(kk)) {
            ...
            if (
              (ctx
                ? ctxRandom(ctx, "Fjord Coast", denomUsed)
                : TerrainBuilder.getRandomNumber(denomUsed, "Fjord Coast")) === 0
            ) {
              applyTerrain(x, y, globals.g_CoastTerrain, false);
            }
          }
