...
export function addIslandChains(iWidth, iHeight, ctx) {
  ...
  const peakPercent = Math.max(
    0,
    Math.min(100, Math.round((STORY_TUNABLES?.hotspot?.volcanicPeakChance ?? 0.33) * 100) + 10)
  );
    ...
    for (let x = 2; x < iWidth - 2; x++) {
      ...
      for (let my = -1; my <= 1 && (!nearActive || !nearPassive); my++) {
        for (let mx = -1; mx <= 1; mx++) {
          ...
          if (!nearActive && StoryTags.activeMargin && StoryTags.activeMargin.has(k))
            nearActive = true;
          if (!nearPassive && StoryTags.passiveShelf && StoryTags.passiveShelf.has(k))
            nearPassive = true;
        }
      }
      ...
      const baseAllowed =
        v > threshold &&
        (ctx
          ? ctxRandom(ctx, "Island Seed", baseIslandDen)
          : TerrainBuilder.getRandomNumber(baseIslandDen, "Island Seed")) === 0;
      ...
      if (isHotspot) {
        ...
        const roll = ctx
          ? ctxRandom(ctx, "HotspotKind", bucket || 1)
          : TerrainBuilder.getRandomNumber(bucket || 1, "HotspotKind");
        classifyParadise = roll < pWeight;
        if (!classifyParadise) {
          // Volcanic: rare cone peeking above sea level
          if (
            (ctx
              ? ctxRandom(ctx, "HotspotPeak", 100)
              : TerrainBuilder.getRandomNumber(100, "HotspotPeak")) < peakPercent
          ) {
      ...
      // Place center tile
      const centerIsLand =
        centerTerrain !== globals.g_CoastTerrain && centerTerrain !== globals.g_OceanTerrain;
      ...
      const count =
        1 +
        (ctx
          ? ctxRandom(ctx, "Island Size", maxCluster)
          : TerrainBuilder.getRandomNumber(maxCluster, "Island Size"));
