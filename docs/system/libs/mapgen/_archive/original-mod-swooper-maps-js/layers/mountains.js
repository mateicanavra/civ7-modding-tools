...
function computePlateBasedScores(ctx, scores, hillScores, options, isWaterCheck) {
  ...
  if (!upliftPotential || !boundaryType) {
    ...
    devLogIf &&
      devLogIf(
        "LOG_MOUNTAINS",
        "[Mountains] Falling back to fractal-only scores (missing WorldModel arrays)"
      );
  ...
  const isWater =
    typeof isWaterCheck === "function"
      ? isWaterCheck
      : (x, y) => GameplayMap?.isWater?.(x, y) ?? false;
  ...
  devLogIf &&
    devLogIf(
      "LOG_MOUNTAINS",
      `[DEBUG #1] Avg uplift on land: ${totalLandTiles > 0 ? (landUpliftSum / totalLandTiles).toFixed(1) : 0} / 255`
    );
  devLogIf &&
    devLogIf(
      "LOG_MOUNTAINS",
      `[DEBUG #1] Avg closeness on land: ${totalLandTiles > 0 ? (landClosenessSum / totalLandTiles).toFixed(1) : 0} / 255`
    );
  devLogIf &&
    devLogIf(
      "LOG_MOUNTAINS",
      `[DEBUG #1] Land by boundary type: none=${landByBoundaryType[0]}, convergent=${landByBoundaryType[1]}, divergent=${landByBoundaryType[2]}, transform=${landByBoundaryType[3]}`
    );
  devLogIf &&
    devLogIf(
      "LOG_MOUNTAINS",
      `[DEBUG #1] Land uplift buckets: <25=${landUpliftBuckets.under25}, 25-50=${landUpliftBuckets.under50}, 50-100=${landUpliftBuckets.under100}, >100=${landUpliftBuckets.over100}`
    );
  devLogIf &&
    devLogIf(
      "LOG_MOUNTAINS",
      `[DEBUG #1] Land closeness buckets: <64=${landClosenessBuckets.under64}, 64-128=${landClosenessBuckets.under128}, 128-192=${landClosenessBuckets.under192}, >192=${landClosenessBuckets.over192}`
    );
  ...
  rowStats.forEach((rs) => {
    devLogIf &&
      devLogIf(
        "LOG_MOUNTAINS",
        `[DEBUG #1]   row ${rs.row}: land=${rs.landCount}, avgUplift=${rs.avgUplift}, avgCloseness=${rs.avgCloseness}`
      );
  });
  devLogIf &&
    devLogIf("LOG_MOUNTAINS", "[DEBUG #1] ================================================");
  // ========== END DEBUG #1 ==========

  const boundaryGate = 0.2; // Minimum closeness (0-1) to consider boundary effects
