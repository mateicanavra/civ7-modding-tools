...
import {
  addVolcanoes as baseAddVolcanoes,
  isTooCloseToExistingVolcanoes,
} from "/base-standard/maps/volcano-generator.js";
...
export function layerAddVolcanoesPlateAware(ctx, options = {}) {
  ...
  if (!worldEnabled || !boundaryCloseness || !boundaryType) {
    devLogIf &&
      devLogIf(
        "LOG_VOLCANOES",
        "[Volcanoes] WorldModel unavailable; falling back to base generator."
      );
  ...
  if (candidates.length === 0) {
    devLogIf &&
      devLogIf(
        "LOG_VOLCANOES",
        "[Volcanoes] No candidates with positive weight; falling back to base generator."
      );
    baseAddVolcanoes(width, height, minSpacing);
    return;
  }
  ...
  for (const candidate of candidates) {
    ...
    if (isTooCloseToExistingVolcanoes(candidate.x, candidate.y, placed, minSpacingClamped))
      continue;
