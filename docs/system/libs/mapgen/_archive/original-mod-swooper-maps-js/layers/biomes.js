...
export function designateEnhancedBiomes(iWidth, iHeight) {
  ...
  for (let y = 0; y < iHeight; y++) {
    for (let x = 0; x < iWidth; x++) {
      ...
      if (lat < TCOAST_LAT_MAX && GameplayMap.isCoastalLand(x, y) && rainfall > TCOAST_RAIN_MIN) {
        TerrainBuilder.setBiomeType(x, y, globals.g_TropicalBiome);
      }
      // Temperate/warm river valleys prefer grassland for playability
      if (GameplayMap.isAdjacentToRivers(x, y, 1) && rainfall > RV_RAIN_MIN && lat < RV_LAT_MAX) {
      ...
      if (StoryTags.corridorLandOpen && StoryTags.corridorLandOpen.has(`${x},${y}`)) {
        if (
          rainfall > 80 &&
          lat < 55 &&
          TerrainBuilder.getRandomNumber(100, "Corridor Land-Open Biome") <
      ...
      if (StoryTags.corridorRiverChain && StoryTags.corridorRiverChain.has(`${x},${y}`)) {
        if (
          rainfall > 75 &&
          lat < 55 &&
          TerrainBuilder.getRandomNumber(100, "Corridor River-Chain Biome") <
        ...
        if (
          ...
          if (edgeAttr && edgeAttr.edge) {
            ...
            if (
              forestRimChance > 0 &&
              rainfall > 90 &&
              TerrainBuilder.getRandomNumber(100, "Corr Forest Rim") <
                Math.round(forestRimChance * 100)
            ) {
              const target =
                lat < 22 && rainfall > 110 ? globals.g_TropicalBiome : globals.g_GrasslandBiome;
            ...
            if (
              reliefChance > 0 &&
              TerrainBuilder.getRandomNumber(100, "Corr Relief Rim") <
      ...
      // Strategic Corridors: kind/style biome bias (very gentle; policy-scaled)
      {
        ...
        if ((cKind === "land" || cKind === "river") && biomesCfg) {
          ...
          if (
            ...
            let totalW = 0;
            for (const k of entries) totalW += Math.max(0, biomesCfg[k] || 0);
            if (totalW > 0) {
              ...
              let target = null;
              if (chosen === "desert") target = globals.g_DesertBiome;
              else if (chosen === "plains") target = globals.g_PlainsBiome;
              else if (chosen === "grassland") target = globals.g_GrasslandBiome;
              else if (chosen === "tropical") target = globals.g_TropicalBiome;
              else if (chosen === "tundra") target = globals.g_TundraBiome;
              else if (chosen === "snow") target = globals.g_SnowBiome;
              if (target != null) {
                // Light sanity gates to avoid extreme mismatches
                let ok = true;
                if (target === globals.g_DesertBiome && rainfall > 110) ok = false;
                if (target === globals.g_TropicalBiome && !(lat < 25 && rainfall > 95)) ok = false;
                if (target === globals.g_TundraBiome && !(lat > 60 || elevation > 800)) ok = false;
                if (target === globals.g_SnowBiome && !(lat > 70 || elevation > 900)) ok = false;
                if (ok) {
                  TerrainBuilder.setBiomeType(x, y, target);
                }
              }
