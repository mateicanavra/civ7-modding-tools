...
export function addDiverseFeatures(iWidth, iHeight, ctx) {
    ...
    for (const key of StoryTags.hotspotParadise) {
      ...
      for (let dy = -2; dy <= 2; dy++) {
        for (let dx = -2; dx <= 2; dx++) {
          ...
          if (GameplayMap.getFeatureType(nx, ny) !== FeatureTypes.NO_FEATURE) continue;
          if (
            (ctx
              ? ctxRandom(ctx, "Paradise Reef", 100)
              : TerrainBuilder.getRandomNumber(100, "Paradise Reef")) < paradiseReefChance
          ) {
            const canPlace =
              ctx && ctx.adapter
                ? ctx.adapter.canHaveFeature(nx, ny, reefIndex)
                : TerrainBuilder.canHaveFeature(nx, ny, reefIndex);
            if (canPlace) {
              if (ctx && ctx.adapter) {
                ctx.adapter.setFeatureType(nx, ny, {
                  Feature: reefIndex,
                  Direction: -1,
                  Elevation: 0,
                });
    ...
    for (const key of StoryTags.passiveShelf) {
      ...
      for (let dy = -1; dy <= 1; dy++) {
        for (let dx = -1; dx <= 1; dx++) {
          ...
          if (
            (ctx
              ? ctxRandom(ctx, "Shelf Reef", 100)
              : TerrainBuilder.getRandomNumber(100, "Shelf Reef")) < shelfReefChance
          ) {
            const canPlace =
              ctx && ctx.adapter
                ? ctx.adapter.canHaveFeature(nx, ny, reefIndex)
  ...
  for (let y = 0; y < iHeight; y++) {
    for (let x = 0; x < iWidth; x++) {
      ...
      if (STORY_ENABLE_HOTSPOTS && StoryTags.hotspotVolcanic.size > 0) {
        ...
        if (nearVolcanic) {
          ...
          if (
            forestIdx !== -1 &&
            rainfall > 95 &&
            (biome === globals.g_GrasslandBiome || biome === globals.g_TropicalBiome)
          ) {
            if (
              (ctx
                ? ctxRandom(ctx, "Volcanic Forest", 100)
                : TerrainBuilder.getRandomNumber(100, "Volcanic Forest")) < volcanicForestChance
            ) {
              const canPlace =
                ctx && ctx.adapter
                  ? ctx.adapter.canHaveFeature(x, y, forestIdx)
                  : TerrainBuilder.canHaveFeature(x, y, forestIdx);
              if (canPlace) {
                if (ctx && ctx.adapter) {
                  ctx.adapter.setFeatureType(x, y, {
                    Feature: forestIdx,
                    Direction: -1,
                    Elevation: 0,
                  });
          ...
          if (
            ...
            if (
              (ctx
                ? ctxRandom(ctx, "Volcanic Taiga", 100)
                : TerrainBuilder.getRandomNumber(100, "Volcanic Taiga")) < volcanicTaigaChance
            ) {
              const canPlace =
                ctx && ctx.adapter
                  ? ctx.adapter.canHaveFeature(x, y, taigaIdx)
                  : TerrainBuilder.canHaveFeature(x, y, taigaIdx);
              if (canPlace) {
                if (ctx && ctx.adapter) {
                  ctx.adapter.setFeatureType(x, y, {
                    Feature: taigaIdx,
      ...
      if (rainforestIdx !== -1 && biome === globals.g_TropicalBiome && rainfall > 130) {
        if (
          (ctx
            ? ctxRandom(ctx, "Extra Jungle", 100)
            : TerrainBuilder.getRandomNumber(100, "Extra Jungle")) < rainforestExtraChance
        ) {
          const canPlace =
            ctx && ctx.adapter
              ? ctx.adapter.canHaveFeature(x, y, rainforestIdx)
              : TerrainBuilder.canHaveFeature(x, y, rainforestIdx);
          if (canPlace) {
            if (ctx && ctx.adapter) {
              ctx.adapter.setFeatureType(x, y, {
      ...
      if (forestIdx !== -1 && biome === globals.g_GrasslandBiome && rainfall > 100) {
        if (
          (ctx
            ? ctxRandom(ctx, "Extra Forest", 100)
            : TerrainBuilder.getRandomNumber(100, "Extra Forest")) < forestExtraChance
        ) {
          const canPlace =
            ctx && ctx.adapter
              ? ctx.adapter.canHaveFeature(x, y, forestIdx)
      ...
      if (taigaIdx !== -1 && biome === globals.g_TundraBiome && elevation < 300) {
        if (
          (ctx
            ? ctxRandom(ctx, "Extra Taiga", 100)
            : TerrainBuilder.getRandomNumber(100, "Extra Taiga")) < taigaExtraChance
        ) {
          const canPlace =
            ctx && ctx.adapter
              ? ctx.adapter.canHaveFeature(x, y, taigaIdx)
