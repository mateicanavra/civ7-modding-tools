...
export function applyLandmassPostAdjustments(windows, geometry, width, height) {
  ...
  const clampWest = Number.isFinite(post.clampWestMin)
    ? Math.max(0, Math.trunc(post.clampWestMin))
    : null;
  const clampEast = Number.isFinite(post.clampEastMax)
    ? Math.min(width - 1, Math.trunc(post.clampEastMax))
    : null;
  const overrideSouth = Number.isFinite(post.overrideSouth)
    ? clampInt(Math.trunc(post.overrideSouth), 0, height - 1)
    : null;
  const overrideNorth = Number.isFinite(post.overrideNorth)
    ? clampInt(Math.trunc(post.overrideNorth), 0, height - 1)
    : null;
  const minWidth = Number.isFinite(post.minWidthTiles)
  ...
  const adjusted = windows.map((win) => {
    ...
    const mutated =
      west !== win.west || east !== win.east || south !== win.south || north !== win.north;
...
export function applyPlateAwareOceanSeparation(params) {
  ...
  const policy =
    params?.policy || worldModel?.policy?.oceanSeparation || FOUNDATION_OCEAN_SEPARATION;
  if (
    !policy ||
    !policy.enabled ||
    !worldModel ||
