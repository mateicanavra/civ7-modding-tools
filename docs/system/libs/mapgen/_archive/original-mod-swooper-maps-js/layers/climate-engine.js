...
function resolveAdapter(ctx) {
  ...
  return {
    ...
    isMountain: (x, y) =>
      GameplayMap.isMountain ? GameplayMap.isMountain(x, y) : GameplayMap.getElevation(x, y) >= 500,
    isCoastalLand: (x, y) => (GameplayMap.isCoastalLand ? GameplayMap.isCoastalLand(x, y) : false),
    isAdjacentToShallowWater: (x, y) =>
      GameplayMap.isAdjacentToShallowWater ? GameplayMap.isAdjacentToShallowWater(x, y) : false,
...
export function applyClimateBaseline(width, height, ctx = null) {
  ...
  const isCoastalLand = (x, y) => {
    ...
    if (typeof GameplayMap?.isCoastalLand === "function") return GameplayMap.isCoastalLand(x, y);
  ...
  const isAdjacentToShallowWater = (x, y) => {
    if (adapter.isAdjacentToShallowWater) return adapter.isAdjacentToShallowWater(x, y);
