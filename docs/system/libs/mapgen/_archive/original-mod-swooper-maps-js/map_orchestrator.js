...
import {
  ...
  logBoundaryMetrics,
  logLandmassAscii,
  logTerrainReliefAscii,
  logRainfallAscii,
  logBiomeAscii,
  logBiomeSummary,
  logFoundationSeed,
  logFoundationPlates,
  logFoundationDynamics,
  logFoundationSurface,
...
function generateMap() {
  ...
  const ctx = createMapContext({ width: iWidth, height: iHeight }, adapter, {
  ...
  if (stageLandmassPlates) {
    ...
    landmassWindows = applyLandmassPostAdjustments(
      landmassWindows,
      LANDMASS_GEOMETRY,
      iWidth,
      iHeight
    ...
    if (landmassWindowsFinal.length) {
      const windowSummary = landmassWindowsFinal.map((win, idx) => {
        if (!win) return { index: idx };
        const spanX =
          Number.isFinite(win.east) && Number.isFinite(win.west) ? win.east - win.west + 1 : null;
        const spanY =
          Number.isFinite(win.north) && Number.isFinite(win.south)
            ? win.north - win.south + 1
            : null;
        return {
          index: idx,
  ...
  console.log("[SWOOPER_MOD] About to calculate MAP_STATS...");
  {
    ...
    for (let y = 0; y < iHeight; y++) {
      for (let x = 0; x < iWidth; x++) {
        ...
        else if (GameplayMap.isMountain(x, y)) mountainCount++;
        else if (GameplayMap.getTerrainType(x, y) === globals.g_HillTerrain) hillCount++;
        else flatCount++;
      }
    }
    ...
    console.log(
      `[MAP_STATS] Total tiles: ${totalTiles}, Water: ${waterCount} (${waterPct}%), Land: ${landCount} (${landPct}%)`
    );
    console.log(
      `[MAP_STATS] Land breakdown: Mountains: ${mountainCount} (${mtnPct}%), Hills: ${hillCount} (${hillPct}%), Flat: ${flatCount} (${flatPct}%)`
    );
  }
  AreaBuilder.recalculateAreas();
