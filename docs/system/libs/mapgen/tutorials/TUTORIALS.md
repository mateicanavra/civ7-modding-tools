<toc>
  <item id="purpose" title="Purpose"/>
  <item id="audience" title="Audience"/>
  <item id="tutorials" title="Tutorial list"/>
  <item id="anchors" title="Ground truth anchors"/>
</toc>

# MapGen tutorials

## Purpose

Tutorials are learning-oriented, end-to-end walkthroughs.

## Audience

- Developers running and debugging MapGen end-to-end.
- Authors learning how complete config values affect outcomes (without extending the SDK).

## Tutorial list

Canonical tutorials:

- Run the standard recipe in Studio: [`docs/system/libs/mapgen/tutorials/run-standard-recipe-in-studio.md`](/system/libs/mapgen/tutorials/run-standard-recipe-in-studio.md)
- Inspect artifacts and projections: [`docs/system/libs/mapgen/tutorials/inspect-artifacts-and-projections.md`](/system/libs/mapgen/tutorials/inspect-artifacts-and-projections.md)
- Tune a complete recipe config: [`docs/system/libs/mapgen/tutorials/tune-a-preset-and-knobs.md`](/system/libs/mapgen/tutorials/tune-a-preset-and-knobs.md)

Potential future tutorials:
- Implement a new step end-to-end

## See also

- [`docs/system/libs/mapgen/MAPGEN.md`](/system/libs/mapgen/MAPGEN.md)
- [`docs/system/libs/mapgen/how-to/HOW-TO.md`](/system/libs/mapgen/how-to/HOW-TO.md)
- [`docs/system/libs/mapgen/reference/REFERENCE.md`](/system/libs/mapgen/reference/REFERENCE.md)

## Ground truth anchors

- Gateway: `docs/system/libs/mapgen/MAPGEN.md`
- Tutorial index (this file): `docs/system/libs/mapgen/tutorials/TUTORIALS.md`
