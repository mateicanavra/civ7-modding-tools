...
| G7 superseded current-stage ids | `normalize-ecology-topology`, `normalize-projection-lakes` | Generic domain, module, and recipe-stage topology blueprints protect current source kinds; there is no direct current doc-content rule. | No current registered rule decides whether evergreen prose presents a retired stage id as current source shape. |
