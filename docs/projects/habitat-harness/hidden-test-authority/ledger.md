...
| `mods/mod-swooper-maps/test/pipeline/no-dual-contract-paths.test.ts` | legacy and target stage tokens must not coexist or remain in runtime source. | leave package-local for now | `.habitat/civ7/mapgen/pipeline/structure` later if narrowed | package test | medium | Broad token scans are acceptable only with an explicit retired-token manifest. Do not fold into `op-calls-op`. |
| `mods/mod-swooper-maps/test/pipeline/no-shim-surfaces.test.ts` | runtime source must not use shim/shadow/dual/compare vocabulary. | leave package-local for now | `.habitat/civ7/mapgen/pipeline/structure` later if narrowed | package test | hard | High false-positive risk from broad vocabulary. Needs allowlisted token manifest and docs/code scope split. |
| `mods/mod-swooper-maps/test/pipeline/foundation-topology-lock.test.ts` | standard stage topology/order and retired aliases. | leave package-local for now | `.habitat/civ7/mapgen/pipeline/structure` later | package test or manifest-backed check | easy | Migrate only with a topology manifest; do not duplicate `STANDARD_STAGES` in a loose script. |
| `mods/mod-swooper-maps/test/pipeline/rng-authority-boundary.test.ts` | source scan for engine RNG, official generators, and internal RNG imports. | split later | `.habitat/civ7/mapgen/pipeline/capabilities/rng-authority-static` | source or command check | medium | Static call bans can migrate; runtime RNG consumption remains package behavior. |
...
| `mods/mod-swooper-maps/test/pipeline/map-stamping.contract-guard.test.ts` | mixed map projection callsite bans, stage order, contract naming, and source token rules. | split later | `.habitat/civ7/mapgen/pipeline/{capabilities,contracts,structure}` | source checks plus package tests | medium | Migrate only the static callsite/source-token slices. Stage ordering and adapter placement semantics stay package-local until a manifest exists. |
| `mods/mod-swooper-maps/test/foundation/contract-guard.test.ts` | mixed foundation legacy surface bans, contract shape, strategy imports, and artifact id assertions. | split later | mostly existing `domain-refactor-guardrails`; additional contracts TBD | command/source checks plus package tests | medium | Many static slices are already covered by full-profile domain-refactor guardrails. Do not remove behavioral contract assertions without parity rows. |
| `mods/mod-swooper-maps/test/morphology/contract-guard.test.ts` | mixed morphology legacy surface bans, overlay isolation, contract dependencies, and map-facing artifact assertions. | split later | `.habitat/civ7/mapgen/pipeline/{boundaries,contracts,structure}` | source checks plus package tests | medium | Static legacy import/token bans can move; artifact dependency assertions need typed contract manifest support. |
| `mods/mod-swooper-maps/test/morphology/catalog-ownership.test.ts` | domain config facade shape, op schema ownership, and recipe tag catalog naming. | split later | existing `domain-refactor-guardrails` plus future contracts subject | command/source checks | medium | Static import/name bans are good candidates; exact facade exports need a manifest or typed check. |
...
| `bun run --cwd mods/mod-swooper-maps test:architecture-cutover` | pass | Owner package command chain now executes the Habitat-owned `op-calls-op` subject script, then leaves remaining cutover package tests local. |
...
| `nx run mod-swooper-maps:test:architecture-cutover --outputStyle=static` | pass | Nx owner target reaches the Habitat-owned op command script and remaining package-local tests. |
| `bun run --cwd tools/habitat-harness test -- test/rules/registry/contract.test.ts test/rules/registry/facts.test.ts` | pass | Registry schema/facts accept the migrated owner-tool counts: 31 `source-check`, 10 `command-check`. |
...
| `bun test mods/mod-swooper-maps/test/pipeline/no-dual-contract-paths.test.ts mods/mod-swooper-maps/test/pipeline/no-shim-surfaces.test.ts mods/mod-swooper-maps/test/pipeline/foundation-topology-lock.test.ts` | pass | Remaining cutover package tests stay package-local in this slice. |
