...
| S11 | https://docs.grit.io/guides/config | `.habitat/grit.yaml`, pattern metadata, imports, `.gritignore`, suppression |
...
- [CONFIDENT] Repo patterns have documented structure. Grit config lives at `.habitat/grit.yaml`; patterns can be listed with `name`, `title`, `body`, `description`, `level`, and `tags`. `level` is one of `none`, `info`, `warn`, `error` and defaults to `info`. Patterns in `.grit/patterns` are imported by default. Markdown patterns in `.grit/patterns` use file name as pattern name, first heading as title, first non-heading paragraph as description, first fenced code block as body, and subheadings for tests. (S10, S11, S12, S13)
