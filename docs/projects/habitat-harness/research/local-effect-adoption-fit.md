...
**Biome/Nx orchestration.** Biome is invoked through `runFix` as a sequential command after Grit (`tools/habitat-harness/src/lib/command-engine.ts:173-183`). Nx affected verification is one command result (`tools/habitat-harness/src/lib/command-engine.ts:189-202`), and the Nx plugin infers targets with static command strings (`tools/habitat-harness/src/plugin.js:25-179`). A fresh `nx show project @internal/habitat-harness --json` showed these inferred targets exist for the harness project, including `grit:check`, `biome:*`, `boundaries`, `generated:check`, and `habitat:check`. Effect may harden command provenance, but Nx target existence and graph semantics must remain proven through Nx output, not Effect.
