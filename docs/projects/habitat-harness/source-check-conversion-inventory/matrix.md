...
| `prohibit_map_projection_dependencies_in_physics_contracts` | mapgen-other | command-check | no | no | high: path-scoped string/source matching over standard-recipe physics contracts is clean Grit pattern authority candidate |
| `prohibit_realized_map_artifact_tags` | mapgen-other | command-check | no | no | deferred: cross-source projection contract surface pressure, not a Grit conversion candidate until ownership is framed |
| `require_standard_recipe_map_effect_name_suffixes` | mapgen-other | command-check | no | no | high: exact tag-contract regex constraint is clean Grit pattern authority candidate |
...
| `prohibit_pnpm_files_in_bun_workspace` | global-docs-toolkit | file-layer | no | no | low: this is staged path/file-name guard metadata, better represented as a generic file-layer rule than a source pattern. |
