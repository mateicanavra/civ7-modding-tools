...
Status: synthesized review complete; updated after systematic burn-down
...
| Pre-canary coverage review: adapter crosswalk included exactly 33 centralized adapters. | P1 | superseded by canary and burn-down | The canary deleted 7 adapter files; the systematic burn-down deleted 25 more. `mechanical-counts.json` now records `adapters: 1`. |
| Pre-canary source-check mismatch: 29 active source-check records vs 33 adapters. | P1 | cleared by canary and burn-down | The 4 stale command-check adapters were deleted, 28 active rules were converted to `grit-check` across canary plus systematic burn-down, and current state is 1 active source-check record with 1 adapter. |
...
| `require_explicit_mapgen_sdk_opt_in` must not be bulk-converted as a whole row. | P1 | accepted holdback | `grit-burn-down/lanes/holdback-review.jsonl` records that the adapter mixes SDK opt-in checks with a mapgen-core adapter-import ban overlapping `preserve_mapgen_core_runtime_neutrality`; it remains the single source-check adapter until split. |
