...
| DL-12 | P3 | pre-H4: `.prettierrc` existed; no doc or gate mentioned formatting | Biome config is present and Biome now owns formatter, ordinary hygiene lint, import organization, `biome:ci`, `habitat fix`, `habitat check`, and CI wiring. H4 closure is still blocked by unrelated DL-15/DL-16 proof gates, not by missing hygiene enforcement. | Resolved by slice H4 (Biome); no decision needed |
