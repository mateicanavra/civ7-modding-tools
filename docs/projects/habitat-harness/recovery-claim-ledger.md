...
- root `lint` runs the graph-owned `@internal/habitat-harness:biome:ci`
...
| CLAIM-H4-BIOME | "Biome excluded protected archives and quarantined ESLint." | `openspec/changes/habitat-biome-hygiene/workstream/phase-record.md`; `docs/projects/habitat-harness/FRAME.md` H4 trade-off; current `biome.json`; `eslint.boundaries.config.mjs`; `tools/habitat-harness/src/plugin.js`; `tools/habitat-harness/test/lib/biome-closure.test.ts` | Current repair proof keeps Biome as formatter, hygiene linter, and import organizer; keeps generated/protected zones excluded; keeps direct Prettier surfaces retired; exposes namespaced `biome:*` Nx targets; registers exactly one `biome-ci` Habitat rule; and keeps ESLint quarantined to `@nx/enforce-module-boundaries`. Later Habitat harness changes can still create ordinary included-file formatting drift; the current command surface repairs that drift through the Biome owner layer and proves `@internal/habitat-harness:biome:ci` plus Habitat `--rule biome-ci` green again. Full root CI execution, broad root `check`/`test`, generated-output semantics, HG row proof, and product/runtime behavior remain non-claims. | verified current repair behavior with non-claims | yes-with-non-claims | records-only unless included-file Biome drift or ownership/config drift appears | `habitat-biome-hygiene` | `bun run nx run @internal/habitat-harness:biome:ci --outputStyle=static --skipNxCache` |
| CLAIM-H45-CLI | "Habitat is a real oclif CLI through root/dev/production surfaces." | `openspec/changes/habitat-oclif-cli/**`; D7 in `FRAME.md`; current command probes | Root help and subcommand help render through `bun run habitat`; a clean build followed by `bun tools/habitat-harness/bin/run.js --help` renders the production oclif runner help; unknown root commands fail through oclif; unknown rule/tool selections fail before rule execution through `rule-selection-integrity`. This repairs the command-trust surface without claiming Grit row semantics, baseline behavior, CI execution, or product/runtime proof. | verified current command behavior with non-claims | yes-with-non-claims | records-only; consume current oclif entrypoint contract, do not reopen command trust unless root/dev/prod command routing drifts | `habitat-oclif-entrypoint-repair` | `bun run habitat -- --help` |
...
| CLAIM-P0-PROD-RUNNER | "Production runner help is current-proofed after build." | `tools/habitat-harness/bin/run.js`; H4.5 production proof claim | `bun run nx run @internal/habitat-harness:build --outputStyle=static --skipNxCache` exits 0, and the built production runner `bun tools/habitat-harness/bin/run.js --help` exits 0 with the same root command list. | verified current behavior | yes | records-only; consume current production runner proof | `habitat-oclif-entrypoint-repair` | `bun run nx run @internal/habitat-harness:build --outputStyle=static --skipNxCache` |
