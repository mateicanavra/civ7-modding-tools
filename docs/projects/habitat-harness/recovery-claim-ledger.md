...
| CLAIM-H7-HOOKS | "hooks are bounded, local, and reversible." | `openspec/changes/habitat-git-hooks/**`; `tools/habitat-harness/src/lib/hooks.ts` | Current repair removes implicit default pre-commit resource publishing and adds a read-only resource-state gate that fails closed for dirty, uninitialized, locked, or unstaged-gitlink resource states with explicit remediation. Clean resources and clean staged gitlinks continue through the local staged hook path. Full hook transaction proof, Grit hook parse/finding proof, pre-push range proof, and CI authority remain open. | verified current repair behavior with blockers | mixed-with-blockers | repair | `habitat-git-hook-hardening` | `bun tools/habitat-harness/bin/dev.ts hook pre-commit` |
