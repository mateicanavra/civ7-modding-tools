...
| CLAIM-H6-ONE-PATH | "structural enforcement has one canonical Habitat path." | `openspec/changes/habitat-enforcement-consolidation/**`; `workstream-record.md` H6 row; current root `package.json` | Root structural proof enters the Nx DAG: `lint` runs project lint plus Habitat checks, `verify` runs package-owned verifier targets, and `check` aggregates build/check/lint/test/verify. Surviving wrapped-script/wrapped-test rules have explicit parser/output/retirement dispositions, stale owner-tool selections such as `wrapped-eslint` fail through `rule-selection-integrity` without a `baseline-integrity` false green, and generated map-bundle runtime-import freshness is repaired through the owning map artifact generator plus generated-zone drift gate. Direct diagnostic aliases remain classified; wrapper retirement, CI execution, broad Nx affected coverage, Grit row semantics, baseline semantics, and product/runtime proof remain non-claims. | verified current behavior with non-claims | yes-with-non-claims | records-only unless root command routing, wrapper policy, selector integrity, or generated-output freshness drifts | `habitat-enforcement-surface-cleanup` | `bun run habitat:check -- --json` |
