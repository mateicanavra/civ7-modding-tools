...
- All fresh D2 investigation docs under `/Users/mateicanavra/Documents/.nosync/DEV/worktrees/wt-codex-deep-habitat-openspec-remediation/docs/projects/habitat-harness/openspec-remediation/agent-scratch/domino-D2-*-investigation.md`
...
The type-state model is no longer left to implementation. D2 chooses `RuleRegistryDocumentV1` with `schemaVersion: 1`, a closed `RuleRegistryRecord` union, and `ownerTool` as compatibility selector vocabulary/internal discriminant (`design.md:93-115`). Variant constraints eliminate the old optional-field states for missing Grit pattern, file-layer policy ambiguity, wrapped-test graph target ambiguity, and hidden hook/prose fallbacks (`design.md:132-142`).
