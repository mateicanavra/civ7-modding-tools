...
| --- | --- | --- |
| Exact combined focused D1 test command | Yes | `openspec/changes/deep-habitat-d1-receipt-contract-boundary/tasks.md`, item 6.6, now names `bun run --cwd tools/habitat-harness test -- test/commands/habitat-entrypoints.test.ts test/lib/proof-artifact.test.ts test/lib/verify-proof.test.ts test/lib/hooks.test.ts test/lib/grit-apply.test.ts`; `workstream/phase-record.md`, Validation Gates row for the same command, matches it. |
