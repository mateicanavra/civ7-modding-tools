...
- `tools/habitat-harness/src/generators/project/schema.json:15` exposes `kind` as
  ...
  `tools/habitat-harness/src/generators/project/schema.json:18`.
- `tools/habitat-harness/src/generators/project/generator.cjs:4` defines the
  actual write contracts for only `plugin`, `foundation`, and `app`; line 22
  derives `SUPPORTED_KINDS` from those contracts.
- `tools/habitat-harness/src/generators/project/generator.cjs:51` normalizes raw
...
- Unsupported kinds throw a generic `Error` at
  `tools/habitat-harness/src/generators/project/generator.cjs:54` with rationale
  embedded in a string.
- Root and package mismatch refusals are also error strings at
  `tools/habitat-harness/src/generators/project/generator.cjs:66` and
  `tools/habitat-harness/src/generators/project/generator.cjs:72`.
- Tests prove no writes for several cases:
  `tools/habitat-harness/test/generators/project-generator.test.ts:90`,
  `tools/habitat-harness/test/generators/project-generator.test.ts:101`,
  `tools/habitat-harness/test/generators/project-generator.test.ts:116`,
  `tools/habitat-harness/test/generators/project-generator.test.ts:131`, and
  `tools/habitat-harness/test/generators/project-generator.test.ts:148`.
...
- `tools/habitat-harness/src/generators/pattern/schema.json:24` exposes
  `lifecycle` as `"candidate" | "registered-advisory" | "registered-enforced"`.
- `tools/habitat-harness/src/generators/pattern/generator.cjs:41` normalizes the
  lifecycle from a raw value and throws for unsupported strings.
- Candidate writes are under
  `tools/habitat-harness/src/generators/pattern/generator.cjs:22`; registered
  promotion is selected by `options.lifecycle !== "candidate"` at
  `tools/habitat-harness/src/generators/pattern/generator.cjs:7`.
- Candidate manifests correctly encode `lifecycle: "candidate"` and
  `registration.accepted: false` at
  `tools/habitat-harness/src/generators/pattern/generator.cjs:101`.
- Pattern Authority already models candidate vs registered manifests as a
  discriminated union at
  `tools/habitat-harness/src/rules/pattern-authority/manifest.ts:67` and
  `tools/habitat-harness/src/rules/pattern-authority/manifest.ts:80`.
...
- Candidate generation proves it does not write active enforcement state at
  `tools/habitat-harness/test/generators/pattern-generator.test.ts:20`.
- Registered generation refuses missing manifest before writes at
  `tools/habitat-harness/test/generators/pattern-generator.test.ts:57` and
  `tools/habitat-harness/test/generators/pattern-generator.test.ts:71`.
- Promotion writes active `.grit` and `rules.json` only after accepted manifest
  and baseline contract at
  `tools/habitat-harness/test/generators/pattern-generator.test.ts:139`,
  `tools/habitat-harness/test/generators/pattern-generator.test.ts:160`, and
  `tools/habitat-harness/test/generators/pattern-generator.test.ts:209`.
- Candidate collision checks guard existing rule, active pattern, and baseline
  states at `tools/habitat-harness/test/generators/pattern-generator.test.ts:239`,
  `tools/habitat-harness/test/generators/pattern-generator.test.ts:252`, and
  `tools/habitat-harness/test/generators/pattern-generator.test.ts:279`.
...
- Pattern Authority explicitly rejects Nx generator options as authority at
  `tools/habitat-harness/src/rules/pattern-authority/manifest.ts:258`; the
  corresponding test is
  `tools/habitat-harness/test/rules/pattern-authority-manifest.test.ts:236`.
...
- Existing capabilities state Habitat is not a broad MapGen authoring toolkit at
  `tools/habitat-harness/docs/CAPABILITIES.md:10` and lists recipe/domain/op/stage
  generation as assumptions Habitat cannot yet answer at
  `tools/habitat-harness/docs/CAPABILITIES.md:25`.
- Existing gaps list the unsupported authoring structures at
  `tools/habitat-harness/docs/GAPS.md:17` and say the current project and pattern
  generators are not MapGen authoring generators at
  `tools/habitat-harness/docs/GAPS.md:29`.
- Domain mapping frames Future MapGen authoring as a gap, not an implemented
  Habitat context, at `tools/habitat-harness/docs/DOMAIN-MAPPING.md:348`.
...
- **Reuse canonical owner modules.** Reuse
  `tools/habitat-harness/src/rules/pattern-authority/manifest.ts` for registered
...
- `tools/habitat-harness/src/generators/pattern/registration.cjs`, only if
  registered promotion boundary needs a narrower input projection
- `tools/habitat-harness/src/rules/pattern-authority/manifest.ts`, only to reuse
...
- `tools/habitat-harness/test/rules/pattern-authority-manifest.test.ts`, only if
  manifest projection behavior changes
- `tools/habitat-harness/docs/CAPABILITIES.md`,
  `tools/habitat-harness/docs/GAPS.md`, and
  `tools/habitat-harness/docs/DOMAIN-MAPPING.md`, only to align command-facing
...
- `tools/habitat-harness/src/rules/rules.json` except through the accepted
  registered-pattern promotion decision
- `tools/habitat-harness/baselines/**` except pre-existing baseline contract
  verification; D13 should not invent or grow baselines
- `tools/habitat-harness/src/rules/pattern-authority/candidates/**` except
...
- Existing command gates should be corrected and extended:
  - keep `bun run --cwd tools/habitat-harness test -- test/generators/project-generator.test.ts test/generators/pattern-generator.test.ts test/rules/pattern-authority-manifest.test.ts`;
  - replace `bun run habitat generate --help` with an actual Nx generator help
    ...
    `classify`, `fix`, `graph`, and `hook` (`tools/habitat-harness/docs/CAPABILITIES.md:34`);
  - require `nx g @internal/habitat-harness:project unsupported-scratch --kind=mod --dry-run` to fail before writes;
...
- Existing Habitat docs list unsupported MapGen authoring structures at
  `tools/habitat-harness/docs/GAPS.md:17` and state the current generators are
  not MapGen authoring generators at `tools/habitat-harness/docs/GAPS.md:29`.
...
- The generator schema description currently says the pattern generator
  ...
  `tools/habitat-harness/generators.json:14`, which is false for candidate
...
- Pattern Authority rejects Nx-option-shaped authority at
  `tools/habitat-harness/src/rules/pattern-authority/manifest.ts:258`.
