...
The source D7 packet explicitly requires current `CheckReport` field inventory, selector scenarios, and baseline/Grit consumer boundaries. The OpenSpec `design.md` does not inventory the current enforcement state at all. It never enumerates the current responsibilities mixed in `tools/habitat/src/lib/command-engine.ts`: selector validation, selected rule set construction, staged Grit filtering, Grit/native/file-layer execution, baseline load/application, baseline integrity, status derivation, report construction, JSON validation, rendering, output writing, and verify consumption.
