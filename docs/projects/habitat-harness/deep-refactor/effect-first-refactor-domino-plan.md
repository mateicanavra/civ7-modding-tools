...
| `GritProvider` | `src/providers/grit/**` | pattern discovery, scan-root admission, check, dry-run apply, pattern tests, cache acquisition, diagnostics/output parsing | `GritProviderError` |
...
| P1 | Grit Effect island is not a provider model | `adapters/grit/runner.ts`; `adapters/grit/request.ts`; `habitat-process.ts`; `workspace-tools.ts` | Grit has more structure than other vendors, but provider concepts are not reusable | Promote Grit into `providers/grit`; use shared provider contracts for version, command, config, parse, cache, failure, proof | Current Grit CheckReport behavior and apply dry-run behavior | Grit parser matrix, scan-root matrix, native pattern tests, current-tree check |
...
- Existing Grit parser/failure facts that are already correct, relocated under
  `providers/grit` or `domains/diagnostic-pattern-catalog`.
