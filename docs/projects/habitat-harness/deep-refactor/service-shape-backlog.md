...
- Follow-up: active rule registry loading still has compatibility singleton consumers in fix admissions, diagnostic rule architecture, artifact-path planning, and the exported active-facts constants; continue replacing those with service-projected rule facts or explicit registry inputs so rule facts are not coupled to global checkout state.
