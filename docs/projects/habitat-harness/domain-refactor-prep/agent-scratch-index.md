...
- Full required skill reads before analysis
- No implementation

Findings from an earlier bad-checkout wave were discarded and are not source material for this corpus.

## Wave 1: Independent Mapping

Wave 1 agents produced clean reports from the mandated worktree and branch, but did not write durable scratch files into this directory. Their results are treated as non-auditable owner synthesis and are not independent source authority after compaction. The consolidated corpus, not raw Wave 1 claims, is the durable Phase 2 input.

| Agent | Lane | Status | Synthesis Use |
| --- | --- | --- | --- |
| `019ed7e3-fd22...` | Product/scenario mapper | Accepted | Supported/unsupported scenarios, product gaps, Authoring Topology fence. |
| `019ed7e4-3d8a...` | Code-flow tracer | Accepted | Command flows, side effects, exports, test anchors. |
| `019ed7e4-8934...` | TypeScript state-space mapper | Accepted | Option bags, ADT candidates, generic boundary risks. |
| `019ed7e4-d754...` | Proof/enforcement mapper | Accepted | Proof classes, rule inventory, non-claims. |
| `019ed7e5-11a9...` | Docs/OpenSpec reconciler | Accepted | Authority order, stale-record risks, downstream realignment. |

## Wave 2: Dependency Modeling

Wave 2 agents wrote scratch documents under this directory.
...
Wave 3 ran after the consolidated corpus existed. The review disposition is recorded in `review-disposition-ledger.md`.
...
The DRA owner is accountable for the consolidated corpus. Agent scratch is evidence, not authority. The consolidated documents are the compaction-survivable source for Phase 2.
