...
| Scaffold supported uniform project | Create only supported generic uniform project shapes. | Scaffolding owns supported kinds and refusal rules. | `nx g @internal/habitat-harness:project <name> --kind=<foundation|plugin|app>`. | Generator tests and refusal tests. | Not MapGen authoring topology. |
