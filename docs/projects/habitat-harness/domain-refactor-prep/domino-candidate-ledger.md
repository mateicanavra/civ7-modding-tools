...
10. D12 Verify Handoff Command after D1, D3, and D7.
...
| D12 | Verify Handoff Command | Verify receipt owner | DRA, reviewers, agents handing off work | `habitat verify` runs check then affected Nx and emits a bounded receipt | Verify depends on check shape, graph target list, post-state, and stream parsing | D1, D3, D7 | Phase 2 closure model | VerifyReceipt schema/tests and command behavior | Verification contract |
