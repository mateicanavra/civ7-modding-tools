...
patterns with dry-run inventory, isolated-copy receipt, path approval, rollback,
...
paths, run handoff gates, and leave rollback/non-goal receipt.
...
- Patterns owns apply admission decisions.
- Host Policy Boundary owns host-specific gates.
- The format-check lane owns formatting, not transaction safety.

## Consumers

`habitat fix`, future approved apply packets, DRA owners, Patterns,
Hook Runtime.
...
through D10 for generated/protected policy and directly for host-specific
pattern gates.

## Current State-Space Problem

`grit-apply.ts` combines generic transaction mechanics, one approved pattern,
inventory parsing, copy receipt, changed path approval, MapGen-specific public ops
...
2. Consume Patterns apply approval and Host Policy gate declarations.
...
5. Keep Biome and other gates as handoffs with receipt/non-goal labels.
...
MapGen-specific gate without host declaration, or success receipt with failed
formatter handoff.
...
- apply safety matrix;
- Patterns ledger rows for apply-approved patterns;
- command docs;
...
- `git status --short --branch`: expected exit 0 before and after dry-run; for
  write-mode fixture receipt, only expected fixture files may change.
- Cache stance: dry-run and isolated-copy receipt must run fresh, not from Nx cache.
...
change. Commit only with clean worktree and rollback receipt.
...
- Biome/gate success is reported as apply safety or product receipt.
