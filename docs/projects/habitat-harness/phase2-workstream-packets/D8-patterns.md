...
Separate Patterns lifecycle from Grit diagnostics, baselines, and apply
...
only after manifest, fixture, baseline, hook-scope, false-positive, and receipt
...
- Scaffolding may create candidates but not register enforced rules by
  implication.
- Pattern Apply may only consume approved apply patterns.

## Consumers

Pattern generator, pattern registration script, Structural Enforcement,
Diagnostic Pattern Catalog, Pattern Apply, Hook Runtime,
recovery ledgers.
...
Each state names required sources, fixtures, receipt classes, baseline contract,
...
1. Model Patterns lifecycle as discriminated states.
...
README text. The product requires refusals and receipt gates.
...
- diagnostic receipt linkage tests;
...
- Injected bad case: include a candidate without accepted manifest, fixture
  receipt, or hook-scope decision and prove it cannot become registered.
...
D6 receipt contracts are available.
