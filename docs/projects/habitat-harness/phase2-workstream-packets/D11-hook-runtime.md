...
Refactor hooks as hook runtime consumers of published Habitat contracts,
without letting pre-commit or pre-push become receipt authority.
...
- Protected Zones owns staged generated-zone rules.
- Diagnostic Pattern Catalog owns staged Grit diagnostics.
- Pattern Apply owns apply/write safety.
...
- hook pass is not CI.
- hook pass is not review receipt.
- hook pass is not runtime/product receipt.
- hook pass is not full apply safety receipt.
...
- Injected bad case: include a staged generated-zone mutation and a failing
  wrapped check; prove hooks report hook runtime without claiming CI receipt.
...
- hook code owns receipt semantics instead of consuming published contracts;
- resource state can contradict allowed commit behavior;
- hooks restage or rewrite outside explicit policy;
- hook success is described as CI or product receipt.
