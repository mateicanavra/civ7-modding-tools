...
- [x] Studio tests cover operation status, failure details, and request
  validation.
...
- [x] Live proof matrix recorded or explicitly bounded by current Civ
  availability.
- [x] Graphite branches committed and repo left clean.
...
- Route-level middleware tests for every HTTP branch were not added; browser
  proof covers the primary disposable route and helper/UI tests cover the
  status contract.
