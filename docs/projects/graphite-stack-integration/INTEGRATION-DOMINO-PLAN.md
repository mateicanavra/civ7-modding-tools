...
1. **Trivia** — delete `codex/graphite-stack-drain-skill` (merged); submit+merge `restore-civ7-orpc-skill`; merge `codex/sieve-engine-reference` (docs-only, 1 file, verified).
2. **Placement s0–s11** — restack onto main (mechanical: zero file overlap with main drift), publish, merge bottom-up (draft PRs #1565–#1580 already open). Retire `placement-realignment` lab branch now. Keep `placement-live-integration` + `wt-placement-live-proof` alive as the rivers conflict oracle.
3. **Taxonomy stack (#1576–#1586)** — zero overlap with placement; merges on operator review. May run parallel with 2.
4. **Dag-viz** — restack onto main and drain (operator approved dag-first ordering vs the studio redesign). The kept code lands on main as-is; the **redesigned** incarnation is specified separately for the studio agent (see §6).
5. **Rivers** — fold 64 → 6 survivors top-down, strip superseded clusters during the fold, restack onto post-placement/post-taxonomy main using `placement-live-integration` to resolve the 46-file placement overlap, re-point kept studio glue to taxonomy endpoints. Then retire the oracle (after salvaging its two unique bits, §5.3).
...
## 6. Dag-viz → Studio-redesign handoff (spec to be written at domino 4)

The dag-viz code drains to main as-is (it is correct, tested, and feature-complete), but its **presentation shell predates the studio redesign's design system**. A handoff spec will be written for the studio redesign agent covering: the DAG tab as a redesigned Studio feature (not a port of the old chrome); the stable semantic core to preserve (recipe DAG projection contract from `packages/mapgen-core/src/authoring/recipe-dag.ts`, the effect-oRPC service/router/schema under `src/server/recipeDag/`, layout/domain-presentation semantics, the test suite); and integration heads-up for the redesign lane's restack (the 7 colliding files, `App.tsx` monolith deletion vs DAG client mount → re-mount inside the decomposed shell). Spec location (planned): `docs/projects/mapgen-studio-redesign/` or alongside this doc — recorded here once written.
