...
3. **Taxonomy stack** — ✅ EXECUTED 2026-06-12: integration-DRA skim found no blockers (the `.civ7` gitlink bump is the known convergent snapshot; the `game map` dual surface is the documented D2 decision); restacked onto main conflict-free; gates direct-control 424, control-orpc 345, cli 234; all 8 PRs (**#1576–#1579, #1582, #1584–#1586**) merged; branches retired.
...
5. **Rivers** — ✅ EXECUTED 2026-06-12: dirty-worktree gate cleared by adjudication (Studio-serializer noise — key reorders + a restated contract default — discarded; config-suite invariant verified). Folded 64 branches → **5 survivors** via staged rebase surgery (13 superseded branches dropped whole: #26, #37–41, #46, #50–52, #59–61; survivor 5 absorbed #42): `rivers-hydrology-core` (24c), `studio-server-restoration` (4c incl. activeInputContext game-ui carry-fix), `rivers-benchmarks-and-inspector` (7c), `native-river-proof-capability` (3c), `rivers-final-fixes-and-closure` (19c incl. cleanup + oracle-salvage commits). Oracle (`placement-live-integration`) used as planned: rerere preimages resolved the placement-overlap conflicts; its river-tile exclusion re-expression (demand-pipeline mask), migrated mountain configs, ecology baselines, public-surface imports, and 30s metrics-test timeouts were adopted verbatim. Cleanup stripped the verify-river-visible-proof harness + probe-river-modeling scripts (findings recorded in ledger docs) and the temporary AGENTS.md session notice; `probe-river-writer` kept pending the deferred cherry-pick evaluation. Gates: full typecheck, mod-swooper-maps 569, mapgen-studio 174, direct-control 427, control-orpc 345, map-policy 17, studio-server 2. Drained as **#1596–#1600**.
6. **Drop lineage-audit stack** — ✅ EXECUTED 2026-06-12: all 73 branches (verified visualizer-only, zero remote refs, local-only) deleted leaf-down per recorded disposition.
