...
| D6 | `mapgen-studio-operations-current` | accepted | Operations current packet accepted after runtime lifecycle, browser recovery deletion, TypeBox/testing, and hardening/black-ice repair reviews; requires terminal-only recent semantics, exact D4/D3 lifecycle matrix for expired-known/pruned/never-known/identity-mismatched ids, D4 `StudioOperationRuntime.current` ownership, shell boot adoption through `operationAdoption.ts` without request-id/status replay, deletion of browser operation recovery storage, protected authoring/preset/theme storage owners, recoverable TypeBox `TSchema` origin with canonical operation DTO reuse, and D8/D9 event-hook hello-adoption protection. |
...
| D12 | `mapgen-studio-game-door-invariant` | accepted; implementation submitted with live/merge-drain proof still open | Game-door invariant closeout packet accepted after direct-control/prework, hardening/black-ice, testing/schema/runtime, and final corpus review repairs; requires evergreen game-door invariant, exact production `Civ7DirectControlSession` constructor guard, TypeBox/Zod contract closeout, retained public/manual status endpoint corpus with diagnostic, mutation-state, and identity classifications, `@civ7/control-orpc` procedure-key/risk corpus, hosted controller bridge proof classification, `RunInGameHttpError` and `StudioEngineError` bridge residue disposition, tuner-session follow-up closeout, active docs/OpenSpec residue searches, and final proof ledger. The D12 implementation branch records submitted Graphite PRs plus the remaining live Civ7 and Graphite merge/sync/drain proof in its closure checklist and next-packet records; packet acceptance is not the same as final runtime-stack drain. |
