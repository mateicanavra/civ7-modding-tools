...
| Unit command | selected or first ready unit; sometimes target plot | `game play unit ready`, then the named unit action; plot actions require `game play unit target` and never route through generic operation validation |
