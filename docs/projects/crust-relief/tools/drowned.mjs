...
for (let i = 0; i < N; i++) {
  ...
  if (co) {
    coastByDist[b] = (coastByDist[b] || 0) + 1;
  }
  if (c === 1) {
    contTot++;
    contSub++;
    if (co) {
      contSubShelf++;
      coastContByDist[b] = (coastContByDist[b] || 0) + 1;
    }
    contSubElev.push(bathy ? bathy.v[i] : 0);
  } else {
    ocTot++;
    ocSub++;
    if (co) ocShelf++;
  ...
  JSON.stringify(
    {
      ...
      map: {
        ...
        coast: coastN,
        ocean: oceanN,
        coastPctMap: pct(coastN, N),
        oceanPctMap: pct(oceanN, N),
      },
      coastByDistToLand: coastByDist,
      coastOnContinentalByDist: coastContByDist,
      crustOfCoast: {
        onContinental: contSubShelf,
        onOceanic: ocShelf,
        contShareOfCoast: pct(contSubShelf, coastN),
      },
      continentalCrust: {
        total: contTot,
        emergentLand: contLand,
        submerged: contSub,
        submergedPctOfCont: pct(contSub, contTot),
        submergedThatAreShelf: contSubShelf,
      },
      submergedContinentalBathy: {
        n: contSubElev.length,
        min: contSubElev[0],
        p10: qv(0.1),
        p50: qv(0.5),
        p90: qv(0.9),
        max: contSubElev[contSubElev.length - 1],
      },
    },
    null,
    2
