...
const rd = (k, kind) => {
  ...
  return { v: new Int16Array(b.buffer, b.byteOffset, Math.floor(b.byteLength / 2)), w: d.width, h: d.height };
};
...
let contLand = 0, contSub = 0, ocLand = 0, ocWater = 0, contHigh = 0;
const lines = [];
for (let y = 0; y < H; y++) {
  ...
  for (let x = 0; x < W; x++) {
    ...
    if (mode === "crust") {
      ...
      else if (e > 30) ch = "@"; else if (e > 15) ch = "^"; else if (e > 5) ch = "#"; else ch = "-";
    } else {
      // drown mode
      if (isLand) {
        if (isCont) { contLand++; if (e > 15) { ch = "^"; contHigh++; } else ch = "#"; }
        else { ocLand++; ch = "o"; }
      } else {
        if (isCont) { contSub++; ch = "+"; }
        else { ocWater++; ch = isCoast(i) ? ":" : "."; }
      }
    }
    row += ch;
  }
