...
- `.codex/environment.toml` is the executable Desktop configuration.
...
- Prime every managed worktree completely: initialize and validate resources, perform a frozen Bun install, bootstrap the Habitat Nx-plugin artifact, then build the workspace.
...
| --- | --- | --- |
| TOML syntax | `python3 -c 'import tomllib; tomllib.load(open(".codex/environment.toml", "rb"))'` | Required before handoff. |
