...
- Keep the action bar owner-oriented. Builds, resource lifecycle, apps, Studio
  development, MapGen evidence, deployment, and Tuner health are visible;
  routine validation chains, formatter writes, Graphite actions, broad cleanup,
  direct game mutations, and package publishing remain documented terminal
  workflows.
- Run Studio through its one Nx-owned continuous graph. The environment action
  owns the foreground process, and stopping its terminal stops the graph.
...
| Studio task graph | `nx run mapgen-studio:dev --graph=stdout` plus a bounded startup/interrupt probe | The daemon is a continuous dependency of Vite, producer tasks run first, and one interrupt stops both listeners. |
