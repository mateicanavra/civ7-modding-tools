...
- **Improve-slice candidates** (separate, flagged, tested): dead write-only `setLiveRuntimeSnapshot` (529); `useLatestRef` formalization (invariant a); effect→derived conversions ONLY where a pinned test exists (manualEra clamp 2164, overlay-variant-pref 2198 are the safest candidates; stage/step defaults are riskier — they're in a Tier-A cascade).
