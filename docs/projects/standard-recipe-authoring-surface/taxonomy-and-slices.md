...
| projection `map-*` | Prior projection surfaces exposed empty runtime step keys, lake readback diagnostics, raw feature-apply op envelopes, and a wrong-owner `riverProjection` threshold surface. | misplaced owner layer; internal parameter leakage; bad naming, ranges, or defaults | Completed by `projection-authoring-surface-alignment`: expose only semantic projection keys (`knobs`, `biomeBindings`), keep empty steps/readback/feature guard internals compiled/defaulted, compile map-rivers knobs into the Hydrology-owned `selectNavigableRiverTerrain` op envelope, enum-bound biome globals with `marine` fixed to `BIOME_MARINE`, and prove shipped-config compile equivalence. |
| placement | Prior placement surface exposed raw `derive-placement-inputs` envelopes, empty product/effect step keys, adapter candidate lists, and runtime start-sector overrides. | internal parameter leakage; coupled low-level fields; misplaced owner layer | Completed by `placement-authoring-surface-alignment`: expose semantic product controls (`naturalWonders`, `discoveries`, `floodplains`, `resources`), compile to internal placement planner/product step configs, keep adapter resource catalogs and runtime starts out of authoring config, and prove shipped-config compile equivalence. |
| shared SDK/Studio | Studio renders whatever generated schema exposes, and generated map entrypoints are the SDK-facing boundary for shipped map configs. | generated or Studio-only schema leakage | Completed by `studio-sdk-authoring-surface-guards`: guard source stage authoring models, generated Studio schema/default/uiMeta artifacts, built-in presets, generated map entrypoint config extraction, and exact source-config hash identity. |
