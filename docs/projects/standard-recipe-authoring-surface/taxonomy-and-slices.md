...
| projection `map-*` | Prior projection surfaces exposed empty runtime step keys, lake readback diagnostics, raw feature-apply op envelopes, and configurable biome bindings. | misplaced owner layer; internal parameter leakage; duplicate policy authority | Superseded by the projection state collapse: configless projection stages persist exactly `{}`, readback evidence is unconditional, map-rivers retains only its real density knob, and Swooper binds Ecology symbols to official Civ7 biome identities through fixed local projection policy. |
