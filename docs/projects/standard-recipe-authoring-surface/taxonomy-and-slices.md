...
| projection `map-*` | Prior projection surfaces exposed empty runtime step keys, lake readback diagnostics, raw feature-apply op envelopes, and unbounded biome globals alongside valid river projection thresholds. | misplaced owner layer; internal parameter leakage; bad naming, ranges, or defaults | Completed by `projection-authoring-surface-alignment`: expose only semantic projection keys (`riverProjection`, `biomeBindings`), keep empty steps/readback/feature guard internals compiled/defaulted, enum-bound biome globals with `marine` fixed to `BIOME_MARINE`, and prove shipped-config compile equivalence. |
