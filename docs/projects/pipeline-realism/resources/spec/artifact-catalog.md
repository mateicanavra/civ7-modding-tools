...
| `artifact:foundation.crustInit` | truth | mesh | `{ type[cell], age[cell], buoyancy[cell], baseElevation[cell], strength[cell], maturity/thickness/thermalAge/damage... }` | basaltic-lid initial crust truth (t=0) | `(mesh, mantleForcing)` | Foundation | `foundation.crustInit.*` |
| `artifact:foundation.crust` | truth | mesh | `{ type[cell], age[cell], buoyancy[cell], baseElevation[cell], strength[cell], maturity/thickness/thermalAge/damage... }` | evolved canonical lithosphere/crust truth (post-history) | `(crustInit, tectonics, history, provenance)` | Foundation | `foundation.crust.*` |
...
| `artifact:foundation.crustTiles` | projection | tile | `{ type[tile], age[tile], buoyancy[tile], baseElevation[tile], strength[tile] }` | projected crust drivers sampled via `tileToCellIndex` | `(crust, tileToCellIndex)` | Morphology + debug | `foundation.crustTiles.*` |
...
| `artifact:morphology.beltDrivers` | truth | tile | `{ boundaryCloseness[u8], boundaryType[u8], upliftPotential[u8], riftPotential[u8], tectonicStress[u8], beltMask[u8], ... }` | unified belt/mountain drivers derived from Foundation history + provenance | `(tectonicHistoryTiles, tectonicProvenanceTiles)` | Morphology + Map-morphology | `morphology.belts.*` |
