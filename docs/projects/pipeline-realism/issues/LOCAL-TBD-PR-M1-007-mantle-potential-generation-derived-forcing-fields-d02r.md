...
Mantle forcing truth artifacts are now generated by dedicated mesh-first ops, and the Foundation stage wires them into the canonical step ordering.
...
- `mods/mod-swooper-maps/src/recipes/standard/stages/foundation/steps/mantlePotential.ts`
- `mods/mod-swooper-maps/src/recipes/standard/stages/foundation/steps/mantleForcing.ts`
...
- Mantle potential is seeded via deterministic Poisson-disk sampling over the mesh, with `minSeparationScale = 0.85` and a fixed smoothing budget (`smoothingIterations = 2`, `smoothingAlpha = 0.35`).
- Advanced authoring knobs map to physics parameters as:
  - `potentialAmplitude01` → amplitude scale `lerp(0.4, 2.0, value)` (applied symmetrically to plumes/downwellings).
  - `lengthScale01` → radius `lerp(0.08, 0.35, value)` for both plume and downwelling kernels.
  - `plumeCount`/`downwellingCount` clamped to `[2, 16]`.
- Forcing derivation uses neighbor finite-difference gradients + Laplacian curvature:
  - `stress = clamp01((|grad| + 0.35 * |laplacian|) / stressNorm)`.
  - `forcing = -velocityScale * grad + rotationScale * rot90(grad)`.
  - `divergence` is the normalized Laplacian (signed).
  - `upwellingClass` uses local extrema with thresholds `0.35`.
- Visualization emits `foundation.mantle.potential` (scalar), `foundation.mantle.forcing` (magnitude + vector segments), plus debug layers for stress and upwelling class.
