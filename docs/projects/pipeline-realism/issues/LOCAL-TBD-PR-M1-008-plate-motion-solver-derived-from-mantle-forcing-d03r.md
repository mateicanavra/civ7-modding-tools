id: LOCAL-TBD-PR-M1-008
+title: Plate motion solver derived from mantle forcing (D03r)
+state: planned
+priority: 2
+estimate: 8
+project: pipeline-realism
+milestone: M1-foundation-maximal-cutover
+assignees: [codex]
+labels: [pipeline-realism]
+parent: null
+children: []
+blocked_by: [LOCAL-TBD-PR-M1-007]
+blocked: []
+related_to: []
+---
+
+<!-- SECTION SCOPE [SYNC] -->
+## TL;DR
+- Derive plate motion as a strict function of mantle forcing (no author intent).
+
+## Deliverables
+- Derive plate motion as a strict function of mantle forcing (no author intent).
- Define/emit `artifact:foundation.plateMotion` with fixed solver/smoothing budgets.
+
+## Acceptance Criteria
+- Deliverables are implemented and wired into the pipeline where applicable.
+- Outputs follow the maximal SPEC contracts (no optional artifacts).
+- Any transitional bridge has an explicit deletion target (or this issue performs the deletion).
+
+## Testing / Verification
+- Add/extend the canonical validation suite for this change (D09r posture).
+- Verify determinism: same seed + config -> identical artifacts (stable fingerprints).
+
+## Dependencies / Notes
+- Blocked by:
  - `LOCAL-TBD-PR-M1-007`
- Related:
  - (none)
+
+### References
+- docs/projects/pipeline-realism/resources/decisions/d03r-plate-motion-derived-from-mantle.md
- docs/projects/pipeline-realism/resources/spec/sections/plate-motion.md
+
+---
+
+<!-- SECTION IMPLEMENTATION [NOSYNC] -->
+## Implementation Details (Local Only)
+
+### Quick Navigation
+- [TL;DR](#tldr)
+- [Deliverables](#deliverables)
+- [Acceptance Criteria](#acceptance-criteria)
+- [Testing / Verification](#testing--verification)
+- [Dependencies / Notes](#dependencies--notes)
+