...
**Status:** **EXECUTING** — checkpoint COMPLETE 2026-07-01 (Q2/E1–E4 all decided; D3–D6 below); B0 SHIPPED 2026-07-01 (draft PR #1997, gates green, 14-agent fan-out review folded); B1 SHIPPED 2026-07-01 (draft PR #1998, scaffold + theme single-source + token fixture, review folded — no-generator policy, nx test→build edge); B2 SHIPPED 2026-07-02 (draft PR #1999, foundation + 16 primitives + full app rewire, review folded — dev-only vite alias made exact-match regex after a string key prefix-captured the theme.css/fonts.css subpath imports and 500'd `vite dev`; app storybook targets now declare their dist deps); B3 SHIPPED 2026-07-02 (draft PR #2000, 12 composites+docks + task-3.1 deletions + clsx/tailwind-merge prune, fan-out review CLEAN zero findings; WaterStatsSection generic adjudicated as contravariance compile-blocker; domainPresentation lucide-regex stack regression fixed in B2 with non-spanning [^}]*); B4 SHIPPED 2026-07-02 (draft PR #2001, forms — five-module rjsf unit moved atomically, CSP deep-subpath byte-verbatim + dist grep 0 `new Function`/0 ajv, @rjsf pruned from app, floor 77→90, fan-out review CLEAN zero findings; adjudications: validator parity test lives in package test/ not co-located [dts-emit leak], fieldIds extraction stays scheduled for E3 per tasks §8.1 [supersedes the LEDGER's "with forms" note], mockWidgetProps' bare self-import in src/storybook is DESIGNED per storybook-sync rule 3, typeboxRjsfValidator reformat token-PROVEN format-only); B5 SHIPPED 2026-07-02 (draft PR #2002, panels + the two designed severances — statusLabels formatters/constructors split with type-only contract imports + recipe-dag split with RecipeDagLoadStatus re-homed into PipelineStage and useRecipeDagQuery back-importing from the barrel; floor 90→99, E1-C fence proven in dist, TS7056=0, buildRecipeDagLayout byte-parity via parent-tip fixture, fan-out review CLEAN zero findings; adjudications: parseArtifactPresentation added to barrel [postdated-test split — pure half in package test/, corpus half app-side], stale isLightMode test attrs removed [internal const, never a prop — app never typechecked tests], ExplorePanel generic per B3's WaterStats shape; lucide-react app prune deferred to B6 where its last importer AppHeader moves); stack per openspec `studio-ui-extraction` tasks.md: B0 `@civ7/studio-contract` → B1 scaffold → B2–B6 moves → B7 sync repoint → B8+ cleanup wave
...
- **D7 (2026-07-02, Matei directive + recon):** **nx target shapes follow the Habitat Toolkit stack** (`codex/habitat-*`, #1925–#1957+; recon read-only over `wt-agent-DRA-habitat-authority-tree-pruning-frame`). Verdict: the extraction stack is ALREADY compliant — package `"nx"` blocks are metadata-only overlays on script-inferred targets (sanctioned coexistence; no same-name script+explicit-command duplication), the app storybook targets' cross-project `{projects:["mod-swooper-maps"], target:"build:studio-recipes"}` edge is the standard form and survives the stack (target moves from root targetDefaults to per-project, same address), taxonomy registration mechanism unchanged, `kind:foundation → kind:foundation` constraints hold. NO rework in B0–B3. Merge-order reconciliation (whichever side lands second): (1) taxonomy count pins move to `tools/habitat/test/lib/validate_boundary_taxonomy_against_workspace_graph.test.ts` (harness relocates `tools/habitat-harness` → `tools/habitat`; nx plugin → `./tools/habitat/src/nx-plugin.ts`) — re-assert our +2 project rows/pins there; (2) boundary audit re-validates tags/imports automatically, expected green; (3) no `.habitat/index.json` ownerRoots entry (UI package owns no habitat rules); (4) B4–B7 style: keep per-package nx additions metadata-only; an explicit-command target goes to sibling `project.json` with the same-name script deleted ("one command truth per task").
- (open) D8+: further decisions land here with reasons.
