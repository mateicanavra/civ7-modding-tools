...
**Default**
- `placement.landmassRegions.regionSlot` — `tile.hexOddQ::grid` — west/east assignment used to split starts.
- `placement.starts.startPosition` — multi-render (grid membership + point markers).

**Debug**
- `placement.starts.sectorId` — `tile.hexOddQ::grid` — diagnostic view of start-sector activation.
...
- Primary expressions (default, debug OFF):
  - `tile.hexOddQ::grid:membership` — player membership per tile (0 = none).
  - `tile.hexOddQ::points` — point markers at chosen plots (colored per player).
