...
| SA-07 | `studio-run-generation-manifest` | Studio server owns one internal Run in Game request-workspace topology for private manifest, artifact id, and correlation helpers; generated `.mapgen-studio/run-in-game/<requestId>/` state remains runtime evidence, not source topology. | structure-check | `structure-studio-run-workspace-topology` | Studio server operation runtime workspace owner | `packages/studio-server/src/operationRuntime` | registered enforced | committed empty baseline after workspace writer lands | none |
