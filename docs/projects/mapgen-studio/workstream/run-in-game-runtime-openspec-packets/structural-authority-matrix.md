...
| SA-14 | `studio-run-diagnostics-retention-guards` | All temporary packet-local Grit patterns are removed or promoted, and authority rows SA-01 through SA-13 remain registered under their exact owners with canonical empty baselines. | Habitat command check | `habitat-studio-run-runtime-authority-closure` | authority ledger and registered rule topology | `.habitat`, `docs/projects/mapgen-studio/workstream/run-in-game-runtime-openspec-packets`, packet write set | registered enforced | closure leaf checks authority-set topology and blocks unresolved temporary patterns; one Nx workspace policy graph executes the MapGen Studio and Swooper Maps owner-local targets | none |
