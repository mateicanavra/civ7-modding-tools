...
| SA-04 | `swooper-catalog-identity-membership` | A map config catalog has one closed identity-membership, envelope-admission, and facade shape; membership contains ids rather than repository paths. | structure-check + Grit | `require_map_config_catalog_source_topology`, `require_map_config_catalog_file_shape` | map config catalog kind | `mods/*/src/maps/catalog` | registered enforced | committed empty baselines after the path-index authority retired | none |
