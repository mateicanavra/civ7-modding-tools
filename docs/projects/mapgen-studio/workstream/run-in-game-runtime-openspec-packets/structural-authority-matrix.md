...
| SA-14 | `studio-run-diagnostics-retention-guards` | All temporary packet-local Grit patterns are removed or promoted, and every surviving authority row remains registered under its exact owner with a canonical empty baseline. | Habitat command check | `habitat-studio-run-runtime-authority-closure` | authority ledger and registered rule topology | `.habitat`, `docs/projects/mapgen-studio/workstream/run-in-game-runtime-openspec-packets`, packet write set | registered enforced | closure leaf checks the currently listed authority-set topology and blocks unresolved temporary patterns; one Nx workspace policy graph executes the MapGen Studio and Swooper Maps owner-local targets | none |
