...
- **Where:**
  - the `standard-config` module in `mod-swooper-maps` (Standard recipe config assembly)
    - `type DeepPartial<T>`
...
- **Where:** the `standard-config` module in `mod-swooper-maps` (throughout `buildStandardRecipeConfig(...)`)
...
- **Where:**
  - the `standard-config` module in `mod-swooper-maps` (e.g., `as FoundationConfig`)
  - Standard recipe step implementations in `mod-swooper-maps` (e.g., `as LandmassConfig`, `as VolcanoesConfig`, etc.)
...
- **Where:** (previously) the `config loader` module in `mod-swooper-maps` (`parseConfig`, `safe-parse helper`, `default-config helper`, `json-schema helper`, `public-schema helper`)
...
- **Where:** the `execution-plan` runtime module in `@swooper/mapgen-core` (engine)
...
- **Where:** the authoring SDK’s `createOp` strategy selection module (strategy union + default handling)
...
- **Where:** ecology ops commonly default config inside `run(...)` (e.g., the ecology `classifyBiomes` op)
...
- **Where:** ecology plot-effects config resolver helpers (e.g., `resolvePlotEffectsConfig(...)`)
...
- **Where:** steps sometimes default op configs manually before calling ops (e.g., the Standard recipe ecology `biomes` step)
