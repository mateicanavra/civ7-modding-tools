...
12. **Refined land water budget and observation** (`compute-land-water-budget` → `compute-climate-diagnostics`): the post-hydrography climate pass derives the simplified **PET** proxy and `aridityIndex` = PET/(PET + precip + 1)-style ratio, then produces invocation-local rain-shadow, continentality, and convergence evidence for optional visualization.

**The climate index outputs you reason against** (where each lives — do not re-derive locally): `surfaceTemperatureC` ← thermal-state; `pet` + `aridityIndex` ← land-water-budget; `freezeIndex` ← cryosphere; `rainShadowIndex` / `continentalityIndex` / `convergenceIndex` ← the climate module's `compute-climate-diagnostics` observation operation; `effectiveMoisture` + `aridityIndex` are **consumed** by ecology `classify-biomes` as advisory indices (its contract explicitly says "do not re-derive from rainfall/humidity locally").
