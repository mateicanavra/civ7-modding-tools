...
**The climate index outputs you reason against** (where each lives — do not re-derive locally): `surfaceTemperatureC` ← thermal-state; `pet` + `aridityIndex` ← land-water-budget; `freezeIndex` ← cryosphere; `rainShadowIndex` / `continentalityIndex` / `convergenceIndex` ← the climate-diagnostics op's invocation-local visualization result; `effectiveMoisture` + `aridityIndex` are **consumed** by ecology `classify-biomes` as advisory indices (its contract explicitly says "do not re-derive from rainfall/humidity locally").
